...
# To check the Id
data %>% 
  filter(File_Id == "P16LAY1") %>% # P16HOBO1
...
BELOW = (ABOVE-1) * Calibration_factors
...
# A FAIRE
data %>% 
  mutate(Ring1 = ifelse(Position == "BELOW", ))
```
...
# P16HOBO
By <- by(Transmittance_cor[2:3], Transmittance_cor[-(2:3)], cor)
...
```{r}
ggplot(Transmittance_data, aes(x = Transmittance_P16HOBO1, y = Transmittance_P16HOBO2))+
  geom_point()+
  stat_smooth(method = "lm", formula = y ~ x) +
  facet_grid(vars(Ring), scales = "free")
