...
```{r}
path <- "D:/Mes Donnees/PhD/Microclimate/Light/LAI-2200/DATA/TEST.TXT"
...
ggplot(data_long, aes(x = Time, y = Value, col = Position)) + 
  ...
  scale_x_discrete(breaks = c("09:53:54", "10:07:22", "10:18:47", "10:21:48"))

ggsave("LAI2200_calibration_data.png",
...
path <- "D:/Mes Donnees/PhD/Microclimate/Light/LAI-2200/DATA/"
...
ggsave(paste("LAI2200_AB",pattern, ".png", sep=""),
       path = "D:/Mes Donnees/PhD/Figures/Sensors/LAI2200",
       width = 25, height = 20, units = "cm", dpi=800, bg="white")
...
ggsave(paste("LAI2200_B_repet",pattern, ".png", sep=""),
       path = "D:/Mes Donnees/PhD/Figures/Sensors/LAI2200",
       width = 25, height = 20, units = "cm", dpi=800, bg="white")
```
...
ggplot(mean, aes(x = Id, y = MeanTransmittance_LAI2200, col = File_Id)) + 
  geom_point() +
  facet_grid(vars(Ring), scales = "free")
...
mean %>% 
  filter(Ring=="Ring1") %>% 
  ggplot() + 
  geom_point(aes(x = factor(Id), y = MeanTransmittance_LAI2200))
...
```{r}
# For LAY
LAY <- read_csv("~/PhD/Microclimate/Light/LAI-2200/LAI2200_data_P16LAY_mean.csv")
# names(LAY)
LAY <- LAY %>%
  select(-c(`...1`,`...2`, Transmittance_mean)) %>% 
  group_by(Id, Ring) %>% 
  mutate(MeanTransmittance_LAI2200 = mean(Transmittance)) %>% 
  mutate(SdTransmittance_LAI2200 = sd(Transmittance)) %>% 
  mutate(SeTransmittance_LAI2200 = plotrix::std.error(Transmittance))
...
ggplot(LAY, aes(x = Id, y = MeanTransmittance_LAI2200, col = LightHabitat)) + 
  geom_point() +
  facet_grid(vars(Ring), scales = "free")

# For HOBO
HOBO <- read_csv("~/PhD/Microclimate/Light/LAI-2200/LAI2200_data_P16HOBO_mean.csv")
...
ggplot(HOBO, aes(x = Id, y = MeanTransmittance_LAI2200, col = LightHabitat)) + 
  geom_point() +
  facet_grid(vars(Ring), scales = "free")
...
LAI2200 <- bind_rows(LAY, HOBO) %>%   # 22 col, 765 rows
...
```{r}
LAI2200 <- read_csv("D:/Mes Donnees/PhD/Microclimate/Light/LAI-2200/LAI2200_data.csv")
