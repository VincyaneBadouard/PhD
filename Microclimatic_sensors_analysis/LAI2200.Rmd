...
data[data$File_Id=="P16LAY1",]$Id # P16HOBO5
...
AB <- data_long %>%
  ...
  mutate(Time = hms::as_hms(Time)) %>%
  mutate(TimeB = hms::as_hms(ifelse(!is.na(BELOW), Time, NA))) %>% 
  mutate(TimeA = hms::as_hms(ifelse(!is.na(ABOVE), Time, NA)))
...
```{r}
ggplot(Transmittance_cor1, aes(x = Transmittance_P16LAY1, y = Transmittance_P16LAY2))+
  geom_point()+
  stat_smooth(method = "lm", formula = y ~ x) +
  facet_grid(vars(Ring), scales = "free")
