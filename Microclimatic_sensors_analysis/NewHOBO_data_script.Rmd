...
FluxTowerData2022 <- FluxTowerData2022 %>% 
  filter(Year >= 2022) %>% 
  mutate(Date = as.Date(`Julian Day`, origin = "2021-12-31")) %>% # Julian Day to calendar date
  select(Year, 'Julian Day', Month, Date, Rain, Rg, Rn, 'Temp(55)') %>% # 365 days, 12 months
  mutate(Season = ifelse(Month  %in% c(7:11), # juill-nov = dry season
                         "Dry", "Wet")) %>% 
  mutate(Month = factor(Month))

FluxTowerData2022 <- FluxTowerData2022 %>% # for 2022 tower data
  mutate(Day = day(Date))
...
test0 <- HOBOdata %>%
  ...
  ungroup() %>% 
  left_join(FluxTowerData2022, by= c("Month","Day")) %>% # for 2022 tower data
  # left_join(FluxTowerData2022, by="Date") %>% # when I have 2023-2024 tower data
