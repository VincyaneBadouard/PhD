...
**Intercalibration !!**:
- intercalibration de la T° sur la période dans le bureau (application de la différence)
- intercalibration moisture même méthode avec données nouragues tous au même endroit)
...
# *Relation between VolumetricMoisture and temperatures*
...
# Correlation test all values (no means)
```{r}
...
cor.test(DF_cor$VolumetricMoisture, DF_cor$Pluie, method= "pearson") # 0.1
...
# faire d'abord la médiane par jour et par capteur 
daily <- Tomstdata %>%
  ...
  mutate(AirTemp = quantile(., .95)) # valeur hautes de T° de la journée
mutate(SurfaceTemp = quantile(., .95)) %>%
  mutate(SoilTemp = quantile(., .95)) %>%
  ungroup() 
...
annual_stats <- Tomstdata %>%
  ...
  summarise(across(c("VolumetricMoisture", "SoilTemp", "SurfaceTemp", "AirTemp"),
                   list(annual_mean = ~mean(., na.rm=T),
                        ...
                        annual_Q95 = ~quantile(., .95), # quantile 95%
                        annual_Q05 = ~quantile(., .05)), # quantile 5%
...
October_stats <- Tomstdata %>%
  ...
  summarise(across(c("VolumetricMoisture", "SoilTemp", "SurfaceTemp", "AirTemp"),
                   list(october_mean = ~mean(., na.rm=T), # october
                        ...
                        october_Q95 = ~quantile(., .95), # quantile 95%
                        october_Q05 = ~quantile(., .05)), # quantile 5%
                   .names = "Tomst_{.col}_{.fn}"))
...
#                         sd = ~sd(.),
#                         varcoef = ~(sd(.) / mean(.)) * 100,
#                         Q95 = ~quantile(., .95), # quantile 95%
# Q05 = ~quantile(., .05)), # quantile 5%
#                    .names = "Tomst_{.col}_{.fn}"))
# 
...
Tomstdata %>% 
  filter(TomstSensorNum == 94223161) %>% 
...
Tomstdata %>% 
  group_by(TomstSensorNum, Date, TopoHabitat) %>% 
  summarise(VolumetricMoisture = median(VolumetricMoisture)) %>% 
  group_by(TomstSensorNum) %>% 
  summarise(VolumetricMoisture = median(VolumetricMoisture)) %>% 
...
Tomstdata %>% 
  group_by(TomstSensorNum, Date, Sables, Season, TopoHabitat) %>% 
  summarise(VolumetricMoisture = mean(VolumetricMoisture)) %>% 
  group_by(TomstSensorNum, Sables, Season, TopoHabitat) %>% 
  na.omit() %>% 
  ...
  gather(metric, value, -TomstSensorNum, -Sables, -Season, -TopoHabitat) %>% 
...
stats %>%
  ...
  ggplot() +
  aes(x = TopoHabitat, y = Tomst_VolumetricMoisture_max_Wet) +
...
stats %>%
  filter(!is.na(TopoHabitat)) %>%
  ggplot() +
  aes(x = TopoHabitat, y = Tomst_VolumetricMoisture_max_Dry) +
  geom_boxplot(fill = "#4698B4") +
...
stats %>%
  ...
  ggplot() +
  aes(x = LightHabitat, y = Tomst_VolumetricMoisture_max_Wet) +
  geom_boxplot(fill = "#4698B4") +
...
stats %>%
  filter(!is.na(TopoHabitat)) %>%
  ggplot() +
  aes(x = TopoHabitat, y = Tomst_AirTemp_max_Night) +
...
stats %>%
  filter(!is.na(LightHabitat)) %>%
  ggplot() +
  aes(x = LightHabitat, y = Tomst_SoilTemp_annual_max) +
  geom_boxplot(fill = "brown") +
...
corrplot(CorMatrix, method="circle", type="lower", diag = F, addCoef.col = 'white', 
         ...
         tl.col="black", tl.cex = 1, tl.srt=45, p.mat = Pval_corr) #avec une croix pour les p-value > 0.05.
