...
format_tms <- function(
  ...
  # The Time in UTC
  Time_UTC <- ymd_hm(data$V2)
  Time <- ymd_hm(data$V2, tz = "America/Cayenne")
...
```{r}
path <- "D:/Mes Donnees/PhD/Microclimate/Tomst - data and scripts/LollyData/Paracou/Paracou_2023_05_25_1311"
...
files <- str_remove(files, "_2023_05_25_0.csv")
...
unique(Tomstdata[Tomstdata$SoilTemp<0,]$Time_UTC)
```
...
Tomstdata <- Tomstdata %>% 
  ...
  filter(Time >= mdy_hms("03/30/2023 18:00:00", tz = "America/Cayenne") & Time < mdy_hms("03/30/2024 18:00:00", tz = "America/Cayenne"))

Tomstdata$Time[1] >= mdy_hms("03/30/2023 18:00:00")
```
...
ggplot(Tomstdata) +
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
cor(Tomstdata$AirTemp, Tomstdata$SurfaceTemp) # 0.96
...
ggplot(Tomstdata) +
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
cor(Tomstdata$AirTemp, Tomstdata$SoilTemp) # 0.53
...
ggplot(Tomstdata) +
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
cor(Tomstdata$SurfaceTemp, Tomstdata$SoilTemp) # 0.69
...
ggplot(Tomstdata) +
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
cor(Tomstdata$Moisture, Tomstdata$SoilTemp) # -0.02
...
ggplot(Tomstdata) +
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
cor(Tomstdata$Moisture, Tomstdata$AirTemp) # -0.005
...
ggplot(Tomstdata) +
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
cor(Tomstdata$Moisture, Tomstdata$SurfaceTemp) # 0.02
...
ggplot(Tomstdata, aes(x = Hour,y = Moisture)) +
  geom_boxplot(aes(color = Phase)) +
  labs(title="Moisture over the day",
       x ="Hour", y = "Moisture")

ggplot(Tomstdata[Tomstdata$Phase=="Night",], aes(x = Hour,y = AirTemp)) +
  geom_point() +
  labs(title="Night air temperature over a day",
       x ="Hour", y = "Air temperature (°C)")

range(Tomstdata[Tomstdata$Phase == "Night",]$AirTemp) # 21.25 - 28.00 °C
range(Tomstdata[Tomstdata$Phase == "Day",]$AirTemp) # 20.50 - 36.75 °C
```
...
```
Linked to LightHabitat:
- Maximum annual surface temperature
- Maximum annual air temperature
- Average annual air temperature
- Average dry season surface temperature
- Maximun wet season surface temperature
- Average dry season air temperature
- Average wet season air temperature
- Maximum wet season air temperature
