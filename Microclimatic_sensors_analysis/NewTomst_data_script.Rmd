...
- RH(55) : Mean Air humidity at 55 m (%)
- RH_Avg_55_CR3000: Air humidity at 55 m  (%) (autre capteur)
...
- RH(2.5) : Air humidity at 2.5 m
- Tsoil : mean Soil temperature  (°C)
- SWC : mean Soil water content (m3 m-3) ()
- VWC : mean Soil water content (m3 m-3) (autre capteur)
...
FluxTowerData2023 <- FluxTowerData2023 %>% 
  ...
  mutate(Season = ifelse(Month  %in% c(3, 8:11), # juill-nov = dry season
                         "Dry", "Wet"))

ggplot(FluxTowerData2023, aes(x = as.character(Month),y = Rain)) +
