...
Which species:\
- 2 convex: Licania_alba, Licania_membranacea - sigmo: no ones - 2 concaves: Oxandra_asbeckii, Iryanthera_hostmannii - 2 no-effects: Lecythis_persistens, Eschweilera_sagotiana
...
as.data.frame(summary(fits[["Lecythis_persistens - 10-20"]],
...
data$InterestSp <- data.table::tstrsplit(data$speciesSt, split = " - ")[[1]]
data$Stratum <- data.table::tstrsplit(data$speciesSt, split = " - ")[[2]] # 2nd

data <- data %>% 
  mutate(Stratum = factor(Stratum, levels = c("1-10", "10-20",">20"))) %>%
    mutate(InterestSp = as.factor(InterestSp)) %>% 
       arrange(InterestSp)
```
...
for(p in 1:length(unique(data$speciesSt))){
  plist[[p]] <- local({
    ...
    ggplot(dataS, aes(x = logTransmittance)) +
      xlim(min(data$logTransmittance), 0) + # fix x
      theme_classic() +
...
### All the species in the same plot

### log

```{r}
plist <- vector('list', length(unique(data$InterestSp)))

for(p in 1:length(unique(data$InterestSp))){
  plist[[p]] <- local({
    
    dataS <- data %>% 
      filter(InterestSp==unique(data$InterestSp)[p])
    
    cl <- unique((dataS[dataS$ScientificName == dataS$InterestSp,])$cl)
    
    label <- dataS %>% filter(ScientificName == InterestSp) %>% select(NSt, Size, Opt) %>% unique()
    
    ggplot(dataS, aes(x = logTransmittance, col=Size)) +
      xlim(NA, 0) +
      theme_classic() +
      theme(plot.title = element_text(colour = cl)) +
      # Presences
      ...
      # Optimum
      geom_vline(aes(xintercept = Opt, col=Size), linewidth=1) + # exp to delog
      # geom_vline(aes(xintercept = `Opt.5%`, col=Size), linetype="dashed") +
      # geom_vline(aes(xintercept = `Opt.95%`, col=Size), linetype="dashed") +
      # Abundances label
      geom_text(data = label, aes(label = NSt, color=Size, x=Opt),
                y = Inf, hjust = 1.5, vjust = 3, show.legend = F) + 
      
      
      labs(title = unique(dataS$InterestSp), y = "Presence probability", col= "Size (cm DBH)")
...
ggsave("RealspPredictsMedian_DBHcat_log_per_sp.pdf",
       path = "D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Plots",
       plots, width = 15, height = 10)
```
...
for(p in 1:length(unique(data$InterestSp))){
  plist[[p]] <- local({
    ...
    label <- dataS %>% filter(ScientificName == InterestSp) %>% select(NSt, Size) %>% unique() %>% 
      mutate(NSt = paste("N=", NSt)) %>% 
      arrange(Size) %>% 
      mutate(x= c(0.65, 0.8, 1))
    
    ggplot(dataS, aes(x = Transmittance, col=Size)) +
      xlim(0, 1) +
      theme_classic() +
      theme(plot.title = element_text(colour = cl)) +
      # Presences
      ...
      geom_ribbon(aes(ymin = `5%`, ymax = `95%`), alpha = 0.2) +
      # Optimum
      geom_vline(aes(xintercept = exp(Opt), col=Size), linewidth=1) + # exp to delog
      # geom_vline(aes(xintercept = exp(`Opt.5%`), col=Size), linetype="dashed") +
      # geom_vline(aes(xintercept = exp(`Opt.95%`), col=Size), linetype="dashed") +
      # Abundances label
      geom_text(data = label, aes(label = NSt, color=Size, x=x),
                y = Inf, hjust = 1, vjust = 3, show.legend = F) + 
...
ggsave("RealspPredictsMedian_DBHcat_delog_per_sp.pdf",
       path = "D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Plots",
       plots, width = 15, height = 10)
```
