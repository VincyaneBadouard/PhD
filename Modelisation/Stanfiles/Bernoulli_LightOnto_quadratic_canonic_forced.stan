...
model {
  // Presence ~ bernoulli_logit(alpha + beta1*Environment + beta2*Environment.*Environment); // developped Likelihood
    Presence ~ bernoulli_logit(a * (Environment - (O*DBH))^2 + gamma); // canonic Likelihood
    // a * (Environment - (O + iota*DBH))^2 + gamma
...
generated quantities {
  ...
  p = inv_logit(a * (Environmentp - (O*DBHp))^2 + gamma) ; // predictions
  
  // For model evaluation with loo;
  vector[N] log_lik; // factors of the log-likelihood as a vector
  for (n in 1:N) {
    log_lik[n] = bernoulli_logit_lpmf(Presence[n] | a * (Environment[n] - (O*DBH[n]))^2 + gamma);
