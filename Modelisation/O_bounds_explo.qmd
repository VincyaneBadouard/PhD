...
tidyr::crossing(
  a = c(-0.001, -0.02, -0.05, -0.1, -0.25, -1, -5, -10, -1000000000000000), 
  O = -6, #c(-6, -3, -1),
  gamma = c(-3, -1, -0.5, 1, 2, 3, 4, 5, 7, 10, 15, 400, 480, 490, 495, 10000000000000000),
  Environment = (seq(-6,0, length.out =50)) 
) %>% 
  mutate(P = 1 / (1 + exp(-(a * (Environment - O)^2 + gamma)))) %>% 
  # unite(a, O, gamma, col = "Combinaison", sep = " ", remove = F) %>% 
...
- gamma dot etre > -3 pour pourvoir observer les courbes.  
