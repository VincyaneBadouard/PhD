---
title: "Canonical_forced_all_niche_forms"
date: "`r Sys.Date()`"
format: html
self-contained: true
theme: cosmo
...
**Objectives:**
Check if the canonical form of the quadratic equation permit to fit different form of light species niche (concave, convex, positive or negative sigmoid, flat), when the model force a concave form of probabilities predictions.    
...
```{r, fit}
# convex <- sampling(M_Quadra_can, data = dataM[["convexbell"]], chains = 4, iter = 2000)

fits <- lapply(dataM, function(x) sampling(M_Quadra_can, data = x, chains = 4, iter = 2000))
names(fits) <- names(dataM)
```
-> **always problems of divergent transitions** after warmup.   
-> and of **largest R-hat, indicating chains have not mixed**.  
...
```
-> There is divergence in the noeffect and convex cases  
...
-> noeffect: good  
-> concave: good  
-> convex: initial *a* (>0) can't be retreived because *a* is forced to be <0 to form a concave.  
-> sigmoid: *a* medians are underestimated but well <0 as asked by the model.   
...
```
-> very bad except for concave form   
...
-> The distributions forms are well predicted, excpect the convexbell  
-> The median extremum is interpretable and well positioned, except for the convexbell and the noeffect  
