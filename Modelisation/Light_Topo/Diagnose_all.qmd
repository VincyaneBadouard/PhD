...
datap %>% 
  filter(parameter=='O' & Sg_effect) %>% 
  # filter(!Species %in% c('Talisia_sylvatica', 'Votomita_guianensis')) %>% # because misidentifications so wrong Q95
  ggplot(aes(x=MaxTreeHeight, y=m, col = Strategy)) +
  theme_minimal() + 
  scale_y_continuous("O median (transmittance %)", n.breaks = 10, labels = ~round(exp(.)*100,2)) + # delog
  ggrepel::geom_text_repel(aes(label = Species), col = "black", size = 2.5) +
  geom_point() +
  ...
  scale_color_manual(name= "Initial classification",
                     values = c("Canopy species" = "#E7B800",
                                "Understory species"= "#009E73",
                                "NA" = "darkgrey")) +
  labs(x= "Species maximum tree height (m)") +
  theme(legend.position="bottom") +
...
ggsave("Light_topo_MaxTreeHeight_checkstrategies.png", path = PATH,
