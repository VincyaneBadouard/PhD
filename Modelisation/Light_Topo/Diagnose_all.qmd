...
for(P in c('O', 'tau', 'a')){
  ...
  print(
    datap %>% 
      ...
      {if(P=='a') geom_vline(xintercept=-0.02, linetype = "dashed", col = "red") } + 
      ...
      {if(P=='tau') list(
        ...
        annotation_custom(textGrob("Less flood\ntolerant"),xmin= -7.5,xmax=-7.5,ymin=-6,ymax=-6)
      ) }
  )
  
  ggsave(paste(P,"monoplot.png", sep='_'), path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
...
ggsave("Light_topo_biplot_strategy.png", path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
       width = 32, height = 20, units = "cm", dpi=800, bg="white")
...
ggplot(databp) +
  ...
  scale_y_continuous(n.breaks = 10, labels = ~round(exp(.)*100,2)) + # delog
  scale_x_continuous(n.breaks = 10, labels = ~round(log(-.),2)) + # log
  # scale_y_continuous(trans="exp") +
  theme(axis.title = ggtext::element_markdown(),
        legend.title = ggtext::element_markdown(),
...
ggplot(databp) +
  ...
  geom_point(aes(x= a_median, y = O_median, col = Strategy, shape = Sg_light_effect)) + # median
  scale_shape_manual(name="Light effect",
                     values=c("FALSE" = 1, "TRUE" = 19),
                     breaks = c("TRUE","FALSE")) +
...
ggsave("O_a_biplot_strategy.png", path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
       width = 28, height = 19, units = "cm", dpi=800, bg="white")
...
ggsave("Boxplots_parameters_strategy.png", path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
