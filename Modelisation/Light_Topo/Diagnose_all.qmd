...
datap <- datap %>%
  ...
  mutate(Species_code = sub("^(.{3}).*_(.{3}).*$", "\\1_\\2", ScientificName))
# -a
# datap <- datap %>%
...
datap %>% 
  ...
  theme_minimal() + 
  scale_y_continuous("O median (transmittance %)", n.breaks = 10, labels = ~round(exp(.)*100,2)) + # delog
  ggrepel::geom_text_repel(aes(label = Species), col = "#999999", size = 2.3) +
...
datap <- datap %>% 
  mutate(Guild = case_when(
    TreeHeightQ95 <25 ~ "Understorey",
    TreeHeightQ95 >40  ~ "Upper-canopy",
    .default = "Mid-canopy")
  ) %>% 
...
datap %>% 
  mutate(Incertitude = hh-ll) %>% 
  left_join(abund %>% select(ScientificName,N), by = 'ScientificName') %>% 
...
# 84% sp with tau significantly (80% CI) different of 0 (9ha, coord cor, 2019 light)
...
cor(databp$tau_median, databp$O_median, method = "pearson") #  -0.15 -> -0.09 (9ha, coord cor, 2019 light)
...
# Check topo
unique((validation %>% filter(Chap2_TWI=="negatif" & !is.na(Topography)))$Topography) # all experts opinion
unique((validation %>% filter(Chap2_TWI=="positif" & !is.na(Topography)))$Topography) # all experts opinion
...
validation %>% filter(Validate_topo) %>% select(Species, N,Topography, Chap2_TWI) # Validate_topo: T # 20/28
validation %>% filter(!Validate_topo) %>% select(Species, N,Topography, Chap2_TWI) # Validate_topo: F # 4/28
validation %>% filter(is.na(Validate_topo) & !is.na(Topography)) %>%
  filter(Chap2_TWI=="no significant effect"|Topography=='no associated') %>%
  select(Species, N, Topography, Chap2_TWI) # Validate_topo: no effect # 4/28
...
databp <- datap %>% 
  ...
  mutate(Incertitude = log(-a_Q80) - log(-a_Q5)) %>% # 50% CI : O_Q50_1-O_Q50_0 ; 80% CI : O_Q80-O_Q5
    # tau significantly different of 0
  mutate(Tau_effect = ifelse(tau_Q50_0<0 & 0<tau_Q50_1, F, T))
...
ggplot(databp) +
  ...
  geom_vline(xintercept = 0, col="red", linetype = "dashed") +
  geom_hline(yintercept=log(0.02), linetype = "dashed", col = "red") +
  
  geom_point(aes(x= tau_median, y = log(-a_median),
                 col= O_median, size=Incertitude, shape= Tau_effect)) + 
  scale_shape_manual(name="*tau* significiant effect",
                     values=c("FALSE" = 1, "TRUE" = 19),
                     breaks = c("TRUE","FALSE")) +
  ...
  viridis::scale_color_viridis(begin =0.45, n.breaks = 5, labels = ~round(exp(.)*100,2)) +
  scale_size_continuous(range = c(0.5,3), trans = 'reverse') +
  
  ggrepel::geom_text_repel(aes(x= tau_median, y = log(-a_median), label = Species_code), col = "black", size = 3) +
  ...
  annotation_custom(textGrob("Drier", gp=gpar(col="#999999")),
                    xmin= -3,xmax=-3,ymin=-0.32,ymax=-0.32) +
  annotation_custom(textGrob("Wetter", gp=gpar(col="#999999")),
                    xmin= 2,xmax=2,ymin=-0.32,ymax=-0.32) + 
  
  annotation_custom(textGrob("Flat", gp=gpar(col="#999999")),ymin= 0,ymax=0,xmin=-3.85,xmax=-3.85) + 
  annotation_custom(textGrob("Wide", gp=gpar(col="#999999")),ymin= -.03,ymax=-.03,xmin=-3.85,xmax=-3.85) + 
  annotation_custom(textGrob("Narrow", gp=gpar(col="#999999")),ymin= -.3,ymax=-.3,xmin=-3.85,xmax=-3.85) +
...
ggplot(databp) +
  ...
  geom_point(aes(x= log(a_median), y = O_median, col = Guild, shape = Sg_effect)) + # median
  scale_shape_manual(name="Light effect",
                     values=c("FALSE" = 1, "TRUE" = 19),
                     breaks = c("TRUE","FALSE")) +
