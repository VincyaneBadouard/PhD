---
title: "DBH_distrib_Paracou_vs_ALT"
date: "`r Sys.Date()`"
format: html
self-contained: true
theme: cosmo
...
# Prepare data
plots 1, 6, 11, 13, 14, 15, 16 (not human impacted plots)) in 2020
```{r Paracou}
...
unique((Paracou %>% filter(CensusYear==2020))$Plot) # Only control & biodiversity plots (1, 6, 11, 13, 14, 15, 16)


Paracou <- Paracou %>% 
  select(Forest, SubPlot, Plot, CensusYear, idTree, Genus, Species, CircCorr, Circ) %>% 
  filter(CensusYear==2020) %>% 
  filter(!SubPlot %in% c(13,14,15,18,19,20,23,24,25)) %>% # not in ALT
...
(nrow(data[data$Bigdiff,])/nrow(data))*100 # 12/24 (22%) species with more than 5 cm dbh diff
```
