...
for(P in c('O', 'tau', 'a')){
  ...
  print(
    datap %>% 
      ...
      {if(P=='O') list(
        annotation_custom(textGrob("Heliophilous", gp = gpar(fontsize=10)),
...
datap %>% 
  ...
  theme_minimal() +
  geom_smooth(method='lm', col = 'darkgrey', alpha = 0.2) +
  # geom_smooth(col = 'darkgrey', span = 0.25, se=F) +
  geom_point() +
  # geom_line() +
...
ggsave("Incertitude_vs_sp_abundance_lm.png", path = PATH,
...
#                     light = rep(c("Sciaphile", "Heliophilous"), times =3))
...
estimate <- databp %>% 
  ...
  mutate(Chap2_Light = case_when(
    ...
    Estimate_light_optimum >= 14 & Sg_light_effect ~ "Heliophilous", # >=14%
...
validation <- left_join(Experts, estimate, by= 'Species') %>% 
  ...
  mutate(Validate_light = case_when(
    Temperament == "Hard struggler" & Chap2_Light=="Heliophilous" ~ F,
    Temperament == "Hard gambler" & Chap2_Light=="Sciaphile" ~ F,
    # Hard gambler	generalist
    
    grepl("Hard gambler", Temperament) & Chap2_Light=="Heliophilous" ~ T,
...
#                     light = c("Sciaphile", "Heliophilous"))
```
...
databp <- datap %>% 
  ...
  # mutate(Incertitude = O_Q80-O_Q5) # 50% CI : O_Q50_1-O_Q50_0 ; 80% CI : O_Q80-O_Q5
...
ggplot(databp, aes(x=Incertitude, y= a_median)) +
  ...
  labs(x= expression(italic(O)~"80% incertitude (transmittance %)"), y= expression(italic(a)~median))
# theme(axis.title = ggtext::element_markdown())

ggsave("O_incertitude_a_lm.png", path = PATH, # _xaxislogged
