...
# datalist <- split(data, data$ScientificName) # 34 ; create a list of data.frames,1 per ScientificName
# names(datalist)
...
```{r}
# load("D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Fits/Realsp_light_1-4.Rdata")
```
...
save(fits, file = "D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Fits/Realsp_light_1-4.Rdata") # need a absolute path after cluster
```
...
```{r}
lapply(as.list(names(dataM)), function(species)
...
ggsave("RealspPredictsMedian_without_opt_quantiles.png", path = "D:/Mes Donnees/PhD/Figures/Modelisation", width = 35, height = 15, units = "cm", dpi=800, bg="white")
```
