...
# for 1 dataset
fit <- stan(file= "Bernoulli_EnvOnly_quadratic.stan", data = dataM[["noeffect"]], chains = 4, iter = 2000) # 0.511 seconds (Total)
fit
```
...
mcmc_pairs(as.array(fits[["noeffect"]]), pars = c("alpha", "beta1", "beta2", "lp__", "o"))
...
mcmc_areas(as.array(fits[["noeffect"]]), prob=0.95, pars = c("alpha", "beta1", "beta2"))
mcmc_areas(as.array(fits[["noeffect"]]), prob=0.95, pars = "o")
...
```{r, eval=F}
# Simulations mean
cbind(mu = apply(as.matrix(fits[["noeffect"]], pars = "p"), 2, mean),# 2 for columns;  mean of all sim
      t(apply(as.matrix(fits[["noeffect"]], pars = "p"), 2,
              quantile, probs = c(0.05, 0.95))),
      Opt = apply(as.matrix(fits[["noeffect"]], pars = "o"), 2, mean)) %>% # optimum
  cbind(data[["noeffect"]]) %>% 
...
mat <- as.data.frame(fits[["noeffect"]], pars = c("p", "lp__")) %>% filter(`lp__`== max(`lp__`)) %>% select(-`lp__`)
op <- as.data.frame(fits[["noeffect"]], pars = c("o", "lp__")) %>% filter(`lp__`== max(`lp__`)) %>% select(-`lp__`)
