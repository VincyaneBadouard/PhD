...
# Parameters
opt <- 0.7
a <- -2 # logit(0.5)= 0) autant de 0 que de 1
b <-15 # à 0 pour pas d'effet
c <- -11 # -b/(opt*2) # à 0 pour une linéaire
...
X <- seq(0, 1, length.out = n) # env var
# Parameters
a <- 0 # logit(0.5)= 0) autant de 0 que de 1
b <- 0 # à 0 pour pas d'effet
...
# fit <- stan(file= "Bernoulli_EnvOnly_quadratic.stan", data = dataM, chains = 4, iter = 2000) # 0.511 seconds (Total)
...
```{r, eval=F}
# Simulations mean
cbind(mu = apply(as.matrix(fit, pars = "p"), 2, mean),# 2 for columns;  mean of all sim
...
mat <- as.data.frame(fit, pars = c("p", "lp__")) %>% filter(`lp__`== max(`lp__`)) %>% select(-`lp__`)
