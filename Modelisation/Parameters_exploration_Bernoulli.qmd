...
La distribution de la variable réponse : présence-absence (discret) \[0;1\] → *Bernoulli(p)* La fonction de prédiction ou prédicteur linéaire : α + 𝜏*Topo + λ*Light La fonction de lien : de \[0;1\] à R → *logit*

Proba de succès : p ∊ \[0,1\] q = 1- p Esperence = p Variance = p\*q

Presence \~ *Bernoulli(p)*

logit(p) = α + 𝜏*Topo + λ*Light Et donc p = logit-1(α + 𝜏*Topo + λ*Light)

Environmental var : Light : continuous, \[0,1\]

```{r, echo=F}
library(tidyverse)
```

# Logit de pleins de valeurs plot

le logit transforme les valeurss sur \[0;1\] en valeurs sur R

```{r}
X <- seq(0,1, by=0.01)
plot(X, rstanarm::logit(X))

X <- seq(0,1, by=0.001)
plot(X, rstanarm::logit(X))
```

# Inverse logit de pleins de valeurs plot

l'inverse logit transforme les valeurs sur R en valeurs sur \[0;1\]
...
```{r}
# null model
null <- function(x, a= 1) a # pas d'effet de l'env, présente partout avec la meme proba
...
data.frame(x = seq(0, 1, length.out = 100)) %>% # x explicative var
  mutate(y = inv_logit(lin(x, 1, 1))) %>% # succès proba
  ggplot(aes(x, y)) +
  geom_line() +
  theme_bw()
```
...
inv_logit(17) # 1
gtools::logit(1)
b1 = 2 # faible effet de la lumière
b1 = 10 # for effet de la lumière
b2 = 2
```
