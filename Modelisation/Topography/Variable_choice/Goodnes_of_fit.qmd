...
sp <- (read.csv("D:/Mes Donnees/PhD/Inventories/Data/Understory/Paracou/InterestSpecies.csv")[,3]) # 75; 2 to have treedatacor


# sp <- c("Licania_membranacea", "Symphonia_sp.1", "Piparea_multiflora", "Iryanthera_hostmannii", "Tabernaemontana_macrocalyx") # 4 forms
# sp <- "Symphonia_sp.1"

Var <- c("logTWI", "logHAND")
```
...
BIC_df <- BIC_df %>% 
  ...
  rowwise() %>% 
  mutate(Diff = BIC_logHAND - BIC_logTWI) %>% # BIC difference
  mutate(test = ifelse(BIC_logTWI <= BIC_logHAND,T,F)) %>% # without abs(Diff)>2
  mutate(HAND = ifelse(BIC_logHAND < BIC_logTWI & abs(Diff)>2,T,F)) %>% 
  mutate(TWI = ifelse(BIC_logTWI < BIC_logHAND & abs(Diff)>2,T,F))

nrow(BIC_df %>% filter(test==T)) # 43 (-> 45 with logHAND)
nrow(BIC_df %>% filter(HAND==T)) # 17 (-> 21 with logHAND)
nrow(BIC_df %>% filter(TWI==T)) # 33 (-> 28 with logHAND) -> best BIC is for TWI
nrow(BIC_df %>% filter(HAND==F,TWI==F)) # 25 (-> 26 with logHAND)

```

```{r}
write.csv(BIC_df, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_affine_logHAND_vs_logTWI.csv")
# BIC_df <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_affine_logHAND_vs_logTWI.csv")
```
...
shape <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_logTWI_affine_vs_quadratic.csv")


g1 <- ggplot(BIC_df, aes(Diff)) +
  ...
  scale_x_continuous(trans = "S_sqrt", 
                     ...
                     name = "BIC difference (log(HAND) - log(TWI))") +
  # n species with bic diff < -2 (HAND pref)
  annotate("text", x = -13, y = 0.01, col="#D55E00",
           label = paste0(sum(BIC_df$Diff < -2), "/", nrow(BIC_df))) +
  # n species with bic diff >2 (TWI pref)
  annotate("text", x = 13, y = 0.01, col="#0072B2",
           label = paste0(sum(BIC_df$Diff > 2), "/", nrow(BIC_df))) +
  ylab("Density") +
  theme(axis.title.y = element_text(vjust = -1))

g2 <- ggplot(shape, aes(Diff)) +
  # xlim(NA, 5) +
  geom_density(fill = "lightgrey", col = NA) +
  theme_minimal() +
  geom_vline(xintercept = 0) +
  ...
  scale_x_continuous(trans = "S_sqrt", 
                     breaks = c(-10, -2, 0, 2, 7),
                     name = "BIC difference (monotonous - quadratic)") +
  # n species with bic diff < -2 (affine pref)
  annotate("text", x = -6.5, y = 0.1, col="#009E73",
           label = paste0(sum(shape$Diff < -2), "/", nrow(shape))) +
  # n species with bic diff >2 (quadratic pref)
  annotate("text", x = 4, y = 0.1, col="#FC4E07",
           label = paste0(sum(shape$Diff > 2), "/", nrow(shape))) +
  # ylab("Density") +
    theme(axis.title.y = element_blank())

# ggsave("BIC_logHAND_vs_logTWI.png", path = "D:/Mes Donnees/PhD/Figures/Modelisation/",
...
library(ggpubr)
var <- BIC_df %>% 
  select(Species, BIC_logHAND, BIC_logTWI) %>% 
...
g3 <- ggpaired(var, x = "Variable", y = "BIC", # boxplots liés par des lignes
  ...
  guides(color = FALSE) +
  ylab("BIC") +
  theme(axis.title.x = element_blank(), axis.title.y = element_text(vjust = -1)) +
  scale_x_discrete(labels = c("log(HAND)", "log(TWI)"))


g4 <- ggpaired(shape, x = "Variable", y = "BIC",
               color = "Variable", line.color = "gray", line.size = 0.4,
               palette = "jco") +
  ...
  scale_x_discrete(labels = c("Monotonous", "Quadratic"))

(g1 + g2)/
(g3 + g4) + plot_layout(heights  = c(1, 2))

ggsave("BIC_topo_form_variable_log_4plots.png", path = "D:/Mes Donnees/PhD/Figures/Modelisation/",
