...
nrow(BIC_df %>% filter(test==T)) # 43
nrow(BIC_df %>% filter(HAND==T)) # 17
nrow(BIC_df %>% filter(TWI==T)) # 33 -> best BIC is for TWI
nrow(BIC_df %>% filter(HAND==F,TWI==F)) # 25

```

```{r}
write.csv(BIC_df, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_affine_HAND_vs_logTWI.csv")
# BIC_df <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_affine_HAND_vs_logTWI.csv")
```
...
g1 <- ggplot(BIC_df, aes(Diff)) +
  ...
  scale_x_continuous(trans = "S_sqrt", 
                     ...
                     name = "BIC difference (HAND - log(TWI))") +
...
# ggsave("BIC_HAND_vs_logTWI.png", path = "D:/Mes Donnees/PhD/Figures/Modelisation/",
...
g3 <- ggpaired(var, x = "Variable", y = "BIC", # boxplots liés par des lignes
  ...
  scale_x_discrete(labels = c("HAND", "log(TWI)"))
...
ggsave("BIC_topo_form_variable_logTWI_4plots.png", path = "D:/Mes Donnees/PhD/Figures/Modelisation/",
