...
for(S in sp){
  for(V in Var){
    ...
    L <- max(fit[[S]][[V]]$draws("lp__"))# the maximized value of the likelihood function of the model (fit[[f]]$summary("lp__"))
...
for(s in sp){
  plist[[s]] <- local({
    ...
    g <- lapply(names(fit[[s]]), function(v){
      # v = "TWI" ; s= "Talisia_megaphylla"
      
      if(v=="TWI") xtit <- "log(TWI)"
      if(v=="HAND") xtit <- "HAND (m)"
      
      
      DATA <- fit[[s]][[v]]$summary("p") # p posterior
      # prediction environment 
      if(v=="HAND") DATA$x <- seq(-0.37, 23.34, length.out = 50)
      if(v=="TWI") DATA$x <- seq(1.358045, 2.400176, length.out = 50)
    ...
    cowplot::plot_grid(plotlist = g, labels=c("log(TWI)", "HAND"), vjust = 3, hjust=-1, label_size=12)
...
ggsave("Allsp_HAND_vs_logTWI_niche.pdf",
