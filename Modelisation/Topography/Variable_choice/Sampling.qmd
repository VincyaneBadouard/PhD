...
datalist <- lapply(names(datalist), function(name) {
  datalist[[name]] <- datalist[[name]] %>% filter(!is.na(logHAND))
...
dataM_HAND <- lapply(datalist, function(x) list(N = nrow(x), # 47 850
                                                Presence = x$Presence,
                                                Environment = x$logHAND,
                                                # number of predictions
                                                Np = Np,
                                                # environment of predictions
                                                Environmentp = seq(min(x$logHAND), max(x$logHAND), length.out = Np))
)

# seq(min(datalist[[1]]$logTWI), max(datalist[[1]]$logTWI), length.out = 50) # 1.358045 - 2.400176
# seq(min(datalist[[1]]$logHAND), max(datalist[[1]]$logHAND), length.out = Np) # -0.37 - 23.34
...
foreach::foreach(
  ...
  chain_path <- file.path("Chains", model_name, "logTWI", sp[i])
...
foreach::foreach(
  ...
  chain_path <- file.path("Chains", model_name, "logHAND", sp[i])
  if(!file.exists(chain_path)){
    # unlink(chain_path, recursive = TRUE)
    dir.create(chain_path)
