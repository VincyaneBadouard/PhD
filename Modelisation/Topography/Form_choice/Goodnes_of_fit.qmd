...
nrow(BIC_df %>% filter(test==T)) # 61 
nrow(BIC_df %>% filter(Affine==T)) # 56 -> species majority prefer affine model
nrow(BIC_df %>% filter(Quadratic==T)) # 9 sp prefer quadratic model
nrow(BIC_df %>% filter(Affine==F,Quadratic==F)) # 10 with no significant diff

```

```{r}
write.csv(BIC_df, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_MNT_affine_vs_quadratic.csv")
# BIC_df <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/GoodnessOfFit/BIC/BIC_MNT_affine_vs_quadratic.csv")
```
...
ggplot(BIC_df, aes(Diff)) +
  ...
  annotate("text", x = -10, y = 0.015, col="#009E73",
           label = paste0(sum(BIC_df$Diff < -2), "/", nrow(BIC_df))) +
  # n species with bic diff >2 (quadratic pref)
  annotate("text", x = 10, y = 0.015, col="#FC4E07",
           label = paste0(sum(BIC_df$Diff > 2), "/", nrow(BIC_df))) +
  ylab("Density")

ggsave("BIC_affine_vs_quadratic.png", path = "D:/Mes Donnees/PhD/Figures/Modelisation/",
