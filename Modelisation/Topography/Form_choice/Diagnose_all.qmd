...
for(S in sp){
  for(M in model_name){
    ...
    fits[[S]][[M]] <- as_cmdstan_fit(list.files(chain_path, full.names = TRUE))
  }
}
...
for(s in sp){
  plist[[s]] <- local({
    ...
    g <- lapply(names(fits[[s]]), function(m){
      # m = "Affine"
      
      fits[[s]][[m]]$summary("p") %>% # p posterior
        mutate(x = seq(5.7325, 23.61375, length.out = 100)) %>% # prediction environment
        ggplot(aes(x = x)) +
        geom_point(aes(y = median), size=0.7) + # p posterior median
        geom_ribbon(aes(ymin = q5, ymax = q95), color = 'red', alpha = 0.2) + # P quantiles
        ...
        labs(title = s, x= "Elevation (m)", y = "Presence probability") + #paste(s, "-", f)
        theme_minimal()
      # theme(axis.title = element_blank())
    })
