...
```{r Sp number, eval=F}
(nrow(Inv %>% filter(ScientificName %in% sp))/nrow(Inv))*100 # the 75 sp corrsponds to 53% of the total stems number 
...
(nrow(allsp)/nrow(Inv))*100 # 87% stems botanicaly identified

allsp %>% 
  select(ScientificName) %>%
  unique()
# 556 sans les sp et sp numéro
...
all(sp %in% newsp$ScientificName) # all the 75 sp have >30 ind on 9ha

abun <- newsp %>% 
  filter(ScientificName %in% sp) %>% 
  arrange(N)
range(abun$N) # 30-2410
```
