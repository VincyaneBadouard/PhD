...
for(N in c(10, 30, 100, 300, 700, 1000, 3000, 7000, 10000, nrow(Inv))){ # , nrow(Inv)
  data[[as.character(N)]] <- data.frame(
    N= N,
    Transmittance = sample(Inv$Transmittance, N, replace=F),
...
ggsave("Light_draws_boxplot.png",
       ...
       width = 15, height = 10)
```
We can see that 100 observations are sufficient to represent the topographic and light environment of the community.
...
b <- 4 
c <- -4#-b/(opt*2)# ; pour une linéaire : inv_logit(-9) ~ 0
...
simulatedata_PA <- function(Abundance, # n presence
                            Absence, # n absence
  ...
  # print(temperament)
  # print(paste("Abundance=",Abundance)); print((paste("Absence=",Absence)))
  ...
  # Intercept at he median of light and for a proba of (N1)/(N1+N2)
  ...
  # Np <- c("rare", "abundant")
  # a = rstanarm::logit((N1)/(N1+N2)) # intercept
  if(Abundance == "rare") {
    if(temperament == "concavebell"){
      a = rstanarm::logit(12/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "possigmoid"){
      a = rstanarm::logit(10/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "convexbell"){
      a = rstanarm::logit(0.0005/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "negsigmoid"){
      a = rstanarm::logit(0.08/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "noeffect"){
      a = rstanarm::logit(30/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
  } # end rare
  if(Abundance == "abundant") {
    if(temperament == "concavebell"){
      a = rstanarm::logit(250/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "possigmoid"){
      a = rstanarm::logit(200/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "convexbell"){
      a = rstanarm::logit(5/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "negsigmoid"){
      a = rstanarm::logit(20/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
    if(temperament == "noeffect"){
      a = rstanarm::logit(600/nrow(Inv)) -
        (b*median(Inv$logTransmittance)) - (c*median(Inv$logTransmittance)^2)
    }
  } # end abundant
  ...
  # print(paste("N1=",N1)); print((paste("Absence=",Absence))) ; print((paste("include=",include)))
  ...
  test <- Inv %>%
    ...
    mutate(TotPresence = rbinom(1, size = 1, # Bernoulli = 1 trial
                                prob = TotProbability)) %>%
    ungroup() %>%
    mutate(SelectPres = if_else(TotPresence==1, 1, NA)) # all the presences
  
  # Na <- c("*1", "max/2", "max")
  N1 <- nrow(test[!is.na(test$SelectPres),])
  if(Absence == "*1") {N2= N1} 
  if(Absence == "max/2" & include) {N2= nrow(Inv)/2}
  if(Absence == "max/2" & !include) {N2= (nrow(Inv)/2)-N1}
  if(Absence == "max" & include) {N2= nrow(Inv)}
  if(Absence == "max" & !include) {N2= nrow(Inv)-N1}
...
for(x in 1:length(plist)){
  if(unique(plist[[x]]$Abundance)=="rare"){
    print(paste(unique(plist[[x]]$temperament),
                unique(plist[[x]]$Abundance), 
                unique(plist[[x]]$NSelectPres),
                unique(plist[[x]]$NSelectPres) <= 60 &
                  unique(plist[[x]]$NSelectPres) >= 20))
  }
  if(unique(plist[[x]]$Abundance)=="abundant"){
    print(paste(unique(plist[[x]]$temperament),
                unique(plist[[x]]$Abundance), 
                unique(plist[[x]]$NSelectPres),
                unique(plist[[x]]$NSelectPres) >= 500 & 
                  unique(plist[[x]]$NSelectPres) <= 1000))
  }
}
```
...
names(plist) <- lapply(1:length(plist), \(x) paste(
  unique(plist[[x]]$temperament),
  unique(plist[[x]]$Abundance), unique(plist[[x]]$Absence),
  # unique(plist[[x]]$NSelectPres), unique(plist[[x]]$NSelectAbs),
  unique(plist[[x]]$IncludeSp), sep = "-")
)
names(plist)
plist[[1]]
...
```{r}
plist <- readRDS("~/PhD/R_codes/PhD/Modelisation/Simdata/simulatedata_PA.rds")
```
...
# rstanarm::logit(0.5) # 0
# fits[1]
```

```{r, eval=F}
rm(a, include, includeSP, N1, N2, Na, Np, temperament, temperaments, simulatedata_PA, x, combinaisons)
```
...
# na.omit pour stan
plist <- lapply(names(plist), function(name) {
  plist[[name]] <- na.omit(plist[[name]])
  plist[[name]]  # Return modified data frame
})
names(plist) <- scenar
# View(plist[[1]])
rm(scenar, combinaisons)
```
...
dataM <- lapply(plist, function(x) list(N = nrow(x), #les noms des var doivent etre les memes que dans le fichier stan
                                        Presence = x$Presence,
                                        Environment = x$logTransmittance)) # doit etre entier pour n'importe quel run!

# table(dataM[[1]]$Presence)


Model <- stan_model("D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Stanfiles/Bernoulli_EnvOnly_quadratic.stan") # remove rds file if doest work
...
for(D in split(vec, ceiling(seq_along(vec)/12))) {
  
  # D = split(vec, ceiling(seq_along(vec)/12))
  ...
  fits <- foreach::foreach(
    ...
    sampling(Model,
         data = dataM[[i]], chains = 4, iter = 2000)
  }
  ...
  saveRDS(fits, file = paste("D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Fits/PA/Pseudo_abs_tests_",min(D),"_",j,".rds", sep=""))
...
```{r, eval=F}
rm(cl, fits, Model, opts, pb, cores, D, i,j,vec,progress) ; gc()

files <- list.files("D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Fits/PA", pattern = "Pseudo_abs_tests_", full.names = T)# 10 files

# files <- files[grepl('[[:digit:]]', files)]
# files <- files[grepl('.rds', files)]
files <- files[order(c(2,3,4,5,1))]#[1:5]
...
for (i in files) {
  print(i)
  allfits[[i]] <- (readRDS(i)) #add files to list position
}
names(allfits)
my_list_new <- unlist(allfits, recursive = FALSE)
names(my_list_new)
names(my_list_new) <- gsub(".*rds.", "", names(my_list_new))
names(my_list_new)==names(plist)
rm(allfits, dataM, plist, files, i);gc()

saveRDS(my_list_new, file ="D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Fits/PA/Pseudo_abs_tests_all.rds")
gc()

# readRDS("D:/Mes Donnees/PhD/R_codes/PhD/Modelisation/Fits/Pseudo_abs_tests.Rdata")

```

# Evaluation - Goodness of fit (not did)
