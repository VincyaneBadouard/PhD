...
Optdata <- data.frame(DBH = c(1, 2.5, 7.5, 15, 25, 35),
  ...
  mutate(OQ5 = OinitQ5 + iota*logDBH) %>% 
  mutate(OQ95 = OinitQ95 + iota*logDBH)
...
ggplot(Optdata, aes(x=logDBH, y= (O))) +
  ...
  geom_text(data = Optdata, aes(label = paste("iota median =", round(iota,2))),
            x = -Inf, y = Inf, vjust= 1.8, hjust = -0.1) +
