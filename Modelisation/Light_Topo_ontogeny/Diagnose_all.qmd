...
length(unique(badRhat$Species)) # 11/37
```
...
lapply(name , function(x)
  fits[[x]]$draws(
    c("lp__", "alpha", "beta1", "beta2_p", "O_1", "iota")
...
(3/40)*100 # 7.5% n'ont fait que 3 chaines au lieu de 4
(4/37)*100 # 10% mixent mal, 10% ont une chaine qui dépasse de la chenille
(11/40)*100 # 28% au total des sp n'ont pas supporté le modèle
```
...
datap <- lapply(names(fits), function(x)
  mcmc_intervals_data(fits[[x]]$draws(c("a", "O_1", "gamma", "tau", "iota")),
...
ggsave("Light_topo_onto_posteriors_iotabounds.pdf", path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
