...
PATH <- "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots/Light_topo_onto/1-3/Autocor/"
```
...
databp <- datap %>% 
  ...
  mutate(Incertitude = O_Q50_1-O_Q50_0) # 50% CI
...
for(s in names(fits)){
  plist[[s]] <- local({
    ...
    databp %>% 
      ...
      # ggrepel::geom_text_repel(aes(x= a_median, y = O_median, label = Species), col = "black", size = 2.5) +
      scale_y_continuous(limits = range(databp$O_median), n.breaks = 10, labels = ~round(exp(.)*100,2)) + # delog
      scale_x_continuous(limits = range(databp$a_median), trans="log") +
      theme(axis.title = ggtext::element_markdown(),
            legend.title = ggtext::element_markdown()) +
      labs(x= "Niche width (log(-*a*))",
           y= "Light optimum (*O*) (% transmittance)", col='*O* incertitude', shape='DBH (cm)') +
      ggtitle(s)
  })
}
...
ggsave("O_a_biplot_onto_autocor_perspecies.pdf",
       path = PATH,
       plots, width = 15, height = 10)
...
# ggsave("O_a_biplot_onto_autocor_perspecies.png", path = PATH, 
#        width = 32, height = 20, units = "cm", dpi=800, bg="white")
