...
for(S in c('Understory species', 'Canopy species')){
  ...
  for(P in c('a', 'O', 'tau')){
    ...
    ggsave(paste(S,P,"accross_onto.png", sep='_'), path = PATH,
...
for(P in c('a', 'O', 'tau')){
  print(
    datap %>% 
      filter(parameter==P) %>% 
      # filter(Strategy==S) %>% 
      ggplot(aes(x= DBH, y = m, col=Strategy)) +
      theme_minimal() + 
      geom_boxplot() +
  ...
  ggsave(paste(P,"_boxplot_accross_onto_strategy.png", sep='_'), path = PATH,
         width = 10, height = 10, units = "cm", dpi=800, bg="white")
}
# }
```
...
databp <- datap %>% 
  ...
  mutate(Incertitude =(exp(O_Q50_1)*100)-(exp(O_Q50_0)*100)) # 50% CI : O_Q50_1-O_Q50_0 ; 95% CI : O_Q95-O_Q5
```
...
databp %>% 
  ...
  # ggrepel::geom_text_repel(aes(x= a_median, y = O_median, label = Species),
  #                          col = "black", size = 2.5) +
...
for(s in names(fits)){
  plist[[s]] <- local({
    ...
    databp %>% 
      ...
      # 95% credible interval
      # geom_segment(aes(x=a_Q5, xend=a_Q95, y=O_median, yend=O_median), size = .5, col= "grey") + # tau
      geom_segment(aes(y=O_Q5, yend=O_Q95, x=a_median, xend=a_median), size = .5, col = "#E7B800") + # O
...
ggplot(databp, aes(x=Incertitude, y= a_median)) +
  ...
  labs(x='*O* 95% incertitude (transmittance %)', y= '*a* median') +
  theme(axis.title = ggtext::element_markdown(),
        legend.title = ggtext::element_markdown())
