...
for(P in c('O', 'a', 'tau')){
    ...
    plist[[p]] <- local({
      ...
      ggplot(datas, aes(x= DBH, y = m, group=1)) +
        ...
        scale_color_manual(name = "Credible interval", 
                           values = c("95%" = "#E7B800",
                                      "50%"= "#FC4E07")) + 
        {if(P!='tau') geom_point() } + # median
        {if(P=='tau') geom_point(aes(alpha = Sg_effect)) } +
        geom_line() +
  ...
  ggsave(paste(P,"_posteriors_stages.pdf", sep=''), path = # _sg
           "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
...
for(S in c('Understory species', 'Canopy species')){
  ...
  for(P in c('a', 'O', 'tau')){
    ...
    datap %>% 
      ...
      {if(P=='tau') list(geom_point(aes(col=Topo)), geom_line(aes(col=Topo)),
    ...
    ggsave(paste(S,P,"accross_onto.png", sep='_'), path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
