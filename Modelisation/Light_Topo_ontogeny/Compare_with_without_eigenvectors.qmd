...
Rhat_table <- lapply(names(fits), function(x)
  fits[[x]]$summary(c("alpha", "beta1", "beta2_p", "tau", "iota"), "rhat") %>% 
    mutate(Species = x) %>% 
    select(Species, variable, rhat) %>% 
    mutate(rhat_eig = fits_eig[[x]]$summary(c("alpha", "beta1", "beta2_p", "tau", "iota"), "rhat")$rhat)
) %>% 
  bind_rows()
...
lapply(unique(badRhat$Species) , function(x)
  fits[[x]]$draws(c("alpha", "beta1", "beta2_p", "tau", "iota")) %>% 
    mcmc_trace() +
    ggtitle(paste("Without eigenvectors - ",x))
)

lapply(unique(badRhat$Species) , function(x)
  fits_eig[[x]]$draws(c("alpha", "beta1", "beta2_p", "tau", "iota")) %>% 
    mcmc_trace() +
...
datap <- lapply(names(fits), function(x)
  mcmc_intervals_data(fits[[1]]$draws(c("a", "O", "gamma", "tau", "iota")),
                      prob = 0.5, prob_outer = 0.95, point_est = "median") %>% 
    mutate(Eigenvectors = F) %>% 
    bind_rows(
      mcmc_intervals_data(fits_eig[[1]]$draws(c("a", "O", "gamma", "tau", "iota")),
                          prob = 0.5, prob_outer = 0.95, point_est = "median") %>% 
        mutate(Eigenvectors = T)
    ) %>% 
    mutate(Species = x) %>% 
        mutate(Eigenvectors = factor(Eigenvectors, labels = c(T,F))) 
...
ggplot(datap, aes(y = Species, col = Eigenvectors)) +
  theme_minimal() + 
  ...
  scale_color_manual(values=c("#009E73","#FC4E07")) +
...
# ggsave("Light_topo_onto_posteriors.pdf", path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Modelisation/Plots",
