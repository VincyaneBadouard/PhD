...
estimate <- muche %>% 
  ...
  rename(Subcategories = Temperament) %>% 
  rename(Chap3_Light = Mega_categories) %>% 
  mutate(Species = str_replace(Species, "_", " "))
```
...
validation <- left_join(Experts, estimate, by= 'Species') %>% 
  ...
  mutate(Chap3_Validate_light = case_when(
    ...
    grepl("Heliophile <10 cm DBH", Temperament) & str_starts(Chap3_Light, "light") ~ T,
...
validation %>% filter(!Chap3_Validate_light) %>% select(Temperament, Chap3_Light) # Chap3_Validate_light: F # 4
```
...
validation_table <- validation %>%
  ...
  select(Species, Chap2_Estimated_light,
         Significant_Onto_effect, Light_opt_shift, Chap3_Estimated_light, Subcategories,
