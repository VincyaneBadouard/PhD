---
params:
  species: Tabernaemontana_macrocalyx
...
t <- fit$summary(c("a", "gamma", "O", "iota", "tau"), "median") %>% # parameters median
  ...
  mutate(data = list(dplyr::select(Residuals, y, Xutm, Yutm,
                                   logTransmittance, logTWI, logDBH))) %>% # covariables
  unnest(data) %>%  
  # Compute predictions
  mutate(p = arm::invlogit(a * (logTransmittance - (O + iota*logDBH))^2 + 
                             gamma + tau*logTWI)) %>%
  # Compute residuals for bernoulli as statmod::qres.binom()
...
arm::binnedplot(Residuals$p_hat ,Residuals$raw_e, nclass=NULL,
                xlab="Predictions", ylab="Average residual",
                main="Binned residual plot",
                cex.pts=0.8, col.pts=1, col.int="gray")

arm::binnedplot(Residuals$logTWI ,Residuals$raw_e, nclass=NULL,
                xlab="logTWI", ylab="Average residual",
                main="Binned residual plot",
                cex.pts=0.8, col.pts=1, col.int="gray")

arm::binnedplot(Residuals$logTransmittance ,Residuals$raw_e, nclass=NULL,
                xlab="logTransmittance", ylab="Average residual",
                main="Binned residual plot",
                cex.pts=0.8, col.pts=1, col.int="gray")

arm::binnedplot(Residuals$logDBH ,Residuals$raw_e, nclass=NULL,
                xlab="logDBH", ylab="Average residual",
                main="Binned residual plot",
                cex.pts=0.8, col.pts=1, col.int="gray")
...
# Compute the reliabilitydiagram
r <- reliabilitydiag(x = t(phat), y = Residuals$y)
...
```{r, eval=F}
trace(performance::binned_residuals, edit=T)
# sort pred_y
Residuals %>% 
  arrange(p_hat) %>% 
  mutate(myids = _n if pred_y < .,) %>% 
  mutate(nbins = floor(sqrt(length(p_hat)))) %>% 
  group_by(nbins) %>%
  mutate(binno = cut(myids) if pred_y < . , group(`nbins´) icodes) %>% 
  group_by(binno) %>% 
  mutate(avefit = mean(pred_y)) %>% 
  mutate(myaveres = mean(resid)) %>% 
  mutate(mysd = sd(resid)) %>% 
  mutate(mytag = tag(binno))
# bysort binno
mutate(binsize = count(pred_y)) %>% 
  mutate(uplim = 2*mysd/sqrt(binsize)) %>% 
  mutate(dwlim = -2*mysd/sqrt(binsize)) 
