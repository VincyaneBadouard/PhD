...
Moran_StPearson <- rslts$Moran_StPearson %>% mutate(Type = "StPearson")

Moran <- bind_rows(Moran_raw, Moran_Pearson, Moran_Deviance, Moran_StPearson)

rm(Moran_raw, Moran_Pearson, Moran_Deviance, Moran_StPearson, params, rslts)
```
...
if(!"logTransmittance" %in% names(Residuals)){
  ...
  Residuals <- Residuals %>% 
    bind_cols(datalist[[sp]] %>% dplyr::select(logDBH, Transmittance, logTransmittance, TWI, logTWI))
...
```{r}
Residuals_covar <- Residuals %>% pivot_longer(cols = c(logDBH, logTransmittance, logTWI),
...
cor(Residuals$Pearson_e, Residuals$StPearson) # 1

```

```{r}
Residuals_wide <- Residuals
Residuals <- Residuals %>% pivot_longer(cols = c(raw_e, Pearson_e, Deviance_e, StPearson_e),
...
p2 <- p1 %+% xs$Deviance_e
p3 <- p1 %+% xs$Pearson_e
p4 <- p1 %+% xs$StPearson_e

p1
gridExtra::grid.arrange(p1,p2,p3,p4, nrow = 1, ncol = 4, top = grid::textGrob(paste(sp, "- presences")))
```
...
gridExtra::grid.arrange(p1,p2,p3,p4, nrow = 1, ncol = 4, top = grid::textGrob(paste(sp, "- absences")))
```
...
```
Reweighting with the expected dispersion, as done in Pearson residuals, or using deviance residuals, helps to some extent, but it does not lead to visually homogenous residuals, even if the model is correctly specified (Hartig, 2024)
