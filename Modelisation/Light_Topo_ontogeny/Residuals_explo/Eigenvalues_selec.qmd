...
CoordXY <- Residuals %>% select(x_m, y_m) %>% unique() # dataframe with x and y coordinates in columns
plot(CoordXY)
```
...
eigenval <- cbind(CoordXY, SWM_selec$best$MEM.select) # n rows = n residuals, n col = n eigenvectors
eigenval <- DATA %>%
