...
```{r}
# Remove part of absences
datalist[["Iryanthera_hostmannii"]] %>% filter(Presence==1) # 373
datalist[["Iryanthera_hostmannii"]] %>% filter(Presence==0) # 6244
# datalist[["Iryanthera_hostmannii"]] <- datalist[["Iryanthera_hostmannii"]] %>% 
#   group_by(Presence) %>% 
#   sample_n(., size=373)
```

```{r}
p1 <-ggplot(datalist[["Iryanthera_hostmannii"]] %>% filter(Presence==0), aes(x=logTransmittance)) +
  xlim(-8,0) +
geom_histogram()
p2<-ggplot(truc %>% filter(Presence==0), aes(x=logTransmittance)) +
    xlim(-8,0) +
geom_histogram()
p1/p2
...
ggplot(datalist[["Iryanthera_hostmannii"]], aes(x= logTransmittance, y= Presence)) +
  geom_point() +
  theme_minimal() +
...
```{r}
lapply(names(fits), function(x) mcmc_pairs(as.array(fits[[x]]), 
                                           pars = c("alpha", "beta1_p", "beta2_p"),
                                           np = nuts_params(fits[[x]]))
)
```
