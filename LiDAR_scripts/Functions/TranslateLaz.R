...
#' 
TranslateLaz <- function(las, matrix){
  
  las@data <- setDT(las@data)
  
  setnames(las@data, c("X", "Y", "Z"), c("x", "y", "z"))  # change names of local coordinates
  ...
  setnames(las@data, c("V1", "V2", "V3"), c("X", "Y", "Z"))  # change names of local coordinates
