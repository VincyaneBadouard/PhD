...
# All the plots scanned by ALS in 2022 
...
```{r}
ctg <- readLAScatalog(folder= "Y:/lidar/ALS/Paracou/2022/TiledClassified") # las catalog
# 4 carrés
crs(ctg) # no crs
plot(ctg)
...
```{r}
shpParacou <- raster::shapefile("D:/Mes Donnees/PhD/SIG_data/Guyafor/GuyaforSubPlots.shp")

Plot13 <- shpParacou[shpParacou@data$Forest == "Paracou" & shpParacou@data$Plot == "13",]
plot(Plot13)
```

```{r}
las <- merge_spatial(ALS_Paracou2022, Plot13, "inpoly")
