---
title: "UAV pre-treatment one shot"
...
# 
# folders <- "P16_C14C15/Decimated/"
# 
# file <- "Subsampled_Returns_and_gpstime_corrected"
# # "UAV_sampletowork" # petit jeu de données
# # "UAV_C19C20_translated"
# 
# matrix1 <- as.matrix(read.table(paste(local, "P16_C14C15/Translation_matrix/Matrix_cor1.txt", sep = ""), header=F))
# matrix2 <- as.matrix(read.table(paste(local, "P16_C14C15/Translation_matrix/Matrix_Greg.txt", sep = ""), header=F))
# 
# trajectory <- "Output_traj.xyz"

# source("~/PhD/R_codes/PhD/LiDAR_scripts/Functions/TranslateLaz.R")
...
# LAStoolsDir =  "D:/Program Files/LAStools/bin/"
```

```{r}
ROI <- terra::vect("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/Plot16_4ha_buffer.shp") # interest zone + 100m buffer
...
# Load LiDAR data
```{r}
las0 <- readLAS(paste(local,folders,file,".laz", sep = "")) 
...
range(las1@data$X);range(las1@data$Y)
las1@data[, `:=`(X = X-285000.00 , Y = Y-581000.00)]

las2 <- TranslateLaz(las1, matrix2, suffix = "_prev")
# View(las2@data[1:10])
range(las2@data$X);range(las2@data$Y)
las2@data[, `:=`(X = X+285000.00 , Y = Y+581000.00)]

las2@data[, c("x_prev", "y_prev", "z_prev","x_local", "y_local", "z_local"):= NULL]
names(las2@data)

new_las<-LAS(data = las2@data) # reset the header
writeLAS(new_las, paste(local,folders,file, "_translated.laz", sep = "")) # coord checked, spatialisation checked
```

```{r}
rm(las1,las0, las2); gc()
```
...
# Write
fwrite(Trans2, paste(local,folders,"UAV_traj_retranslated.xyz", sep = "")) # coord checked, spatialisation checked
```
...
```{r}
options(digits=22)
traj_crop <- traj[y<581800 & x<285800,] # 286321

# rgl::plot3d(traj_crop[,3:5], aspect=F) # plot the trajectory (x,y,z)

# Write
fwrite(traj_crop, paste(local,folders,"UAV_traj_retranslated_cropped.xyz", sep = "")) # coord checked, spatialisation checked
```

```{r}
las_crop <- new_las
las_crop@data <- las_crop@data[Y<581800 & X<285800,]
# plot(las_crop) # fait planter R

min(las_crop@data$X);min(las_crop@data$Y)

# Write
writeLAS(las_crop, paste(local,folders,file, "_cropedtraj.laz", sep = "")) # coord checked, spatialisation checked
```
```{r}
traj <- traj_crop ; rm(traj_crop)
rm(new_las) ; gc()
```
...
hist(laz_scanAngles@data$ScanAngleRank) # 0 - 162

laz_scanAngles@data <- laz_scanAngles@data[ScanAngleRank<=99,] # 99 to be 8 bits

writeLAS(laz_scanAngles, paste(local,folders,file, "_scanAngles.laz", sep = "")) # # coord checked, spatialisation checked
```

# Filter < 2m range and angles < 45°
UAV altitude flight: 85m
```{r}
# laz_scanAngles <- readLAS(paste(local,folders,file, "_scanAngles.laz", sep = ""))
laz_scanAngles@data <- laz_scanAngles@data[ScanAngleRank<=45,]
laz_scanAngles@data <- laz_scanAngles@data[Z<=80,]
laz_scanAngles@data <- laz_scanAngles@data[Range>2,] # enlever le drone
...
max(laz_scanAngles@data$Z) # 63.437
...
## In and output
```{r}
inFiles = paste(local,folders, file, "_AnglesandZ_filtered.laz", sep = "" )
...
laz_GroundClassif <- readLAS(paste(local,folders, file, "_lastoolsclassif.laz", sep = "" ))
...
range(laz_GroundClassif$Z) # 4.642 65.534
range(laz_GroundClassif@data[Classification == 2L,]$Z) # Points sol: 4.642 32.813
range(laz_GroundClassif@data[Classification == 1L,]$Z) # Points vegetation: 4.642 65.534
...
```{r}
rm(laz_GroundClassif, mnt, mntROI, matrix1, matrix2, UAV_DEM, UAV_DEMROI, diffmnt) ; gc()
```

--------------------------------------------------------------------------------
# Fusion
**A faire après les traitements de préparation pour AMAPVox**

## Merge DEMs
```{r}
mnt_C14C15 <- rast(paste(local,"AMAPVox/MNT/dtm2023_4ha_UAV_C14C15_buffer.asc", sep=""))
mnt_C19C20 <- rast(paste(local,"AMAPVox/MNT/dtm2023_4ha_UAV_C19C20_buffer.asc", sep=""))
...
# Define las/laz files to be processed
inFiles <- paste(local, "AMAPVox/LAZ", '/*.laz', sep='') # files
outFile = "P16_2023_UAV_4ha_buffer_intensitycor_lastools.laz"
outDir <- paste(local, "AMAPVox/LAZ", sep='')
...
## Merge trajectories
```{r}
traj_C14C15 <- fread(paste(local,"AMAPVox/Traj/UAV_C14C15_traj_retranslated.xyz", sep=""))
traj_C19C20 <- fread(paste(local,"AMAPVox/Traj/UAV_C19C20_traj_retranslated.xyz", sep=""))

traj_UAV <- rbind(traj_C14C15, traj_C19C20)

fwrite(traj_UAV, paste(local,"AMAPVox/Traj/UAV_P16_4ha_traj.txt", sep=""))
```
