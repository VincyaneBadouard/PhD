...
```{r}
PC <- readLAS("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_4ha_buffer.laz")
...
writeLAS(PC, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_4ha_buffer.laz")
# //amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_4ha_buffer.laz
# "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_4ha_HighAlt_buffer.laz"
# "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/P16_2022_4ha_buffer.laz"
```
...
```{r}
path <- "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_HighAlt_4ha_buffer_Light_annual.txt"
...
fwrite(Lightcrop, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_HighAlt_4ha_Light_annual_1m_UTM.txt", row.names = F, col.names = T)
...
write.csv(Lightcrop, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_HighAlt_4ha_Light_annual_1m_UTM.csv")
...
HighWet <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_HighAlt_4ha_Light_Wet_1m_UTM.csv") %>% 
  mutate(Transmittance_HighWet = rowMeans(select(., `Period.1`, `Period.2`, `Period.3`)))  %>% select(Xutm, Yutm, Z, Transmittance_HighWet)
HighDry <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_HighAlt_4ha_Light_Dry_1m_UTM.csv") %>% 
  mutate(Transmittance_HighDry = rowMeans(select(., `Period.1`, `Period.2`))) %>% select(Xutm, Yutm, Z, Transmittance_HighDry)
# Periodes ok ? mean ok ? arrondir ok ?
...
LAI2200_Lidar <- LAI2200 %>% 
  mutate(Xutm = round(Xutm), 
         Yutm = round(Yutm),
         Zutm = round(Zutm)) %>% 
  left_join(LiDAR, by= c("Xutm","Yutm","Zutm")) # ttes les coordonnées ne matchent pas

# Relations plots
LAI2200_Lidar %>% 
  filter(Ring=="Ring1") %>%
  ggplot() +
  aes(x = Transmittance_mean, y = Transmittance_HighDry) +
...
# Correlation tests
# Transmittance_mean, Transmittance_LowAnnual, Transmittance_HighAnnual,Transmittance_HighDry, Transmittance_HighWet

DF_cor <- LAI2200_Lidar %>% 
  filter(Ring=="Ring1") %>%
  select(Transmittance_mean,Transmittance_LowAnnual,Transmittance_HighAnnual,Transmittance_HighDry,Transmittance_HighWet) # 
# na.omit()
