...
ST <- readLAS("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_4ha_buffer.laz")
...
ROI <- vect("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/Plot16_4ha_buffer.shp") # interest zone + 100m buffer
plot(ROI)
...
mnt <- rast("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/dtm2023.asc")
ggplot() + 
  geom_spatraster(data = mnt, aes(fill = dtm2023)) + # mnt_roi36ha_1m
...
plot(mntROI)
ggplot() + 
  geom_spatraster(data = mntROI, aes(fill = dtm2023)) + # mnt_roi36ha_1m
  scale_fill_gradientn(name = "Elevation (m)",
                       colors = terrain.colors(30, rev=T),
                       na.value="white") +
  geom_sf(data = sf::st_cast(ROI, "LINESTRING"))

writeRaster(raster(mntROI),"//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/dtm2023_4ha_buffer.asc", format="ascii", overwrite=T)
...
# if one file :
path <- "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/trajectory.txt"
...
# Crop trajectory
options(digits=22)
range(PC@data$gpstime) # 354557509.8303056955338 354559330.2065098285675
traj <- traj[time<=max(PC@data$gpstime) & time>=min(PC@data$gpstime),]
...
fwrite(traj, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/ALS_P16_2022_4ha_buffer_traj.txt")
...
writeLAS(ST, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_4ha_buffer.laz")
...
```{r}
VX <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_LowAlt_4ha_buffer.vox")
