...
VX@data[,Forest := NULL][,Plot := NULL][,SubPlot := NULL][,Projet := NULL][,Protocole := NULL][,SubPlotCon := NULL][,x := NULL][,y := NULL][,z := NULL]

# names(VX@data) == names(VXals@data)

VXals@data[,attenuation_PPL_MLE := NULL] 

VX_PAD <- VX@data[,.(i,j,k,HLE, PadBVTotal)]
VXals_PAD <- VXals@data[,.(i,j,k,HLE, PadBVTotal)] ; gc()

VX_merge <- merge(VX,VXals)
