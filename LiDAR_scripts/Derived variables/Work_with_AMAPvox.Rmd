...
ST <- readLAS(paste(local,folders, "Returns_and_gpstime_corrected_lastoolsclassif.laz", sep=""))
...
mnt <- rast(paste(local,folders,"Returns_and_gpstime_corrected_DEM.tif", sep = ""))
...
writeLAS(PC, paste(local,"AMAPVox/LAZ/P16_2023_",file,"_buffer_intensitycor.laz", sep=""))
...
# Pour UAV, crop vox and remove drone
```{r}
VX2m <- readVoxelSpace(
...
View(VX2m@data)
AMAPVox::plot(VX2m, variable.name = "ground_distance") 
```

## Remove drone path
```{r}
VX2m@data <- VX2m@data[ground_distance<61.52,]

AMAPVox::plot(VX2m, variable.name = "ground_distance") 
AMAPVox::plot(VX2m, variable.name = "nbSampling") 
```

## Crop vox
```{r}
VX2m <- AMAPVox::crop(VX2m, imin = (-70), imax = 170, jmin = (-150), jmax = 170)
obj <- c(-70, -150, 0)
names(obj) <- c("x", "y", "z")
VX2m@header$mincorner <- obj
