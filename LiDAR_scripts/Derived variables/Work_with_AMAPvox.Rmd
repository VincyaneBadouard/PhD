...
# Si 1 fichier
ST <- readLAS("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/HighFlight_alt4ha_buff100m_2023_RefAsInt_New.laz")
# "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/HighFlight_alt4ha_buff100m_2023_RefAsInt_New.laz"
# "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/LowFlight_alt4ha_buff100m_2023_RefAsInt_New.laz"
...
FluxTowerData <- FluxTowerData %>% 
  ...
  filter(Rg > 0) %>%
  group_by(Day) %>% 
  mutate(Rain = sum(as.numeric(Rain))) %>%  # il faut sommer la pluie par jour pour visualiser les saisons
  ungroup() %>%
  select(Year, Day, Month, Rain, Rg) %>% # 365 days, 12 months
  mutate(Season = ifelse(Month  %in% c(2,3, 8:11), # mars, aout-nov = dry season
                         "Dry", "Wet")) %>% 
  unique()

ggplot(FluxTowerData, aes(x = as.character(Month),y = Rain)) +
