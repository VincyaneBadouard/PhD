...
# Load LiDAR data
```{r}
las <- readLAS(paste(local,folders,file,".laz", sep = "")) # 1 min (20.4 GB)
# "Y:/users/VincyaneBadouard/Lidar/HovermapUAV2023/Processed/UAV_sampletowork.laz"
...
traj_beforetranslation <- fread(paste(local,"users/VincyaneBadouard/Lidar/HovermapUAV2023/P16_C14C15/C14C15_Output_traj.xyz", sep = ""))[,1:5]
# traj$time %in% traj_beforetranslation$time
traj <- traj[traj_beforetranslation, on = .(time)]
traj <- merge(traj,traj_beforetranslation, by = "time", all.x = T) 
traj <- traj[!is.na(gpstime),]
...
ROI <- vect(paste(local,"users/VincyaneBadouard/Lidar/ALS2022/Plot16_4ha_buffer100m.shp", sep = "")) # interest zone + 100m buffer
ROI <- st_as_sf(ROI) # as sf object
ROI <- st_set_crs(ROI, 2972) # attribuer le dernier crs
las <- st_set_crs(las, 2972) 

las_crop <- lidR::clip_roi(las = las, geometry = ROI) # très long
las;las_crop 
plot(las_crop)

writeLAS(las_crop, paste(local,folders,file, "_croped.laz", sep = "" ))
```
...
max(laz_angles_filtered@data$Z) # 65.6
range(laz_angles_filtered@data$ScanAngle) #
```
...
outFile = paste(file, "_GroundClassif.laz", sep = "" )
```
...
table(las@data$Classification) # 2 = sol
plot(las, color = "Classification") # points sols trop hauts !

range(laz_GroundClassif@data[, Classification := 2]$Z) # Points sol: 7.4242 65.6011! (attendu : (Z>(-0.2) & Z< 0.5))
```
...
UAV_DEM <- lidR::rasterize_terrain(laz_GroundClassif, algorithm = lidR::tin()) # to mnt
terra::writeRaster(UAV_DEM, file.path(paste(file, "_DEM.asc", sep = "" )), overwrite=TRUE)
```

## Check
```{r}
UAV_DEM <- rast(paste(local,folders,file, "_DEM.asc", sep = "" ))
