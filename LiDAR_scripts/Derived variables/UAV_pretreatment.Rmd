...
## Check
```{r}
laz_GroundClassif <- readLAS(paste(local,folders, subplot, "_translated_merged_CloudCompareClassif.las", sep = "" ))
...
UAV_DEM <- lidR::rasterize_terrain(laz_GroundClassif, res = 1, algorithm = lidR::tin()) # to mnt
terra::writeRaster(UAV_DEM, file.path(paste(local,folders, subplot, "_DEM.asc", sep = "" )))
```

## Check
```{r}
UAV_DEM <- rast(paste(local,folders, subplot, "_DEM.asc", sep = "" ))
...
ggsave(paste(subplot, "_DEM_1m.png", sep = "" ), path = "D:/Mes Donnees/PhD/Figures/lidar/UAV", width = 25, height = 15, units = "cm", dpi=800, bg="white")
```
...
ggsave(paste("ALS-", subplot, "_2023_DEM_difference.png", sep = ""), path = "D:/Mes Donnees/PhD/Figures/lidar/UAV", width = 25, height = 15, units = "cm", dpi=800, bg="white")
```
