...
terra::writeRaster(UAV_DEM, file.path(paste(local, folders, file, "_DEM.asc", sep = "")), overwrite=T)
```

## Check
```{r}
UAV_DEM <- raster(paste(local, folders, file, "_DEM.asc", sep = ""))
...
ggsave(paste(file, "_DEM2_1m_crop.png", sep = "" ), path = "D:/Mes Donnees/PhD/Figures/lidar/UAV", width = 25, height = 15, units = "cm", dpi=800, bg="white")
```
...
las <- readLAS(paste(local,folders,"UAV_",subP,"_classif_retranslated1step",".las", sep = "")) # 1 min (20.4 GB)
...
writeLAS(las_crop, paste(local,folders, "UAV_",subP,"_classif_retranslated_croped.laz", sep = "" ))
```
