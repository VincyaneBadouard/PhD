...
MNT 2022 de l'entreprise ? non le reconstruire moi avec tin
```{r}
# Triangulation
mnt <- rasterize_terrain(ST, res = 5, algorithm = tin())
plot_dtm3d(mnt, bg = "black")

writeRaster(mnt, "Z:/users/VincyaneBadouard/Lidar/ALS2022/P13/P13_2022_mnt5m.tif", overwrite = TRUE) 

mnt <- raster("Z:/users/VincyaneBadouard/Lidar/ALS2022/P13/P13_2022_mnt5m.tif")

```


# Compute Topographic Wetness Index (TWI)
A calculer pour l’ALS et pour le MLS mais **pas pour l’UAV**
...
wbt_d_inf_flow_accumulation(input = "Z:/users/VincyaneBadouard/Lidar/ALS2022/P13/P13_2022_mnt5m.tif",
                            output = "Z:/users/VincyaneBadouard/Lidar/ALS2022/P13/P13_2022_sca.tif",
                            out_type = "Specific Contributing Area")

# Compute the slope raster in degrees
wbt_slope(dem = "Z:/users/VincyaneBadouard/Lidar/ALS2022/P13/P13_2022_mnt5m.tif",
...
ggplot() + 
  ...
  coord_sf()
```
Même à 5m de résolution ce n'est pas ce qu'on attend. On fera donc le TWI sous SAGA

**A FAIRE TWI vérifier avec celui de saga**

## With saga
Il faut saga 8.4.1 max
- **envirem::topoWetnessIndex()** 
- RSAGA::rsaga.geoprocessor(lib = "terrain_analysis", module = "Topographic Wetness Index (One Step)", env = envi)
...
Saga_env <- rsaga.env() 
Saga_env
# Saga_env <- rsaga.env(workspace = ".", path = "C:/Progra~1/SAGA",
#                       modules = "C:/Progra~1/SAGA/tools", cores = 2, parallel = TRUE, version = "9.0.0")

rsaga.geoprocessor(lib = "terrain_analysis", module = "Topographic Wetness Index (One Step)", env = Saga_env)

SWI1 <- rsaga.wetness.index("P13_2022_mnt.sgrd", "TWI.sgrd", env = Saga_env)

# Avec envirem
library(envirem)
SWI2 <- topoWetnessIndex(mnt, sagaEnv = Saga_env)
```
