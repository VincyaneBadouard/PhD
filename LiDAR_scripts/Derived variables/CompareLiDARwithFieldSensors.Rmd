...
LAI2200_Lidar <- LAI2200_Lidar %>% 
  select(Sensor,Transect,Id, Xutm,Yutm, Ring,azimut,Transmittance_mean,Transmittance_High,Transmittance_Low, `GAP[1]_High`, `GAP[1]_Low`) %>% 
  ...
  mutate(Transmittance_HighvsLAI2200 = Transmittance_High -Transmittance_mean) %>% 
  mutate(Transmittance_LowvsLAI2200 = Transmittance_Low - Transmittance_mean)


LAI2200_Lidarsf <- st_as_sf(LAI2200_Lidar, coords = c('Xutm','Yutm'))
...
for(v in c("Transmittance_HighvsLAI2200", "Transmittance_LowvsLAI2200")){
  print(
    LAI2200_Lidar %>% 
      ...
      ggplot() +
      ggtitle("Paracou P16 4ha - Transmittances differences") +
      aes(x = Transmittance_mean, y = get(v)) +
      geom_point() +
      geom_hline(yintercept=0) +
      geom_segment(aes(yend=0)) +
...
LAI2200_Lidar %>% 
  ...
  aes(x = Transmittance_mean, y = Transmittance_Low) +
...
LAI2200_Lidar %>% 
  ...
  aes(x = Transmittance_mean, y = `GAP[1]_Low`) +
