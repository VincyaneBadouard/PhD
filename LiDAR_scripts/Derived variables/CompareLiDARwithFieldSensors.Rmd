...
HighAnnual_int2m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Intensity_2m/P16_2023_HighAlt_4ha_buffer_LAI2200sim_intensity2m_test.txt"))
LowAnnual_int2m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/Intensity_2m/P16_2023_LowAlt_4ha_buffer_LAI2200sim_intensity2m_test.txt"))
HighAnnual_int1m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Intensity_1m/P16_2023_HighAlt_4ha_buffer_LAI2200sim_intensity1m_test.txt"))

# Equal echo weighting 
HighAnnual_equal2m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Equalecho_2m/P16_2023_HighAlt_4ha_buffer_LAI2200sim_Equalecho_2m_test.txt"))
LowAnnual_equal2m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/Equalecho_2m/P16_2023_LowAlt_4ha_buffer_LAI2200sim_Equalecho_2m_test.txt"))
```
...
rm(HighAnnual_int2m, LowAnnual_int2m, HighAnnual_int1m, HighAnnual_equal2m, LowAnnual_equal2m)
```
...
LiDAR <- LiDAR %>% 
  ...
  mutate(MeanTransmitta.nce_High1m = mean(Transmittance_High1m)) %>%
  mutate(SdTransmittance_High1m = sd(Transmittance_High1m)) %>% 
  mutate(SeTransmittance_High1m = plotrix::std.error(Transmittance_High1m)) %>%
  # Equal echo
...
for(acq in c("MeanTransmittance_HighInt2m", "MeanTransmittance_LowInt2m","MeanTransmittance_High1m")){
  print(
    LiDARsf %>% 
...
# Difference plots
```{r}
# "MeanTransmittance_HighInt2m", "MeanTransmittance_LowInt2m","MeanTransmittance_High1m"
# names(LAI2200_LiDAR)
LAI2200_LiDAR <- LAI2200_LiDAR %>% 
  # select(-c("...1.x","...1.y")) %>% 
  select(Sensor,Transect,Id, Xutm,Yutm, Ring,azimut,Transmittance_LAI2200, Transmittance_sd,
         ...
         MeanTransmittance_High1m, SdTransmittance_High1m, SeTransmittance_High1m,
  ...
  mutate(Transmittance_HighInt2mvsLAI2200 = MeanTransmittance_HighInt2m -Transmittance_LAI2200) %>% 
  mutate(Transmittance_LowInt2mvsLAI2200 = MeanTransmittance_LowInt2m - Transmittance_LAI2200) %>% 
  mutate(Transmittance_High1mvsLAI2200 = MeanTransmittance_High1m -Transmittance_LAI2200) %>% 
  mutate(Transmittance_High1mvsHigh2m = MeanTransmittance_High1m -MeanTransmittance_HighInt2m) %>% 
  
  mutate(Transmittance_HighEqual2mvsLAI2200 = MeanTransmittance_HighEqual2m -Transmittance_LAI2200) %>% 
  mutate(Transmittance_LowEqual2mvsLAI2200 = MeanTransmittance_LowEqual2m - Transmittance_LAI2200) %>% 
...
range(LAI2200_LiDAR$Transmittance_High1mvsLAI2200) # -0.57688611  0.09193129 : le moins proche du lAI2200

range(LAI2200_LiDAR$Transmittance_HighEqual2mvsLAI2200) #
range(LAI2200_LiDAR$Transmittance_LowEqual2mvsLAI2200) #
range(LAI2200_LiDAR$Transmittance_LowInt2mvsLowEqual2m) #
range(LAI2200_LiDAR$Transmittance_HighInt2mvsHighEqual2m) #
...
for(v in c("Transmittance_LowvsHigh_2m", "Transmittance_HighInt2mvsLAI2200", "Transmittance_LowvsLAI2200", "Transmittance_High1mvsLAI2200", "Transmittance_High1mvsHigh2m")){
  print(
    ggplot() +
      geom_sf(data = sf::st_cast(zone, "LINESTRING")) + # zone of interest
...
```
```{r}
# Transmittance_LAI2200, "MeanTransmittance_HighInt2m", "MeanTransmittance_LowInt2m","MeanTransmittance_High1m"
  ...
  for(R in 1:5){
    ...
    print(
      LAI2200_LiDAR %>% 
        ...
        filter(Ring==R) %>%
        ggplot(aes(x = Id)) +
        geom_point(aes(y = MeanTransmittance_HighInt2m, colour = "High altitude - 2m")) +
        geom_line(aes(y = MeanTransmittance_HighInt2m, colour = "High altitude - 2m")) +
        geom_errorbar(aes(ymin=MeanTransmittance_HighInt2m-SeTransmittance_HighInt2m,
                          ymax=MeanTransmittance_HighInt2m+SeTransmittance_HighInt2m, colour = "High altitude - 2m"), width=.2) +
        
        
        geom_point(aes(y = MeanTransmittance_LowInt2m, colour = "Low altitude - 2m")) +
        geom_line(aes(y = MeanTransmittance_LowInt2m, colour = "Low altitude - 2m")) +
        geom_errorbar(aes(ymin=MeanTransmittance_LowInt2m-SeTransmittance_LowInt2m,
                          ymax=MeanTransmittance_LowInt2m+SeTransmittance_LowInt2m, colour = "Low altitude - 2m"), width=.2) +
        
        
        geom_point(aes(y = MeanTransmittance_High1m, colour = "High altitude - 1m")) +
        geom_line(aes(y = MeanTransmittance_High1m, colour = "High altitude - 1m")) +
        geom_errorbar(aes(ymin=MeanTransmittance_High1m-SeTransmittance_High1m,
                          ymax=MeanTransmittance_High1m+SeTransmittance_High1m, colour = "High altitude - 1m"), width=.2) +
        ...
        scale_colour_manual(values = c("High altitude - 2m" = "blue",
                                       ...
                                       "High altitude - 1m" = "#009E73",
                                       "LAI2200" = "#D55E00"),
                            breaks=c("LAI2200", "High altitude - 2m", "Low altitude - 2m", "High altitude - 1m")) +
    ...
    ggsave(paste("P16_2023_4ha_Long_profile_",t,"_transect_Ring",R,"_se.png", sep=""),
           path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_2m",
           width = 25, height = 20, units = "cm", dpi=800, bg="white")
  }
}
...
```{r}
# MeanTransmittance_LAI2200, "MeanTransmittance_HighInt2m", "MeanTransmittance_LowInt2m","MeanTransmittance_High1m"

LAI2200_LiDAR_ring <- LAI2200_LiDAR %>% 
  group_by(Ring) %>% #Transect,
  mutate(MeanTransmittance_LAI2200_ring = mean(MeanTransmittance_LAI2200)) %>% # mean
  mutate(SeTransmittance_LAI2200_ring = plotrix::std.error(MeanTransmittance_LAI2200)) %>% # standard error
  mutate(SdTransmittance_LAI2200_ring = sd(MeanTransmittance_LAI2200)) %>% # standard deviation
  
  mutate(MeanTransmittance_HighInt2m_ring = mean(MeanTransmittance_HighInt2m)) %>% 
  mutate(SeTransmittance_HighInt2m_ring = plotrix::std.error(MeanTransmittance_HighInt2m)) %>% 
  mutate(SdTransmittance_HighInt2m_ring = sd(MeanTransmittance_HighInt2m)) %>% 
  
  mutate(MeanTransmittance_LowInt2m_ring = mean(MeanTransmittance_LowInt2m)) %>%
  mutate(SeTransmittance_LowInt2m_ring = plotrix::std.error(MeanTransmittance_LowInt2m)) %>%
  mutate(SdTransmittance_LowInt2m_ring = sd(MeanTransmittance_LowInt2m)) %>%
  
  mutate(MeanTransmittance_High1m_ring = mean(MeanTransmittance_High1m)) %>% 
  mutate(SeTransmittance_High1m_ring = plotrix::std.error(MeanTransmittance_High1m)) %>% 
  mutate(SdTransmittance_High1m_ring = sd(MeanTransmittance_High1m)) %>% 
  
  # Equal echo
  mutate(MeanTransmittance_HighEqual2m_ring = mean(MeanTransmittance_HighEqual2m)) %>% 
  mutate(SeTransmittance_HighEqual2m_ring = plotrix::std.error(MeanTransmittance_HighEqual2m)) %>% 
  mutate(SdTransmittance_HighEqual2m_ring = sd(MeanTransmittance_HighEqual2m)) %>% 
  
  mutate(MeanTransmittance_LowEqual2m_ring = mean(MeanTransmittance_LowEqual2m)) %>%
  mutate(SeTransmittance_LowEqual2m_ring = plotrix::std.error(MeanTransmittance_LowEqual2m)) %>%
  mutate(SdTransmittance_LowEqual2m_ring = sd(MeanTransmittance_LowEqual2m)) %>%
...
LAI2200_LiDAR_ring %>% 
  ...
  geom_errorbar(aes(ymin=MeanTransmittance_LAI2200_ring-SeTransmittance_LAI2200_ring,
                    ymax=MeanTransmittance_LAI2200_ring+SeTransmittance_LAI2200_ring,
                    colour = "LAI2200"), alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_LowInt2m_ring-SeTransmittance_LowInt2m_ring,
                    ymax=MeanTransmittance_LowInt2m_ring+SeTransmittance_LowInt2m_ring,
                    colour = "Low altitude - Intens - 2m"),alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_HighInt2m_ring-SeTransmittance_HighInt2m_ring,
                    ymax=MeanTransmittance_HighInt2m_ring+SeTransmittance_HighInt2m_ring,
                    colour = "High altitude - Intens - 2m"),alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_High1m_ring-SeTransmittance_High1m_ring,
                    ymax=MeanTransmittance_High1m_ring+SeTransmittance_High1m_ring,
                    colour = "High altitude - Intens - 1m"),alpha =1, width=.2) +
  
  geom_errorbar(aes(ymin=MeanTransmittance_LowEqual2m_ring-SeTransmittance_LowEqual2m_ring,
                    ymax=MeanTransmittance_LowEqual2m_ring+SeTransmittance_LowEqual2m_ring,
                    colour = "Low altitude - Equal - 2m"),alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_HighEqual2m_ring-SeTransmittance_HighEqual2m_ring,
                    ymax=MeanTransmittance_HighEqual2m_ring+SeTransmittance_HighEqual2m_ring,
                    colour = "High altitude - Equal - 2m"),alpha =1, width=.2) +
  
  geom_point(aes(y = MeanTransmittance_HighInt2m_ring, colour = "High altitude - Intens - 2m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_HighInt2m_ring, colour = "High altitude - Intens - 2m")) +
  
  geom_point(aes(y = MeanTransmittance_LowInt2m_ring, colour = "Low altitude - Intens - 2m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_LowInt2m_ring, colour = "Low altitude - Intens - 2m")) +
  
  geom_point(aes(y = MeanTransmittance_High1m_ring, colour = "High altitude - Intens - 1m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_High1m_ring, colour = "High altitude - Intens - 1m")) +
  
  geom_point(aes(y = MeanTransmittance_HighEqual2m_ring, colour = "High altitude - Equal - 2m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_HighEqual2m_ring, colour = "High altitude - Equal - 2m")) +
  
  geom_point(aes(y = MeanTransmittance_LowEqual2m_ring, colour = "Low altitude - Equal - 2m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_LowEqual2m_ring, colour = "Low altitude - Equal - 2m")) +
  
  geom_point(aes(y = MeanTransmittance_LAI2200_ring, colour = "LAI2200"),size = 3) +
  geom_line(aes(y = MeanTransmittance_LAI2200_ring, colour = "LAI2200")) +
...
ggsave(paste("Long_profile_The2_transects_MeanStandardErrorTransmittance_of_the_transect_byRing_IntensityandEqualEcho.png", sep=""),
...
LAI2200_LiDAR_transect <- LAI2200_LiDAR %>% 
  ...
  mutate(MeanTransmittance_LAI2200_ring = mean(MeanTransmittance_LAI2200)) %>% # mean
  mutate(SeTransmittance_LAI2200_ring = plotrix::std.error(MeanTransmittance_LAI2200)) %>% # standard error
  mutate(SdTransmittance_LAI2200_ring = sd(MeanTransmittance_LAI2200)) %>%
...
for(t in c("HOBO", "LAY")){
  ...
  ggsave(paste("Long_profile_",t," transect","_MeanStandardErrorTransmittance_of_the_transect_byRing_IntensityandEqualEcho.png", sep=""),
         path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_1m",
         width = 25, height = 20, units = "cm", dpi=800, bg="white")
}
```
...
LAI2200_LiDAR %>% 
  ...
  geom_point(aes(y = Transmittance_LowInt2m, colour = "Low altitude - 2m")) +
  geom_point(aes(y = Transmittance_HighInt2m, colour = "High altitude - 2m")) +
  scale_colour_manual(values = c("Low altitude - 2m" = "green", 
                                 "High altitude - 2m" = "darkgreen")) +
  stat_smooth(method = "lm",
              formula = y ~ x) +
  geom_abline() +
  coord_fixed(ratio=1) + # same axis size
...
DF_cor <- LAI2200_LiDAR %>% 
  select(Ring, Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_High1m,
         Transmittance_HighEqual2m,Transmittance_LowEqual2m) %>%  
  unique() %>% 
  pivot_wider(names_from = Ring,
              values_from = c(Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_HighInt1m,
                              Transmittance_HighEqual2m,Transmittance_LowEqual2m),
              values_fn = list,
...
DF_cor <- LAI2200_LiDAR %>% 
  select(Ring, Transmittance_LAI2200,
         Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_HighInt1m,
         Transmittance_HighEqual2m,Transmittance_LowEqual2m) %>% # `GAP[1]_High`,`GAP[1]_Low`, LAI_High, LAI_Low 
  unique() %>% 
  pivot_wider(names_from = Ring,
              values_from = c(Transmittance_LAI2200,
                              Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_HighInt1m,
                              Transmittance_HighEqual2m,Transmittance_LowEqual2m),
              values_fn = list,
              names_glue = "{.value}_Ring{Ring}") %>% 
...
## Combine HOBO with LiDAR and LAI2200 data
```{r}
...
DF_cor <- HOBO_LAI2200_LiDAR %>% 
  # filter(Ring==1) %>%
  select(Ring, Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_High1m, Transmittance_LAI2200,HOBO_Light_october_sum, HOBO_Light_annual_sum) %>% 
  unique() %>% 
  pivot_wider(names_from = Ring,
              values_from = c(Transmittance_LAI2200,
                              Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_High1m),
              values_fn = list,
              names_glue = "{.value}_Ring{Ring}") %>% 
  unnest() # pcq les colonnes sont en listes
# na.omit()
