...
rm(HighAnnual_int2m, LowAnnual_int2m, HighAnnual_int1m, HighAnnual_equal2m, LowAnnual_equal2m, EqualEch, Read_LAI2200_Transfile)
```
...
for(t in c("HOBO", "LAY")){
  for(R in 1:5){
    ...
    ggsave(paste("P16_2023_4ha_Long_profile_",t,"_transect_Ring",R,"_se_IntensityandEqualEcho.png", sep=""),
...
LAI2200_LiDAR_ring <- LAI2200_LiDAR %>%
  ...
  summarise(across(c("MeanTransmittance_LAI2200", "MeanTransmittance_HighInt2m", "MeanTransmittance_LowInt2m", "MeanTransmittance_HighInt1m", "MeanTransmittance_HighEqual2m", "MeanTransmittance_LowEqual2m"), # Variables à analyser
...
DF_cor_LiDAR <- LAI2200_LiDAR %>% 
  select(Ring, MeanTransmittance_HighInt2m, MeanTransmittance_LowInt2m, MeanTransmittance_HighInt1m, MeanTransmittance_HighEqual2m, MeanTransmittance_LowEqual2m) %>%  
  unique() %>% 
...
DF_cor_LiDARvsLAI2200 <- LAI2200_LiDAR %>% 
  select(Ring, Transmittance_LAI2200,
         MeanTransmittance_HighInt2m, MeanTransmittance_LowInt2m, MeanTransmittance_HighInt1m,
         MeanTransmittance_HighEqual2m, MeanTransmittance_LowEqual2m) %>% # `GAP[1]_High`,`GAP[1]_Low`, LAI_High, LAI_Low 
  unique() %>% 
  pivot_wider(names_from = Ring,
...
DF_cor <- HOBO_LAI2200_LiDAR %>% 
  # filter(Ring==1) %>%
  select(Ring, Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_HighInt1m, Transmittance_LAI2200,HOBO_Light_october_sum, HOBO_Light_annual_sum) %>% 
  unique() %>% 
  pivot_wider(names_from = Ring,
              values_from = c(Transmittance_LAI2200,
                              Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_HighInt1m),
