...
```
# Profils en long (moyennes des Transmittances par acuiqition, par ring)
```{r}
LAI2200_LiDAR <- LAI2200_LiDAR %>% 
  group_by(Ring) %>% 
...
## Correlation tests
a colum for each transmittance for each ring
```{r}
# Transmittance_LAI2200, Transmittance_Low, Transmittance_High
...
corrplot(CorMatrix, method="circle", type="lower", diag = F, 
         col=brewer.pal(n=8, name="PuOr"), tl.col="black", tl.cex = 1, tl.srt=45, p.mat = Pval_corr)
# dev.off()
