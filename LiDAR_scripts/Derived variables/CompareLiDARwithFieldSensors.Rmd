...
```{r}
zone <- st_as_sf(vect("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/Shapefiles/Parcelles_Understory.shp")) # 4 carrés
```
...
## Load LAI2200 data
```{r, include = F}
LAI2200_coord <- read_csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/Coordinates/LAI2200_coordshift_Qgis_local.csv") # 46 LAI2200 sensors coordinates
...
LAI2200_data %>% 
  ...
  summarise(n = n()) 
# LAY id 22-26 only 2 rep
# (20*4 +26*3)*5 = 790
# 790-765 = 25 = 5*5

LAI2200_data %>%
  filter(File_Id == "P16LAY3") %>% 
  select(Id) %>% unique() # 21

LAI2200_data %>% 
  ggplot(aes(Id, Transmittance, col=File_Id)) +
  geom_point() +
  facet_grid(~Transect, scales = "free")
```

According to the LAI2200 calibration description (see https://www.licor.com/support/LAI-2200C/topics/some-esoteric-details.html): “The wand calibration involves placing the wand into an integrating sphere (an isotropic environment), and results in the five ring multipliers that are stored in the wand's software. When these multipliers are applied, the wand can be used to make valid distribution measurements of brightness, in which the output of each ring can be compared to the others. That is, under a perfectly uniform sky, all rings would read the same value in a complete open area and homogeneous environment".  
Our current understanding is that the radiance per ring is internally standardized so that each reading does not report the actual radiance of the corresponding sky sector. Therefore we fully agree that summing all 5 rings values to compute hemispherical transmittance (i.e. the ratio of the total transmitted radiant flux through the canopy to the total incident flux, where the incident light comes from all directions over a hemisphere) is not valid.

We have changed this and now compute hemispherical transmittance from LAI as: 
**sum((reading_i_below / reading_i above) * weighting factor)**.  

Each ring should contribute in proportion of the sky view sampled by all rings together (instead of 20% after Licor calibration). Each weighting factor is computed (angle_sr_i)/sum(angle_sr_i), giving the following numerical values:  

ring 1 = 0.14/3.52 ~0.04  
ring 2 = 0.5/3.52 ~ 0.14  
ring 3 = 0.74/3.52 ~0.21  
ring 4 = 0.94/3.52 ~0.27  
ring 5 = 1.2 / 3.52  ~0.34  
...
LAI2200_data <- LAI2200_data %>% 
  ...
  mutate(Transmittance = ifelse(Ring==4, Transmittance * 0.96,
                                Transmittance)) %>%
  mutate(Transmittance = ifelse(Ring==5, Transmittance * 0.23,
                                Transmittance)) %>%
  group_by(Transect, Id, Ring) %>% 
  mutate(MeanTransmittance_LAI2200 = mean(Transmittance)) %>% 
  mutate(SdTransmittance_LAI2200 = sd(Transmittance)) %>% 
  mutate(SeTransmittance_LAI2200 = plotrix::std.error(Transmittance)) %>% 
  ungroup() %>% 
  
  # Transmittance integrated over all the rings (to validate with HOBOs)
  # weighting factors
  mutate(Transmittance_W = case_when(
    Ring == 1 ~ Transmittance*(0.14/3.52),
    Ring == 2 ~ Transmittance*(0.5/3.52),
    Ring == 3 ~ Transmittance*(0.74/3.52),
    Ring == 4 ~ Transmittance*(0.94/3.52),
    Ring == 5 ~ Transmittance*(1.2/3.52))
  ) %>% 
  group_by(File_Id, Id) %>% 
  # mutate(Transmittance_integrated = sum(BELOW)/sum(ABOVE)) %>%
  mutate(Transmittance_integrated = sum(Transmittance_W)) %>% # sum per location and repetition across the rings
  ungroup() %>% 
  group_by(Transect, Id) %>% 
  mutate(MeanTransmittance_integrated = mean(Transmittance_integrated)) %>% # average the repetitions per location in the transect
  ungroup() %>% 
  select(c(Transect, Id, Ring,
           MeanTransmittance_LAI2200, SdTransmittance_LAI2200, SeTransmittance_LAI2200,
           MeanTransmittance_integrated)) %>%
  unique() # 46*5 = 230
...
LAI2200 <- rbind(LAI2200, HOBO10) # # 46*5 = 230
...
```{r}
ggplot(LAI2200_data, aes(x=as.factor(Id), y=MeanTransmittance_integrated)) +
  geom_point() +
  facet_grid(~Transect)
```
...
ggplot() +
  geom_sf(data = sf::st_cast(zone, "LINESTRING")) + # zone of interest
  geom_sf(data = LAI2200UTMsf, aes(color = MeanTransmittance_LAI2200), size = 1.5) +
...
print(
  LAI2200_LiDAR_corr %>% 
    ...
    geom_point(aes(y = MeanTransmittance_HighInt1m, colour = "High altitude ALS")) +
    geom_line(aes(y = MeanTransmittance_HighInt1m, colour = "High altitude ALS")) +
    geom_linerange(aes(ymin=MeanTransmittance_HighInt1m-SeTransmittance_HighInt1m,
                       ymax=MeanTransmittance_HighInt1m+SeTransmittance_HighInt1m, colour = "High altitude ALS")) +
    
    geom_point(aes(y = MeanTransmittance_LowInt1m, colour = "Low altitude ALS")) +
    geom_line(aes(y = MeanTransmittance_LowInt1m, colour = "Low altitude ALS")) +
    geom_linerange(aes(ymin=MeanTransmittance_LowInt1m-SeTransmittance_LowInt1m,
                       ymax=MeanTransmittance_LowInt1m+SeTransmittance_LowInt1m, colour = "Low altitude ALS")) +
    
    geom_point(aes(y = MeanTransmittance_UAVInt1m, colour = "UAV")) +
    geom_line(aes(y = MeanTransmittance_UAVInt1m, colour = "UAV")) +
    geom_linerange(aes(ymin=MeanTransmittance_UAVInt1m-SeTransmittance_UAVInt1m,
                       ymax=MeanTransmittance_UAVInt1m+SeTransmittance_UAVInt1m, colour = "UAV")) +
    
    
    scale_colour_manual(values = c("High altitude ALS" = "blue",
                                   "Low altitude ALS" = "#56B4E9",
                                   "UAV" = "#E69F00",
    ...
    #                                Transect = c("HOBO" = "HOBO transect",
    #                                             "LAY" = "LAY transect"))) +
    ggh4x::facet_grid2(Ring ~ Transect, scales = "free",
                       labeller = labeller(Ring = c('1'='Ring 1', '2'='Ring 2'),
                                           Transect = c("HOBO" = "HOBO transect",
                                                        "LAY" = "LAY transect")), independent = "all") +
    
    ggh4x::facetted_pos_scales(
      x = list(
        Transect == "LAY" ~ scale_x_continuous(breaks = seq(from =1, to=26, by=2)),
        Transect == "HOBO" ~ scale_x_continuous(breaks = seq(from =1, to=20, by=2))
      )
    ) +
...
ggsave("P16_2023_4ha_Long_profile_facet_Intensity1m.png",
       path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_1m/DZ_cor",
       width = 30, height = 20, units = "cm", dpi=800, bg="white")
...
print(
  LAI2200_LiDAR_corr %>% 
    ...
    theme_minimal() +
    ggh4x::facet_grid2(Ring ~ Transect, scales = "free",
                       labeller = labeller(Ring = c('3'='Ring 3', '4'='Ring 4', '5'='Ring 5'),
                                           Transect = c("HOBO" = "HOBO transect",
                                                        "LAY" = "LAY transect")), independent = "all") +
    # facet_wrap(~ Transect + Ring, scales = "free",
    #            labeller = labeller(Ring = c('3'='Ring 3', '4'='Ring 4', '5'='Ring 5'),
...
# ggsave("P16_2023_4ha_Long_profile_facet_Intensity1m_otherRings.png",
#        path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_1m/DZ_cor",
#        width = 45, height = 15, units = "cm", dpi=800, bg="white")
...
LAI2200_LiDAR_ring <- LAI2200_LiDAR %>%
  ...
  summarise(across(c(all_of(vars)), # Variables à analyser
                   ...
                   .names = "{.col}_{.fn}")) 
measuredTranmittance_mean <- mean(LAI2200_LiDAR_ring$MeanTransmittance_LAI2200_mean) # 0.032

# Relative differences in percent (i.e. (estimated-measured)/mean(measured)*100)
# label <- LAI2200_LiDAR_ring %>%
#   group_by(Ring) %>%
#   mutate(across(all_of(
#     c("MeanTransmittance_HighInt1m_mean", "MeanTransmittance_LowInt1m_mean","MeanTransmittance_UAVInt1m_mean")),
#     ~ ((.x - across("MeanTransmittance_LAI2200_mean"))/measuredTranmittance_mean)*100,
#     .names = "{col}_RD")) 
# 
...
# 
# 
# (LAI2200_LiDAR_ring$MeanTransmittance_HighInt1m_mean[1] - LAI2200_LiDAR_ring$MeanTransmittance_LAI2200_mean[1])/measuredTranmittance_mean

# Average transmittance difference across the rings in percent
label <- data.frame(
  Transect = as.factor("Both transect"),
  Ring = 4, 
  MeanTransmittance_LAI2200_mean = c(0.079, 0.073, 0.067) + 0.03, 
  Acq = c("Low altitude ALS", "High altitude ALS", "UAV"),
  error = paste("RE = ",round(unique(c(
    mean(LAI2200_LiDAR_ring$MeanTransmittance_LowInt1m_mean-LAI2200_LiDAR_ring$MeanTransmittance_LAI2200_mean),
    mean(LAI2200_LiDAR_ring$MeanTransmittance_HighInt1m_mean-LAI2200_LiDAR_ring$MeanTransmittance_LAI2200_mean),
    mean(LAI2200_LiDAR_ring$MeanTransmittance_UAVInt1m_mean-LAI2200_LiDAR_ring$MeanTransmittance_LAI2200_mean))/measuredTranmittance_mean*100
  ),0),"%", sep="")
)

# write.csv(label, "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_1m/Relative_differences_byRing_(estimated-measured)divmean(measured)x100.csv")
...
LAI2200_LiDAR_ring %>% 
  ...
  geom_linerange(aes(ymin=MeanTransmittance_HighInt1m_mean-MeanTransmittance_HighInt1m_se,
                     ...
                     colour = "High altitude ALS"),alpha =1) +
  geom_linerange(aes(ymin=MeanTransmittance_LowInt1m_mean-MeanTransmittance_LowInt1m_se,
                     ...
                     colour = "Low altitude ALS"),alpha =1) +
  ...
  geom_point(aes(y = MeanTransmittance_HighInt1m_mean, colour = "High altitude ALS"),size = 3) +
  geom_line(aes(y = MeanTransmittance_HighInt1m_mean, colour = "High altitude ALS")) +
  
  geom_point(aes(y = MeanTransmittance_LowInt1m_mean, colour = "Low altitude ALS"),size = 3) +
  geom_line(aes(y = MeanTransmittance_LowInt1m_mean, colour = "Low altitude ALS")) +
  
  
  geom_point(aes(y = MeanTransmittance_UAVInt1m_mean, colour = "UAV"),size = 3) +
  geom_line(aes(y = MeanTransmittance_UAVInt1m_mean, colour = "UAV")) +
  ...
  scale_colour_manual(values = c("High altitude ALS" = "blue", # "#009E73",
                                 "Low altitude ALS" = "#56B4E9", # "#CC79A7",
                                 "UAV" = "#E69F00", # "#F0E442", 
                                 "LAI2200" = "#D55E00", 
                                 "Standard error" = "#666666"),
                      breaks=c("High altitude ALS", "Low altitude ALS",
                               "UAV", 
                               "LAI2200", 
                               "Standard error")) +
  ...
  geom_text(data = label, aes(y = MeanTransmittance_LAI2200_mean,label = error), size = 5, show.legend = F) +
...
LAI2200_LiDAR_transect <- LAI2200_LiDAR %>%
  ...
  summarise(across(all_of(vars), # Variables à analyser
                   list(mean = ~mean(.), # les stats
                        ...
                        se = ~plotrix::std.error(.)),
                   .names = "{.col}_{.fn}")) 
LAI2200_LiDAR_ring <- cbind(LAI2200_LiDAR_ring, Transect = "Both transect")
LAI2200_LiDAR_transect <- rbind(LAI2200_LiDAR_transect, LAI2200_LiDAR_ring)
...
print(
  LAI2200_LiDAR_transect %>% 
    # filter(Transect==t) %>% # LAY # HOBO
    mutate(Transect = factor(Transect, levels = c("Both transect", "LAY", "HOBO"))) %>% 
    ggplot(aes(x = Ring)) +
    ...
    labs(y="Mean transmittance", x="Ring") +
    geom_text(data = label, aes(y = MeanTransmittance_LAI2200_mean,label = error, color = Acq), size = 6, show.legend = F) +
    facet_wrap(~ Transect, scales = "free_x",
               labeller = labeller(Transect = c("Both transect" = "Both transects",
                                                "HOBO" = "HOBO transect",
                                                "LAY" = "LAY transect"))) +
    
    theme(legend.position="bottom",legend.title= element_blank(), strip.text = element_text(size=18), axis.title= element_text(size=18), axis.text= element_text(size=18), legend.text= element_text(size=18))
)
...
# ggsave("P16_2023_4ha_Long_profile_allrings_facet_Intensity1m.png",
#        path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_1m/DZ_cor",
#        width = 30, height = 20, units = "cm", dpi=800, bg="white")
...
LAI2200_LiDAR %>% 
  ...
  aes(x = MeanTransmittance_LAI2200, y = Transmittance_Low) +
  geom_point(aes(y = Transmittance_LowInt2m, colour = "Low altitude ALS")) +
  geom_point(aes(y = Transmittance_HighInt2m, colour = "High altitude ALS")) +
  scale_colour_manual(values = c("Low altitude ALS" = "green", 
                                 "High altitude ALS" = "darkgreen")) +
  stat_smooth(method = "lm",
              formula = y ~ x) +
  geom_abline() +
  xlim(0,0.1) +
...
# Plot de la matrice de corrélation :
png("D:/Mes Donnees/PhD/Figures/Sensors/Correlation_plot_HOBOvsLAI2200_integrated_weighted.png", width = 1000, height = 743, pointsize=20)
