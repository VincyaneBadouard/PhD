...
HighAnnual_trans_1m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Intensity_1m/P16_2023_HighAlt_4ha_buffer_LAI2200sim_intensity1m_test.txt"))
```

# Fusion
```{r, include = F}
LiDAR <- HighAnnual_trans %>%
  left_join(LowAnnual_trans, by= c("position.x", "position.y", "position.z", "ring", "azimut"), suffix = c("_High2m", "_Low2m"))

LiDAR <- LiDAR %>%
  left_join(HighAnnual_trans_1m, by= c("position.x", "position.y", "position.z", "ring", "azimut")) %>% 
  rename(Transmittance_High1m = Transmittance) %>% 
  unique()

rm(HighAnnual_trans, LowAnnual_trans, HighAnnual_trans_1m)
```
...
LiDAR <- LiDAR %>% 
  ...
  mutate(MeanTransmittance_High2m = mean(Transmittance_High2m)) %>%
  mutate(SdTransmittance_High2m = sd(Transmittance_High2m)) %>% 
  mutate(SeTransmittance_High2m = plotrix::std.error(Transmittance_High2m)) %>%
  
  mutate(MeanTransmittance_Low2m = mean(Transmittance_Low2m)) %>%
  mutate(SdTransmittance_Low2m = sd(Transmittance_Low2m)) %>%
  mutate(SeTransmittance_Low2m = plotrix::std.error(Transmittance_Low2m)) %>%
  
  mutate(MeanTransmittance_High1m = mean(Transmittance_High1m)) %>%
...
for(acq in c("MeanTransmittance_High2m", "MeanTransmittance_Low2m","MeanTransmittance_High1m")){
  print(
    LiDARsf %>% 
...
# Difference plots
```{r}
# "MeanTransmittance_High2m", "MeanTransmittance_Low2m","MeanTransmittance_High1m"
# names(LAI2200_LiDAR)
LAI2200_LiDAR <- LAI2200_LiDAR %>% 
  # select(-c("...1.x","...1.y")) %>% 
  select(Sensor,Transect,Id, Xutm,Yutm, Ring,azimut,Transmittance_LAI2200, Transmittance_sd,
         MeanTransmittance_High2m, SdTransmittance_High2m, SeTransmittance_High2m,
         MeanTransmittance_Low2m, SdTransmittance_Low2m, SeTransmittance_Low2m,
  ...
  mutate(Transmittance_LowvsHigh_2m =  MeanTransmittance_Low2m - MeanTransmittance_High2m) %>% 
  mutate(Transmittance_High2mvsLAI2200 = MeanTransmittance_High2m -Transmittance_LAI2200) %>% 
  mutate(Transmittance_LowvsLAI2200 = MeanTransmittance_Low2m - Transmittance_LAI2200) %>% 
  mutate(Transmittance_High1mvsLAI2200 = MeanTransmittance_High1m -Transmittance_LAI2200) %>% 
  mutate(Transmittance_High1mvsHigh2m = MeanTransmittance_High1m -MeanTransmittance_High2m)

range(LAI2200_LiDAR$Transmittance_High2mvsLAI2200) # -0.54124286  0.06917472
range(LAI2200_LiDAR$Transmittance_LowvsLAI2200) # -0.49684541  0.06841737 : le plus proche du lAI2200
...
for(v in c("Transmittance_LowvsHigh_2m", "Transmittance_High2mvsLAI2200", "Transmittance_LowvsLAI2200", "Transmittance_High1mvsLAI2200", "Transmittance_High1mvsHigh2m")){
  print(
    ggplot() +
      geom_sf(data = sf::st_cast(zone, "LINESTRING")) + # zone of interest
...
```
```{r}
# Transmittance_LAI2200, "MeanTransmittance_High2m", "MeanTransmittance_Low2m","MeanTransmittance_High1m"
  ...
  for(R in 1:5){
    ...
    print(
      LAI2200_LiDAR %>% 
        ...
        filter(Ring==R) %>%
        ggplot(aes(x = Id)) +
        geom_point(aes(y = MeanTransmittance_High2m, colour = "High altitude - 2m")) +
        geom_line(aes(y = MeanTransmittance_High2m, colour = "High altitude - 2m")) +
        geom_errorbar(aes(ymin=MeanTransmittance_High2m-SeTransmittance_High2m,
                          ymax=MeanTransmittance_High2m+SeTransmittance_High2m, colour = "High altitude - 2m"), width=.2) +
        
        
        geom_point(aes(y = MeanTransmittance_Low2m, colour = "Low altitude - 2m")) +
        geom_line(aes(y = MeanTransmittance_Low2m, colour = "Low altitude - 2m")) +
        geom_errorbar(aes(ymin=MeanTransmittance_Low2m-SeTransmittance_Low2m,
                          ymax=MeanTransmittance_Low2m+SeTransmittance_Low2m, colour = "Low altitude - 2m"), width=.2) +
...
```{r}
# MeanTransmittance_LAI2200, "MeanTransmittance_High2m", "MeanTransmittance_Low2m","MeanTransmittance_High1m"
...
LAI2200_LiDAR_ring <- LAI2200_LiDAR %>% 
  ...
  mutate(MeanTransmittance_High2m_ring = mean(MeanTransmittance_High2m)) %>% 
  mutate(SeTransmittance_High2m_ring = plotrix::std.error(MeanTransmittance_High2m)) %>% 
  mutate(SdTransmittance_High2m_ring = sd(MeanTransmittance_High2m)) %>% 
  
  mutate(MeanTransmittance_Low2m_ring = mean(MeanTransmittance_Low2m)) %>%
  mutate(SeTransmittance_Low2m_ring = plotrix::std.error(MeanTransmittance_Low2m)) %>%
  mutate(SdTransmittance_Low2m_ring = sd(MeanTransmittance_Low2m)) %>%
...
LAI2200_LiDAR_ring %>% 
  ...
  geom_errorbar(aes(ymin=MeanTransmittance_LAI2200_ring-SeTransmittance_LAI2200_ring,
                    ymax=MeanTransmittance_LAI2200_ring+SeTransmittance_LAI2200_ring, colour = "LAI2200"),alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_Low2m_ring-SeTransmittance_Low2m_ring,
                    ymax=MeanTransmittance_Low2m_ring+SeTransmittance_Low2m_ring, colour = "Low altitude - 2m"),alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_High2m_ring-SeTransmittance_High2m_ring,
                    ymax=MeanTransmittance_High2m_ring+SeTransmittance_High2m_ring, colour = "High altitude - 2m"),alpha =1, width=.2) +
  geom_errorbar(aes(ymin=MeanTransmittance_High1m_ring-SeTransmittance_High1m_ring,
                    ymax=MeanTransmittance_High1m_ring+SeTransmittance_High1m_ring, colour = "High altitude - 1m"),alpha =1, width=.2) +
  
  geom_point(aes(y = MeanTransmittance_High2m_ring, colour = "High altitude - 2m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_High2m_ring, colour = "High altitude - 2m")) +
  
  geom_point(aes(y = MeanTransmittance_Low2m_ring, colour = "Low altitude - 2m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_Low2m_ring, colour = "Low altitude - 2m")) +
  
  geom_point(aes(y = MeanTransmittance_High1m_ring, colour = "High altitude - 1m"),size = 3) +
  geom_line(aes(y = MeanTransmittance_High1m_ring, colour = "High altitude - 1m")) +
...
ggsave(paste("Long_profile_The2_transects_MeanStandardErrorTransmittance_of_the_transect_byRing.png", sep=""),
...
for(t in c("HOBO", "LAY")){
  ...
  ggsave(paste("Long_profile_",t," transect","_MeanStandardErrorTransmittance_of_the_transect_byRing.png", sep=""),
         path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_1m",
         width = 25, height = 20, units = "cm", dpi=800, bg="white")
}
```
