...
## UAV
UAV_int2m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_2m/P16_2023_UAV_4ha_bufferALS_LAI2200sim_intensity2m_test.txt")) %>% mutate(Transmittance = as.numeric(Transmittance))
UAV_int1m <- unique(Read_LAI2200_Transfile("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_1m/P16_2023_UAV_4ha_bufferALS_LAI2200sim_intensity1m_test.txt")) %>% mutate(Transmittance = as.numeric(Transmittance))
...
```{r,eval = F }
test <- unique(LAI2200_LiDAR %>% filter(Ring==1) %>% select(Transect, Id, Xutm,Yutm, Transmittance_LAI2200, Transmittance_sd, MeanTransmittance_HighInt2m)) # 46 
...
```{r}
# Transmittance_LAI2200, "MeanTransmittance_HighInt2m", "MeanTransmittance_LowInt2m","MeanTransmittance_HighInt1m"
...
LAI2200_LiDAR <- LAI2200_LiDAR %>% 
  ...
  mutate(Transmittance_HighInt1mvsLAI2200 = MeanTransmittance_HighInt1m -Transmittance_LAI2200) %>% 
  ...
  mutate(Transmittance_UAVInt2mvsLAI2200 = MeanTransmittance_UAVInt2m - Transmittance_LAI2200) %>% 
  mutate(Transmittance_UAVInt1mvsLAI2200 = MeanTransmittance_UAVInt1m -Transmittance_LAI2200) %>% 
...
range(LAI2200_LiDAR$Transmittance_UAVInt2mvsLAI2200) # 
range(LAI2200_LiDAR$Transmittance_UAVInt1mvsLAI2200) # 
range(LAI2200_LiDAR$Transmittance_UAVInt1mvsUAV2m) # 
    ...
    print(
      LAI2200_LiDAR %>% 
        ...
        geom_errorbar(aes(ymin=MeanTransmittance_HighInt2m-SeTransmittance_HighInt2m,
                          ymax=MeanTransmittance_HighInt2m+SeTransmittance_HighInt2m, colour = "High altitude - Intens - 2m"), width=.2) +
        ...
        geom_errorbar(aes(ymin=MeanTransmittance_LowInt2m-SeTransmittance_LowInt2m,
                          ymax=MeanTransmittance_LowInt2m+SeTransmittance_LowInt2m, colour = "Low altitude - Intens - 2m"), width=.2) +
        ...
        geom_errorbar(aes(ymin=MeanTransmittance_HighInt1m-SeTransmittance_HighInt1m,
                          ymax=MeanTransmittance_HighInt1m+SeTransmittance_HighInt1m, colour = "High altitude - Intens - 1m"), width=.2) +
        ...
        geom_errorbar(aes(ymin=MeanTransmittance_UAVInt2m-SeTransmittance_UAVInt2m,
                          ymax=MeanTransmittance_UAVInt2m+SeTransmittance_UAVInt2m, colour = "UAV - Intens - 2m"), width=.2) +
        ...
        geom_errorbar(aes(ymin=MeanTransmittance_UAVInt1m-SeTransmittance_UAVInt1m,
                          ymax=MeanTransmittance_UAVInt1m+SeTransmittance_UAVInt1m, colour = "UAV - Intens - 1m"), width=.2) +
        ...
        # geom_errorbar(aes(ymin=MeanTransmittance_HighEqual2m-SeTransmittance_HighEqual2m,
        #                   ymax=MeanTransmittance_HighEqual2m+SeTransmittance_HighEqual2m, colour = "High altitude - Equal - 2m"), width=.2) +
        # 
        # 
        ...
        # geom_errorbar(aes(ymin=MeanTransmittance_LowEqual2m-SeTransmittance_LowEqual2m,
      #                   ymax=MeanTransmittance_LowEqual2m+SeTransmittance_LowEqual2m, colour = "Low altitude - Equal - 2m"), width=.2) +
...
LAI2200_LiDAR_ring %>% 
  ...
  geom_errorbar(aes(ymin=MeanTransmittance_HighInt2m_mean-MeanTransmittance_HighInt2m_se,
                    ymax=MeanTransmittance_HighInt2m_mean+MeanTransmittance_HighInt2m_se,
                    colour = "High altitude - Intens - 2m"),alpha =1, width=.2) +
  # 1m
  geom_errorbar(aes(ymin=MeanTransmittance_HighInt1m_mean-MeanTransmittance_HighInt1m_se,
                    ymax=MeanTransmittance_HighInt1m_mean+MeanTransmittance_HighInt1m_se,
                    colour = "High altitude - Intens - 1m"),alpha =1, width=.2) +
  
  # UAV
  geom_errorbar(aes(ymin=MeanTransmittance_UAVInt2m_mean-MeanTransmittance_UAVInt2m_se,
                    ymax=MeanTransmittance_UAVInt2m_mean+MeanTransmittance_UAVInt2m_se,
                    colour = "UAV - Intens - 2m"),alpha =1, width=.2) +
  
  geom_errorbar(aes(ymin=MeanTransmittance_UAVInt1m_mean-MeanTransmittance_UAVInt1m_se,
                    ymax=MeanTransmittance_UAVInt1m_mean+MeanTransmittance_UAVInt1m_se,
                    colour = "UAV - Intens - 1m"),alpha =1, width=.2) +
  # # Equal echo
  # geom_errorbar(aes(ymin=MeanTransmittance_LowEqual2m_mean-MeanTransmittance_LowEqual2m_se,
  #                   ymax=MeanTransmittance_LowEqual2m_mean+MeanTransmittance_LowEqual2m_se,
  #                   colour = "Low altitude - Equal - 2m"),alpha =1, width=.2) +
  # geom_errorbar(aes(ymin=MeanTransmittance_HighEqual2m_mean-MeanTransmittance_HighEqual2m_se,
...
for(t in c("HOBO", "LAY")){
  ...
  # ggsave(paste("Long_profile_",t," transect","_MeanStandardErrorTransmittance_of_the_transect_byRing_Intensity2m.png", sep=""),
  #        path = "D:/Mes Donnees/PhD/Figures/lidar/Correlations/Long_profile/Intensity_2m",
  #        width = 25, height = 20, units = "cm", dpi=800, bg="white")
}
```
...
# png("D:/Mes Donnees/PhD/Figures/lidar/Correlations/Correlations/Intensity_1m/Correlation_plot_LiDAR_Acquisitions.png",
#      width = 1000, height = 743, pointsize=20)

# corrplot(CorMatrix, method="number", type="lower", diag = F,
#         tl.col="black", tl.cex = 0.7, tl.srt=45,# labels
#         number.cex = 0.5, # values
#         col=brewer.pal(n=8, name="PuOr"),order="hclust") # palette
...
# Plot de la matrice de corrélation :
# png("D:/Mes Donnees/PhD/Figures/lidar/Correlations/Correlations/Intensity_1m/Correlation_plot_LiDAR_vs_LAI2200_sig.png",
#     width = 1000, height = 743, pointsize=20)
...
scale_colour_steps2(low="#00AFBB", high="#FC4E07", mid = "white", midpoint = 0,
...
onlySignCorr(DF_cor_LiDAR, "LiDAR_Acquisitions")
...
HOBO %>% 
  ...
  geom_errorbar(aes(ymin=HOBO_Light_annual_mean-HOBO_Light_annual_se,
                    ymax=HOBO_Light_annual_mean+HOBO_Light_annual_se, colour = "Annual"), width=.2) +
  
  geom_point(aes(y = HOBO_Light_october_mean, colour = "October")) +
  geom_line(aes(y = HOBO_Light_october_mean, colour = "October")) +
  geom_errorbar(aes(ymin=HOBO_Light_october_mean-HOBO_Light_october_se,
                    ymax=HOBO_Light_october_mean+HOBO_Light_october_se, colour = "October"), width=.2) +
  
  geom_point(aes(y = HOBO_Light_mean_Dry, colour = "Dry season")) +
  geom_line(aes(y = HOBO_Light_mean_Dry, colour = "Dry season")) +
  geom_errorbar(aes(ymin=HOBO_Light_mean_Dry-HOBO_Light_se_Dry,
                    ymax=HOBO_Light_mean_Dry+HOBO_Light_se_Dry, colour = "Dry season"), width=.2) +
  
  geom_point(aes(y = HOBO_Light_mean_Wet, colour = "Wet season")) +
  geom_line(aes(y = HOBO_Light_mean_Wet, colour = "Wet season")) +
  geom_errorbar(aes(ymin=HOBO_Light_mean_Wet-HOBO_Light_se_Wet,
                    ymax=HOBO_Light_mean_Wet+HOBO_Light_se_Wet, colour = "Wet season"), width=.2) +
  
  
  scale_colour_manual(values = c("Wet season" = "blue",
                                 "Annual" = "#CC79A7",
                                 "Dry season" = "#999999",
                                 "October" = "#D55E00"),
                      breaks=c("Annual",
  ...
  labs(title = "HOBO light", y="Light mean (lux)", x="ID", color = "Period") +
  scale_x_continuous(breaks = seq(1:idmax)) + # 26 # 20
  theme(title= element_text(size=18), axis.title= element_text(size=18), axis.text= element_text(size=18), legend.title= element_text(size=18), legend.text= element_text(size=18)) +
  theme_minimal()
...
DF_cor <- HOBO_LAI2200_LiDAR %>% 
  # filter(Ring==1) %>%
  select(Ring, Transmittance_HighInt2m,Transmittance_LowInt2m, Transmittance_HighInt1m,
         Transmittance_LAI2200,
         HOBO_Light_october_sum, HOBO_Light_annual_sum) %>% 
  unique() %>% 
  pivot_wider(names_from = Ring,
              values_from = c(Transmittance_LAI2200,
