...
data %>% 
  select(Zutm_disc, MeanTransmittance, MinTransmittance, MaxTransmittance, SdTransmittance) %>% 
  # filter(Zutm_disc>=0 & Zutm_disc<=60) %>% 
  unique() %>% 
  ggplot(aes(x= Zutm_disc, y= MeanTransmittance )) +
  geom_point() + 
  geom_line() +
  ggtitle("P16 - 2023 - 4ha - LowAlt - Light profile (Grid 10x10m, each 1m in Z)") +
  labs(y="Transmittance", x="Altitude") +
  xlim(0,70) +
  coord_flip() + # coordonnées inversées au départ pour que la ligne suive la hauteur au sol et non le Transmittance
  geom_errorbar(aes(ymin=MinTransmittance, ymax=MaxTransmittance), width=.2)
