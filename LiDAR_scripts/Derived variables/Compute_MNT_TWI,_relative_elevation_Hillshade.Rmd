...
dem="test/Paracou_MNT_cor.tif" # dans le meme dossier que cd
twi="test/Paracou_TWI.srgd" # dans le meme dossier que cd
...
translate in .tif
```{r, eval=FALSE}
TWI <- terra::rast("test/Paracou_TWI.sdat") %>%  # sdat données du fichier TWI au format SAGA 
  terra::aggregate(5) 
...
terra::rast("test/Paracou_MNT_cor.tif") %>% 
  terra::plot()
terra::rast("test/Paracou_TWI.tif") %>% 
  terra::plot()
```
```{r}
ggplot() + 
  geom_spatraster(data = TWI, aes(fill = Paracou_TWI)) +
...
# TWI
ggplot() + 
  geom_spatraster(data = P16_TWI, aes(fill = Paracou_TWI)) +
  scale_fill_gradientn(name = "TWI",
