...
class(ALT$geometry) # sfc_POLYGON
car <- st_cast(st_union(ALT), "POLYGON") # unified the polylons in 1 polygon
car <- st_set_crs(car, 2972) # crs
coord <- st_coordinates(car)[c(7,4,1,10),][,1:2] # retrieve corners coordinates
...
plot(coord)
text(coord[1:4,1], coord[1:4,2],labels = rownames(coord[1:4,])) # ya pas erreur là ? le nord est dit à gauche, l'ouest en haut
```
## New rotation center 
```{r}
center <- c(mean(coord[,1]),mean(coord[,2])) # moyenne des x et moyenne des y
```

## Add tag point
```{r}
NorthCenter <- c(mean(coord[c(1,2),1]),mean(coord[c(1,2),2])) # moyenne entre les 2 points au Nord
points <- rbind(center, NorthCenter) # center and center of the north
```
...
Dans le plan cartésien, une matrice de transformation est une matrice qui permet, à partir des *coordonnées d’un point initial* (ici le centre) représentées par une *matrice colonne*, de trouver celles de son image par une transformation géométrique donnée. Les coordonnées de l’image sont alors obtenues en effectuant la multiplication de la matrice colonne (les coordonnées d’un point) par la *matrice* correspondant à la transformation géométrique concernée.
c'est ça ?
...
# matrice d'1 ligne avec les coordonnées x,y,z du centre
Trans <- matrix(data = c(center[1], center[2], 0), 
                nrow = 1,
                dimnames  =  list(c(),c("x","y","z")))

Tmat <- id  # matrice identitée
Tmat[1:3,4] <- -as.numeric(Trans) # coord inversées du centre en colonne 4, après matrice identité
```

## Rotation by pi/2 -angle anti-clockwise
```{r}
vec <- coord[2,] - coord[1,] # NE - NW
rho <- atan(vec[1]/vec[2]) # X/Y (angle)

# Matrice de rotation (3 lignes, 3 colonnes) une matrice de rotation est une matrice orthogonale de déterminant 1
mat_rot <- matrix(data =
...
# VOP (matrice de transformation) = matrice de rotation (R) * matrice colonne du point ? (Tmat)
...
datbis <- dat %*% t(Tr) # %*% multiple 2 matrices. t() : Matrix Transpose (Tr = VOP)
plot(datbis[, c(1,2)]) # droit
