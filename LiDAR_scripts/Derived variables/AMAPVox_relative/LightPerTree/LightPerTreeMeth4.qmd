...
range(datalocal$Z) # 9.68270 72.69894

datalocal[Z>max(Light$Z),]

datalocal <- datalocal[Z<max(Light$Z),] # remove trees too big
...
result <- RANN::nn2(data = Light[, .(X, Y, Z)], # Pool to search
                    ...
                    k = 30, # including target # 5, 8, 9, 15, 27, 30, 35, 35
                    searchtype = "radius", radius = 3)
...
# Recompute attenuation from local neighborhood :
...
```{r, eval=F}
range(datalocal$Transmittance) # 0.0006329673 1.0000000894
hist(datalocal$Transmittance)
datalocal[Transmittance<0.001,] # 18 trees
range(datalocal[Transmittance<0.001, Z]) # 15.24241 24.72391
(nrow(datalocal[Transmittance<0.001,])/nrow(datalocal))*100 # 0.04%

toodark <- datalocal[Transmittance<0.001,]
length(unique(toodark$ScientificName)) # 16
