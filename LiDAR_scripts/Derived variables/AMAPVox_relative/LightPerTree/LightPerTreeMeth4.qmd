...
result <- RANN::nn2(data = Light[, .(X, Y, Z)], # Pool to search
                    ...
                    searchtype = "radius", radius = 4) # pour éviter les NA en bord de scene 
# k = 30, radius = 3
# k = 7, radius = 5
...
ggplot(datalocal, aes(x=TreeHeight, y=Transmittance)) + # , col=Strategy
  theme_minimal() +
  ggpubr::stat_cor(label.x = 0.01, label.y = 0.2) +
  geom_point(size=0.3) +
  scale_y_continuous("Transmittance %", trans="log", n.breaks = 10, labels = ~round((.)*100,2)) + # log
  labs(x = 'Tree height (m)')

ggsave("Transmittance_vs_TreeHeight_AllP16.png",
...
# Compare light before and after coordinates correction
```{r eval=F}
...
# datalocal <- read.csv("~/PhD/Inventories/Data/Adults_Understory/Paracou/AllP16_Light_2019_-1m.csv")
```
...
# Light range under 20m height

## Coordinates in UTM (use VOP)
```{r}
source("~/PhD/R_codes/PhD/LiDAR_scripts/Functions/Relative2UTM.R")
VOP <- as.matrix(read.table("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/VOP/VOP_P16_25ha.txt"))

Light <- Relativ2UTM(data = Light,VOP) %>% 
  select(-Zutm)
```
...
data <- Light %>% cbind(alt) %>% setDT()
```

```{r}
data <- data[Z > Zutm, ] # only Z above the ground

options(scipen=999)
quantile(data[Z<20, Transmittance*100],  probs = c(0, .05, .5, .95, .99, 1))
# 0%            5%    50%    95%   99%    100% 
# 0.0002      0.13   1.09  19.31 45.73   92.82 
```
