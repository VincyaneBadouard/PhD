...
# path_script <- '/lustre/badouardv/lightpertree/ByTree_scripts/'

# local 
path_input <- '//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/ByTree_Input/'
path_output <- '//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/ByTree_Output_local/'
...
# Data -------------------------------------------------------------------------
## Trees
Trees <- (read.csv(paste(path_input,"AllP16_filtred.csv",sep='')) %>% # 28 049 rows
...
# For each tree ----------------------------------------------------------------
# Parallelisation
plan(multisession) # for local
options(future.globals.maxSize= 10e100)
# for cluster
# workers <- c("129.20.25.61", "129.20.25.217")
# cl <- makeClusterPSOCK(workers, revtunnel = TRUE, outfile = "") # cluster
# print(cl) # socket cluster with 2 nodes on hosts '129.20.25.61', '129.20.25.217'
# plan(cluster, workers = cl)

for(ID in 1:3){ # nrow(Trees)
  ...
  future({
    ...
    writeVoxelSpace(VX_crop, 
                    paste(path_output,"Tree_Vox/P16_2023_HighAlt_4ha_buffer_intensity1m_",idtree,".vox", sep='')
    )
    rm(VX_crop)
    ...
    # Input voxelspace
    VXin <- paste(path_output,"Tree_Vox/P16_2023_HighAlt_4ha_buffer_intensity1m_",idtree,".vox", sep='')
