...
# ALS intensity 1m res
vxsp_High <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Intensity_1m/P16_2023_HighAlt_4ha_buffer_intensity1m.vox")
vxsp_Low <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/LowAltitudeFlight/Intensity_1m/P16_2023_LowAlt_4ha_buffer_intensity1m.vox")
# UAV intensity 1m & 2m res
vxsp_UAV <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_1m/P16_2023_UAV_4ha_buffer_intensity1m_crop.vox")
# vxsp_UAV2m <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_2m/P16_2023_UAV_4ha_buffer_intensity2m.vox")

# All only on the 4ha
source("~/PhD/R_codes/PhD/LiDAR_scripts/Functions/Crop_vox_with_shapefile.R")
vxsp_High <- Crop_vox_with_shapefile(vxsp_High, zone, VOP)
vxsp_Low <- Crop_vox_with_shapefile(vxsp_Low, zone, VOP)
vxsp_UAV <- Crop_vox_with_shapefile(vxsp_UAV, zone, VOP)
# AMAPVox::plot(vxsp_UAV)

median(vxsp_UAV1m@data$nbSampling)
median(vxsp_UAV2m@data$nbSampling)

hist(vxsp_UAV1m@data$nbSampling, breaks=100)
hist(vxsp_UAV2m@data$nbSampling, breaks=100)

sum(vxsp_UAV1m@data$nbSampling)
sum(vxsp_UAV2m@data$nbSampling)
...
fullcanopy_UAV <- OnlyVegetation(vxsp_UAV)
...
fbv_High <-  pulsemin_fbv_High$fbv # 0.0832371
# pulsemin_fbv_Low <- pulsemin_fbv(fullcanopy_Low)
# pulsemin_Low <- pulsemin_fbv_Low$pulse.min
# fbv_Low <- pulsemin_fbv_Low$fbv # 0.07884395

pulsemin_fbv_UAV1m <- pulsemin_fbv(fullcanopy_UAV1m)
pulsemin_UAV1m <- pulsemin_fbv_UAV1m$pulse.min
fbv_UAV1m <-  pulsemin_fbv_UAV1m$fbv #

pulsemin_fbv_UAV2m <- pulsemin_fbv(fullcanopy_UAV2m)
pulsemin_UAV2m <- pulsemin_fbv_UAV2m$pulse.min
fbv_UAV2m <-  pulsemin_fbv_UAV2m$fbv #
...
```{r}
Poorly_smpld_vxl <- function(fullcanopy, path, thres =25){
...
Poorly_smpld_vxl(fullcanopy_High, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Intensity_1m/Poorly25_smpld_vxl_Proportion2023HighAlt1m.csv")

Poorly_smpld_vxl(fullcanopy_UAV1m,
                 "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_1m/Poorly25_smpld_vxl_Proportion2023UAV1m.csv")
Poorly_smpld_vxl(fullcanopy_UAV2m,
                 "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_2m/Poorly25_smpld_vxl_Proportion2023UAV2m.csv")
...
dataHigh_1 <- Poorly_sampled_voxels_per_altitude(fullcanopy_High, 1) %>% mutate(threshold=1)
dataHigh_10 <- Poorly_sampled_voxels_per_altitude(fullcanopy_High, 10) %>% mutate(threshold=10)
dataHigh_20 <- Poorly_sampled_voxels_per_altitude(fullcanopy_High, 20) %>% mutate(threshold=20)
dataHigh <- rbind(dataHigh_1, dataHigh_10, dataHigh_20)
rm(dataHigh_1, dataHigh_10, dataHigh_20, vxsp_High, vxsp_Low, vxsp_UAV)

dataLow_1 <- Poorly_sampled_voxels_per_altitude(fullcanopy_Low, 1) %>% mutate(threshold=1)
dataLow_10 <- Poorly_sampled_voxels_per_altitude(fullcanopy_Low, 10) %>% mutate(threshold=10)
dataLow_20 <- Poorly_sampled_voxels_per_altitude(fullcanopy_Low, 20) %>% mutate(threshold=20)
dataLow <- rbind(dataLow_1, dataLow_10, dataLow_20)
rm(dataLow_1, dataLow_10, dataLow_20)

dataUAV_1 <- Poorly_sampled_voxels_per_altitude(fullcanopy_UAV, 1) %>% mutate(threshold=1)
dataUAV_10 <- Poorly_sampled_voxels_per_altitude(fullcanopy_UAV, 10) %>% mutate(threshold=10)
dataUAV_20 <- Poorly_sampled_voxels_per_altitude(fullcanopy_UAV, 20) %>% mutate(threshold=20)
dataUAV <- rbind(dataUAV_1, dataUAV_10, dataUAV_20)
rm(dataUAV_1, dataUAV_10, dataUAV_20)
...
ggplot(dataLow, aes(x= ground_distance_disc, y= `%_empty_voxels`, group = as.factor(threshold))) +
  geom_point(data = dataHigh, aes(colour = "High", shape= as.factor(threshold), alpha=as.factor(threshold))) +
  geom_line(data = dataHigh, aes(colour = "High")) +
  geom_point(data = dataLow, aes(colour = "Low", shape= as.factor(threshold), alpha=as.factor(threshold))) +
  geom_line(data = dataLow, aes(colour = "Low")) +
  geom_point(data = dataUAV, aes(colour = "UAV", shape= as.factor(threshold), alpha=as.factor(threshold))) +
  geom_line(data = dataUAV, aes(colour = "UAV")) +
  
  scale_colour_manual(values = c("High" = "blue",
                                 "Low" = "#56B4E9", 
                                 "UAV" = "#E69F00")) +
  theme_minimal() +
  ggtitle("P16-2023-4ha - Proportion of poorly sampled voxels per altitude - res 1m") +
  labs(y="% of poorly sampled voxels", x="Canopy height (m)", color = "LiDAR acquisition", shape= "Sampling number threshold", alpha= "Sampling number threshold") +
  coord_flip() +
  xlim(1,45)
# scale_x_continuous(breaks = seq(0,160615, by= 25000))

ggsave("P16_2023_4ha_intensity1m_res_Poorly_sampled_voxels_nbsampling_per_altitude.png",
       path = "D:/Mes Donnees/PhD/Figures/lidar/Voxélisation/Empty_voxels_profil",
       width = 20, height = 20, units = "cm", dpi=800, bg="white")
# "P16_2023_4ha_buffer_HighvsLow_Poorly_sampled_voxels_20sampling_per_altitude_intensity2m.png"
```
...
```{r}
gc()
fullcanopy_High <- HLE(fullcanopy_High, # voxel space (only vegetation)
...
# fullcanopy_Low <- HLE(fullcanopy_Low, # voxel space (only vegetation)
#                       variable.name1 = "bsIntercepted",
#                       variable.name2 = "weightedFreepathLength",
#                       variable.min = -Inf, variable.max = Inf, 
...
fullcanopy_UAV1m <- HLE(fullcanopy_UAV1m, # voxel space (only vegetation)
                        variable.name1 = "bsIntercepted",
                        variable.name2 = "weightedFreepathLength",
                        variable.min = -Inf, variable.max = Inf, 
                        variable.fallback = fbv_UAV1m, radius = 2, pulse.min = pulsemin_UAV1m)

fullcanopy_UAV2m <- HLE(fullcanopy_UAV1m, # voxel space (only vegetation)
                        variable.name1 = "bsIntercepted",
                        variable.name2 = "weightedFreepathLength",
                        variable.min = -Inf, variable.max = Inf, 
                        variable.fallback = fbv_UAV2m, radius = 2, pulse.min = pulsemin_UAV2m)

gc()

"HLE" %in% names(fullcanopy_High@data);"HLE" %in% names(fullcanopy_Low@data)
range(fullcanopy_High$HLE);range(fullcanopy_Low$HLE)

sum(is.na(fullcanopy_High@data$HLE)) ; sum(is.na(fullcanopy_Low@data$HLE)) 
sum(!is.na(fullcanopy_High@data$HLE)) ; sum(!is.na(fullcanopy_Low@data$HLE))  # 4887658
...
hist(fullcanopy_High$HLE);hist(fullcanopy_Low$HLE)
```
...
gc()
range(vxsp_High@data$PadBVTotal);range(vxsp_Low@data$PadBVTotal)
any(is.na(vxsp_High@data$PadBVTotal))
```
...
Compute_Transmittance <- function(vxsp){
  vxsp@data$Transmittance <- exp(-(vxsp_High@data$HLE))
...
range(vxsp_High@data$Transmittance);range(vxsp_Low@data$Transmittance)
...
writeVoxelSpace(vxsp_High, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/Intensity_2m/P16_2023_4ha_buffer_HighAlt_PadHLE_intensity2m.vox")
writeVoxelSpace(vxsp_Low, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/LowAltitudeFlight/Intensity_2m/P16_2023_4ha_buffer_LowAlt_PadHLE_intensity2m.vox")
