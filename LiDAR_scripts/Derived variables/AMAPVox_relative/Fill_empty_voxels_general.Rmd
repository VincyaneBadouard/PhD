...
HLE <- function (
    ...
    variable.min = 0, variable.max = 2, # bornes pas forcément utiles
  ...
  # sum(neighbors==0)/length(neighbors)*100 # 75% missing neighbors
  ...
  # sum(is.na(vxsp@data$bsIntercepted))
  # sum(is.na(vxsp@data$weightedFreepathLength))
  range(na.omit(fill.value))
  # sum(is.nan(fill.value))/length(fill.value)*100 # 91% of NaN
  # sum(na.omit(fill.value==0))/length(fill.value)*100 # 2% of 0
  # sum(na.omit(fill.value>2))/length(fill.value)*100 # 0.006% HLE >2
  ...
  na.count <- length(which(is.na(fill.value))) # 0 NA
  ...
  # range(na.omit(vxsp@data$HLE))
  # ggplot(vxsp@data, aes(x = HLE)) + geom_histogram() + scale_x_continuous(trans="log") +  ggtitle("Before fill empty voxel")


  vxsp@data[neighbors[,1], HLE := fill.value] # modified attenuation. [,1] for the interest voxel
  # sum(vxsp@data$HLE>2) # 42394
  
  # Apply the thresholds
  vxsp@data[HLE>variable.max, HLE := variable.max]
  vxsp@data[HLE<variable.min, HLE := variable.min]
  
  # ggplot(vxsp@data, aes(x = HLE))  + geom_histogram() + scale_x_continuous(trans="log") + ggtitle("After fill empty voxel")

  
  na.count <- length(which(is.nan(vxsp@data$HLE))) # 0
...
vxsp <- readVoxelSpace(
  ...
  # laz pour 25 ha high alt intensity
   # "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/ForTrees/Vox/P16C19_2023_1ha_HighAlt_intensity2m_testnewEB.vox"
  ...
  # ALS 2019
  # "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2019/Vox/P16_2019_25ha_buffer_intensity2m.vox"
...
fullcanopy <- HLE(fullcanopy, # voxel space (only vegetation)
                  ...
                  variable.min = 0, variable.max = 1.5, 
...
# ALS 2019
  "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2019/Vox/P16_2019_25ha_buffer_PadHLE_intensity2m.vox"
