...
median(vxsp@data$nbSampling) # High Alt : 193 # UAV2m : 1264, UAV1m : 275
...
data %>% 
  filter(ground_distance_disc < 11) %>%
  ggplot(aes(x= ground_distance_disc, y= MeanNbsampling)) +
  # xlim(0,55) +
  geom_point(aes(x = ground_distance_disc, y = MeanNbsampling, colour = "UAV - 2m")) +
  geom_line(aes(x = ground_distance_disc, y = MeanNbsampling, colour = "UAV - 2m")) +
  
  scale_colour_manual(values = c("UAV - 2m" = "#E69F00")) + # "#F0E442" "#E69F00"
  theme_minimal() +
  ggtitle("P16 - 2023 - 4ha + buffer - extinction profile (intensity) res 2m") +
  ...
  scale_x_continuous(breaks = seq(0, max(10), by=2)) +
  coord_flip() #+
...
data %>% 
  ...
  geom_point(aes(colour = "UAV - 1m")) +
  geom_line(aes(colour = "UAV - 1m")) +
  # geom_point(data = data2, aes(colour = acq2)) +
  # geom_line(data = data2, aes(colour = acq2)) +
  scale_colour_manual(values = c("UAV - 1m" = "#E69F00" #,
                                 # acq2 = "#56B4E9"
  )) +
  theme_minimal() +
  ...
  # xlim(0,45) +
  scale_x_continuous(breaks = seq(0, max(data$ground_distance_disc), by=2))+
  # scale_x_continuous(breaks = seq(0,160615, by= 25000))
  coord_flip()

ggsave("P16_2023_4ha_buffer_UAV_1m_res_Poorly_sampled_voxels_20sampling_per_altitude_intensity.png",
