...
Certains voxels (en bas de canopée) ne sont pas ou mal échantillonnés (seuil) du fait de l’occlusion des tirs LiDAR. Ces données manquantes ont été comblées en appliquant la **moyenne des valeurs de transmittance des 26 voisins les plus proches**.
...
2) Compute a reasonnable fallback value (the median of attenuation) if local re-estimation fails

3) Re-estimation of poorly sampled voxels
- subset of voxels below sampling threshold level
- but all voxels can contribute to local estimate
- Get neighbors in immediate neighborhood (27 nearest voxels including target = 3 neighbors)
- Recompute attenuation from local neighbourhood : sum(bsIntercepted)/sum(weightedFreepathLength)
...
# Read voxelSpace
```{r}
vxsp <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/P16_2022_4ha_buffer.vox")
vxsp
summary(vxsp@data)
```
...
On utilise l'atténuation biaisée pcq la débiaisée ne peut pas être utilisée à multiple échelle.

Biais d'échantillonage : quand peu échantilloné a tendence à renvoyer une grande densité

```{r}
fbv <- median(fullcanopy@data[nbSampling>=25, attenuation_FPL_biasedMLE]) # 0.0257098
```
...
```{r}
pulse.min = 25
na <- nrow(fullcanopy@data[nbSampling < pulse.min])/nrow(fullcanopy@data) # proportion of voxels below sampling threshold level (à garder pour comparer entre les différentes méthodes d'acquisition lidar)
write.csv(na, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/EmptyVoxelsProportion.csv")
...
# Create PadBVTotal
ref : VINCENT, Gregoire; PIMONT, François; VERLEY, Philippe, 2021, "A note on PAD/LAD estimators implemented in AMAPVox 1.7", doi:10.23708/1AJNMP , DataSuds, V1

Computes Plant Area Density either from (transmittance or) attenuation coefficient estimates.
PAD is defind as the plant area per unit volume (PAD plant area / voxel volume = m^2 / m^3).
Plant area density (PAD in m2·m−3)
...
writeVoxelSpace(vxsp,"//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/P16_2022_4ha_buffer_PadHLE.vox")
```

# Plot vertical profile
```{r}
vxsp <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2022/P16_2022_4ha_buffer_PadHLE.vox")
...
ggplot(data = vxsp@data, aes(x=PadBVTotal, y=ground_distance)) +
  geom_path() + geom_point()

```
# Compute Transmittance from attenuation
T = I/Io, where I = transmitted light (“output”) and Io = incident light (“input”)
Transmission=1-Attenuation ?

(l'attenuation n'est pas censé être >0, elle l'est dues aux statistiques, mais on ne l'ajuste pas à 0 car en moyenne elle est débiaisée)

```{r}
# "attenuation_FPL_biasedMLE" "attenuation_FPL_biasCorrection" "attenuation_FPL_unbiasedMLE" "attenuation_PPL_MLE" 
range(na.omit(vxsp@data$attenuation_FPL_unbiasedMLE)) ; range(na.omit(vxsp@data$attenuation_PPL_MLE))
# -17.28929 1657.19849 ;  0 20

vxsp@data[,Transmittance_FPL := 1-attenuation_FPL_unbiasedMLE] # -1656.19849 18.28929"
vxsp@data[,Transmittance_PPL := 1-attenuation_PPL_MLE] # -19 1
range(na.omit(vxsp@data$Transmittance_FPL)) ; range(na.omit(vxsp@data$Transmittance_PPL))
