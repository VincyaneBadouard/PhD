...
vxsp_UAV1m <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/HovermapUAV2023/AMAPVox/Intensity_1m/P16_2023_UAV_4ha_buffer_intensity1m.vox")
...
dataUAV1m <- Poorly_sampled_voxels_per_altitude(fullcanopy_UAV1m)
...
ggplot(dataLow, aes(x= ground_distance_disc, y= `%_empty_voxels`)) +
  geom_point(data = data1, aes(colour = acq1)) +
  geom_line(data = data1, aes(colour = acq1)) +
  geom_point(data = data2, aes(colour = acq2)) +
  geom_line(data = data2, aes(colour = acq2)) +
  scale_colour_manual(values = c(acq1 = "blue",
                                 acq2 = "#56B4E9")) +
  theme_minimal() +
  ggtitle("P16-2023-4ha+buffer - Proportion of poorly sampled voxels per altitude - res 1m") +
  labs(y="% of poorly sampled voxels (<25 pulses)", x="Ground distance", color = "LiDAR acquisition") +
  coord_flip() +
  xlim(0,45)
# scale_x_continuous(breaks = seq(0,160615, by= 25000))

ggsave("P16_2023_4ha_buffer_UAV_1vs2m_res_Poorly_sampled_voxels_25sampling_per_altitude_intensity.png",
