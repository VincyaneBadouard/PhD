...
-   now :
median([fullcanopy\@data](mailto:fullcanopy@data){.email}\$nbSampling)
...
median(vxsp_High@data$nbSampling)
median(vxsp_HighI@data$nbSampling)

hist(vxsp_High@data$nbSampling, breaks=100)
hist(vxsp_HighI@data$nbSampling, breaks=100)

sum(vxsp_High@data$nbSampling)
sum(vxsp_HighI@data$nbSampling)
...
```{r}
Poorly_smpld_vxl <- function(fullcanopy, path){
...
ggsave("P16_2023_4ha_buffer_HighvsLow_Poorly_sampled_voxels_20sampling_per_altitude_intensity2m.png",
...
# Compute Transmittance from attenuation (pas besoin, ampavox le calcul)
