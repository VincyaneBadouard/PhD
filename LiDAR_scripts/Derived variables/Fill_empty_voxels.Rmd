...
```{r}
vxsp_High <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/P16_2023_HighAlt_4ha_buffer.vox")
vxsp_Low <- readVoxelSpace("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/lowAltitudeFlight/P16_2023_LowAlt_4ha_buffer.vox")
...
pulse.min = 44 # seq(4,28, by=8) =  4 12 20 28 44
...
```{r}
Poorly_sampled_voxels_per_altitude <- function(fullcanopy){
  data <- fullcanopy@data 
  data <- data %>% 
    mutate(ground_distance_disc = as.integer(ground_distance)) %>% 
    group_by(ground_distance_disc) %>% 
    mutate(Poor = ifelse(nbSampling<25, 1, 0)) %>%
    summarize(N_poorly_sampled_voxels = sum(Poor)) %>% 
    filter(ground_distance_disc>=0 & ground_distance_disc<=50)
}

dataHigh <- Poorly_sampled_voxels_per_altitude(fullcanopy_High)
dataLow <- Poorly_sampled_voxels_per_altitude(fullcanopy_Low)

ggplot(dataLow, aes(x= ground_distance_disc, y= N_poorly_sampled_voxels)) +
  ...
  scale_colour_manual(values = c("High altitude" = "darkgreen",
                                 "Low altitude" = "green")) +
  ggtitle("P16 - 2023 - 4ha + buffer - Poorly sampled voxels per altitude") +
  labs(y="Number of poorly sampled voxels", x="Ground distance", color = "LiDAR acquisition") +
  coord_flip() +
  scale_y_continuous(breaks = seq(0,160615, by= 25000))
```
