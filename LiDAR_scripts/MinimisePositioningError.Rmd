...
# names(LAI2200_coord)
LAI2200_coord <- LAI2200_coord %>% select(x,y)
nrow(LAI2200_coord) # 46
```

# Generate a grid of 0.5 res and 2m max
```{r}
est <- data.frame(outer(LAI2200_coord$x, seq(0, 2, by=0.5), FUN = '+'),
                     y=LAI2200_coord$y)
ouest <- data.frame(outer(LAI2200_coord$x, seq(0, 2, by=0.5), FUN = '-'),
                     y=LAI2200_coord$y)

nord <- data.frame(outer(LAI2200_coord$y, seq(0, 2, by=0.5), FUN = '+'),
                     x=LAI2200_coord$x)
sud <- data.frame(outer(LAI2200_coord$y, seq(0, 2, by=0.5), FUN = '-'),
                     x=LAI2200_coord$x)
...
y <- pivot_longer(y,cols=X1:X5, names_to=NULL, values_to="y")
...
# ggsave("Paracou_P16_LAI2200_grid_simulation__UTM_positions.png",
...
```{r}
write.csv(LAI2200UTMxyz, "//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/Coordinates/LAI2200_gridsimulation_UTMxyz.csv")
```
