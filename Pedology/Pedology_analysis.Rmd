...
DF_CH_cor <- DF_cor %>% select(- c(`N° Labo`, `Sample ID`, TreeNum1, Sample, Site, Horizon, Texture_Jamagne))
...
DF_P_CH_cor <- DF_cor %>% 
  ...
  select(- c(`N° Labo`, `Sample ID`, TreeNum1, Sample, Site, Horizon, Texture_Jamagne))
...
# Table de granulo et densité apparente pour calibrer les Tomst
...
Granulo_data <- read_csv("~/PhD/Pédologie/Data/Labo_data_binded.csv") ; names(Granulo_data)
Bulk_density <- read_csv("~/PhD/Pédologie/Data/Raw/Bulk_density.csv") ; names(Bulk_density)
# "Site", "Sample ID", "Topography type", "Bulk density"    

GetPedodata4Tomst <- function(site){
  Granulo_data <- Granulo_data %>% 
    select(Site, Topography, `Sample ID`,
           `Argiles %`, `Limons fins %`,`Limons grossiers %`,
           `Sables fins %`, `Sables grossiers %`) %>% 
    filter(Site==site) %>% 
    rename(SampleID = `Sample ID`) %>% 
    rename(Argiles = `Argiles %`) %>% 
    rowwise() %>% 
    mutate(Limons = `Limons fins %`+`Limons grossiers %`) %>% 
    mutate(Sables = `Sables fins %`+`Sables grossiers %`) %>% 
    ungroup() %>% 
    mutate(ID = data.table::tstrsplit(SampleID, split = "-")[[4]]) %>% 
    select(Site, Topography, SampleID, ID, Argiles, Limons, Sables)
  
  Granulo_data <- Granulo_data %>% filter(grepl(pattern = 'CH', Granulo_data$SampleID)) 
  
  Pedo_data <- Bulk_density %>%
    filter(Site==site) %>% 
    select(Site, `Sample ID`, `Topography type`, `Bulk density`) %>% 
    rename(SampleID = `Sample ID`) %>% 
    rename(Topography = `Topography type`) %>% 
    rename(BulkDensity = `Bulk density`) %>% 
    mutate(ID = data.table::tstrsplit(SampleID, split = "-")[[4]]) %>% 
    left_join(Granulo_data, by= c("Site", "ID", "Topography"))
  
  Pedo_data <- Pedo_data %>% 
    group_by(Topography) %>% 
    summarise(Argiles = mean(Argiles),
              Limons = mean(Limons),
              Sables = mean(Sables),
              BulkDensity = mean(BulkDensity)) %>% 
    ungroup()
  
  # Standardise pour sommer à 100%
  Pedo_data <- Pedo_data %>%
    rowwise() %>% 
    mutate(Sum = Argiles + Limons + Sables) %>% 
    mutate(Argiles_st = round((Argiles/sum(Sum))*100, digits = 1)) %>% 
    mutate(Limons_st = round((Limons/sum(Sum))*100, digits = 1)) %>% 
    mutate(Sables_st = round((Sables/sum(Sum))*100, digits = 1)) %>% 
    mutate(Sum_st = Argiles_st + Limons_st + Sables_st) %>% 
    # 2ème tour
    mutate(Argiles_st = round((Argiles_st/sum(Sum_st))*100, digits = 1)) %>% 
    mutate(Limons_st = round((Limons_st/sum(Sum_st))*100, digits = 1)) %>% 
    mutate(Sables_st = round((Sables_st/sum(Sum_st))*100, digits = 1)) %>% 
    mutate(Sum_st = Argiles_st + Limons_st + Sables_st) %>% 
    select(Topography, Argiles_st, Limons_st, Sables_st, Sum_st, BulkDensity) %>% 
    rename(Argiles = Argiles_st) %>% 
    rename(Limons = Limons_st) %>% 
    rename(Sables = Sables_st) 
  
  return(Pedo_data)
}
Pedo_data_Paracou <- GetPedodata4Tomst(site="Paracou")
Pedo_data_Nouragues <- GetPedodata4Tomst(site="Nouragues")

write.csv(Pedo_data_Paracou, "~/PhD/Microclimate/Tomst - data and scripts/Paracou_Tomst_humidity_granulo_parameters.csv")
write.csv(Pedo_data_Nouragues, "~/PhD/Microclimate/Tomst - data and scripts/Nouragues_Tomst_humidity_granulo_parameters.csv")
