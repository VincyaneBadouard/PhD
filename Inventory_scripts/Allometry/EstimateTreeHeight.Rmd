...
```{r}
FTH <- read_csv("~/PhD/Inventories/Data/Height-Diameter/Understory/Allometry_FTH_GSellan_Paracou_20240125.csv")
```
...
CC <- read_csv("~/PhD/Inventories/Data/Height-Diameter/Adults/CanopyConstructorDataP16.csv") %>% 
  ...
  filter(DBH >20) # only trees >20 cm besause cc is not less good under the canopy
...
Adults <- Adults %>% 
  ...
  left_join((FTH %>% select(-c(DBH, Family, ScientificName))), by=c("idTree"))

Measuredtrees <- Adults %>% filter(!is.na(TreeHeight))  # 445 trees
