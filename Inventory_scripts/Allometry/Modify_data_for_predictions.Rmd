...
```{r}
sp <- data.frame(species_noSing = unique(fit$data$species_noSing),
                 Match = T) %>% 
  separate(species_noSing, sep = " ", into = c("Genus", "Species"), remove = F) # 470
```
...
test <- Inventory %>% 
  ...
  unique() %>% 
  mutate(species_noSing = str_replace(ScientificName, "indet", "sp\\.")) %>% # species have points between genus and species names in the model
  separate(species_noSing, sep = ".subsp.", into = c("species_noSing", "Subspecies"), remove = F) %>% 
  mutate(species_noSing = data.table::tstrsplit(species_noSing, split = ".var\\.")[[1]]) %>% 
  mutate(species_noSing = str_replace(species_noSing, "_aff.", "_")) %>% 
  ...
  mutate(species_noSing = ifelse(ScientificName=="Indet.Indet._Indet.", "Indet.Indet. Indet.", species_noSing)) %>% 
  ...
  mutate(species_noSing = recode(species_noSing, 
                                 "Eschweilera grandiflora" = "Eschweilera grandiflora_form2",
                                 "Tovomita sp.9" = "Tovomita sp.9-CAY",
                                 "Sloanea sp.14" = "Sloanea sp.14-CAY",
                                 "Sloanea sp.20" = "Sloanea sp.20-CAY",
                                 "Tovomita sp.2" = "Tovomita sp.2-CAY",
                                 "Indet.Nyctaginaceae sp.4" = "Indet.Nyctaginaceae sp.4-CAY",
                                 "Aniba rosodora" = "Aniba rosaeodora",
                                 "Inga melinonii" = "Inga melinonis",
                                 "Pouteria gonggrijpii" =  "Pouteria gongrijpii"
  ))
...
# Test match 
```{r}
sp$species_noSing[!sp$species_noSing %in% test$species_noSing] # 91

nomatch <- test %>%  # 326
  left_join(sp, by= c("species_noSing")) %>% 
  filter(is.na(Match)) %>% 
  select(ScientificName, species_noSing) %>% unique()
...
nrow(nomatch) + nrow(match) == length(unique(Inventory$ScientificName))

# un Symphonia_sp.1 dans Family (vérifier qu'il n'y que du aceae dans Family sinon mettre dans ScientificName)
...
# Eugenia.sp.FG9 vs Eugenia.sp.FG9-Holst  -> même espece
# Inga.graciliflora vs Inga.gracilifolia -> pas meme espese
# Eugenia.cucullata vs Eugenia.cupulata -> pas même espèce
...
# Eschweilera.grandiflora vs Eschweilera.grandiflora_form2  -> même espèce
# Aniba.rosodora vs Aniba.rosaeodora -> Aniba.rosaeodora
# Inga.melinonii vs Inga.melinonis -> même espèce

```

```{r}
write.csv(match, "D:/Mes Donnees/PhD/Inventories/Data/Understory/Paracou/Species_correspondence_Claudia.csv")
```
