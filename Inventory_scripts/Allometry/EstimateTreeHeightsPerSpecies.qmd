...
**J'aurais du utiliser WFO, il ya des synonymies** 
...
length(unique(match$ScientificName)) # 399
# length(unique(Inventory$ScientificName)) # 725
725-399 # 326
(326/725)*100
...
nrow(Inventory) == nrow(Understory) + nrow(Adults) # 47850
```
```{r, eval=F}
communityallom <- Inventory %>% 
  filter(!ScientificName %in% unique(match$ScientificName))
(nrow(communityallom)/nrow(Inventory))*100 # 23% of indiv have been estimated by a community allometry (11,177 trees)
```
...
length(is.na(unique(Inventory$species_noSing))) # 380 (=399-20-NA)
length(unique((Inventory %>% filter(is.na(Inventory$species_noSing)))$ScientificName)) # 326
...
```
```{r, eval=F}
H_allometry_df <- Inventory %>% 
  select(ScientificName, species_noSing) %>% 
  unique() %>% 
  mutate(H_allometry = ifelse(is.na(species_noSing),'Community','Specific')) %>%  # 725
  select(-species_noSing) 

write_csv(H_allometry_df,'D:/Mes Donnees/PhD/Inventories/Data/Height-Diameter/H_allometry_method.csv')
```
...
```{r prediction with 100 posterior values, eval = F}
y_rep <- posterior_predict(fit, newdata = same_sp, ndraws = 100) # Génère des valeurs simulées de la variable réponse, 1 prédiction par draw du posterior
```
...
range(other_sp$TreeHeight) # 2.646925 36.729651 ->  2.63676 36.75312 apres coord cor
```
## Plot
```{r}
...
```{r}
pred_test <- pred %>% 
  filter(ScientificName %in% FTH$ScientificName) %>% # among FTH measured species
  # filter(!ScientificName %in% match$ScientificName) %>% # not species with specific allometries
  mutate(DBH_d = round(DBHcor)) %>% 
  select(ScientificName, DBH_d, TreeHeight) %>% 
  group_by(ScientificName, DBH_d) %>% # by species and diameter
  summarise(Treeheight_median = median(TreeHeight)) %>% # compute the median
  ungroup() %>% 
  mutate(type = ifelse(ScientificName %in% match$ScientificName, 'Specific', 'Community')) # allometry type


FTH_test <- FTH %>%
  # filter(!ScientificName %in% match$ScientificName) %>% # not species with specific allometries
  filter(!is.na(ScientificName)) %>% 
  # par classe genre 1cm ou 5 -ou10
  mutate(DBH_d = round(DBH)) %>% 
  arrange(DBH_d) %>% 
  select(ScientificName, DBH_d, TreeHeight) %>% 
  group_by(ScientificName, DBH_d) %>% 
  summarise(Treeheight_median = median(TreeHeight)) %>% 
  left_join(pred_test, by = c('ScientificName', 'DBH_d'), suffix = c('_m', '_e')) %>% 
  mutate(diff = Treeheight_median_e - Treeheight_median_m)

quantile(FTH_test$diff, na.rm =T) # Q50% = 1.4
quantile(FTH_test[FTH_test$type=='Community', ]$diff, na.rm =T) # Q50% = 0.9
quantile(FTH_test[FTH_test$type=='Specific', ]$diff, na.rm =T) # Q50% = 1.6
...
ggplot(aes(x= DBH_d, y = diff, group = type, col = type)) + # , group = type
  theme_minimal() +
...
pred %>% 
  filter(ScientificName %in% FTH$ScientificName) %>% 
  filter(!ScientificName %in% match$ScientificName) %>% 
  ggplot(aes(x=DBH, y=TreeHeight)) + 
  # xlim(1,20) +
  theme_minimal() +
  geom_point(col= "grey", size=1, alpha=0.2) +
  geom_point(data = FTH %>% filter(!ScientificName %in% match$ScientificName),
             col= "darkgreen", size=1, alpha=0.2) +
  # scale_x_continuous(breaks =seq(0, 20, by= 5)) +
  # guides(color = F) +
  labs(y = "Tree height (m)", x="DBH (cm)")
```
...
ggsave("Treeheights_species_vs_nospecies_effect_vs_FTH_aftercoordcor.pdf",
       path = "D:/Mes Donnees/PhD/R_codes/PhD/Inventory_scripts/Exploration/pdf",
       plots, width = 15, height = 10)
```
Pour (before coord cor):
species effect:
Mouriri_huberi
...
# pred <- read_csv("D:/Mes Donnees/PhD/Inventories/Data/Adults_Understory/Paracou/AllP16_TreeHeights.csv") # 47,850
