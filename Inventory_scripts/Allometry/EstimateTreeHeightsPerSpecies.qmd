...
pred_test <- pred %>% 
  ...
  select(DBH_d, TreeHeight) %>% 
  group_by(DBH_d) %>% 
  summarise(Treeheight_median = median(TreeHeight))



FTH_test <- FTH %>% filter(!ScientificName %in% match$ScientificName) %>% # not species with specific allometries
# par classe genre 1cm ou 5 -ou10
  ...
  group_by(DBH_d) %>% 
  summarise(Treeheight_median = median(TreeHeight)) %>% 
  left_join(pred_test, by = 'DBH_d', suffix = c('_m', '_e')) %>% 
  mutate(diff = Treeheight_median_e - Treeheight_median_m)

quantile(FTH_test$diff, na.rm =T)
# Q0%       Q25%       Q50%       Q75%      Q100% 
# 0.5       0.7        1.4        2.3       6.4 m
mean(quantile(FTH_test$diff, na.rm =T)) # 2.3 m
