...
same_sp_pred <- brms::predict(fit, newdata = same_sp, robust=T, na.action = na.omit) # 1 min, robust=T to take the posterior median, mean if F
same_sp$TreeHeight <- same_sp_pred[,1]
range(same_sp$TreeHeight) #  2.003142 49.087198 -> 2.034959 49.109834 apres coord cor
```

```{r prediction with 100 posterior values}
