...
length(!is.na(unique(Inventory$species_noSing))) # 380 (=398-20-NA)
...
```{r}
same_sp_pred <- predict(fit, newdata = same_sp, na.action = na.omit) # 1 min
same_sp$TreeHeight <- same_sp_pred[,1]
range(same_sp$TreeHeight) # 1.449219 58.189122
```
## Plot
```{r}
...
```{r}
other_sp_pred <- predict(fit, newdata = other_sp, re_formula = NA, na.action = na.omit) # 1 min
other_sp$TreeHeight <- other_sp_pred[,1]
range(other_sp$TreeHeight) # 2.655371 36.751789
```
## Plot
```{r}
...
ggplot(same_sp, aes(x=dbh.y, y=TreeHeight)) + 
  ...
  theme_minimal() +
  geom_point(col= "grey") +
  geom_point(data = other_sp, col= "darkgrey") +
  geom_point(data = FTH, col= "darkgreen") +
...
# Write data
```{r}
write.csv(data_filtred, "D:/Mes Donnees/PhD/Inventories/Data/Adults_Understory/Paracou/AllP16_TreeHeights.csv")
```
