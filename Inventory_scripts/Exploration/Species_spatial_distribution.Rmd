...
```{r}
data <- Inventory %>% 
  select(idTree, SubPlot, ScientificName, Strategy, DBHcor, Xutm, Yutm, TreeHeight) %>% 
...
for(p in 1:length(unique(data$ScientificName))){
  plist[[p]] <- local({
    ...
    ggplot(dataS) + 
      geom_spatraster(data = mask(crop(MNT,P16), P16), aes(fill = dtm2023_HighAlt_25ha_buffer)) +
      # scale_fill_gradientn(name = "Elevation (m)",
      #                      colors = terrain.colors(30, rev=T, alpha = 0.9),
      #                      na.value="white") +
      # scale_colour_gradient(name = "DBH (cm)",
...
# ggsave("Spatial_distribution_of_each_species_on_MNT_notlog.pdf", 
...
# ggsave("Spatial_distribution_of_each_species_on_TWI.pdf",
#        path = "D:/Mes Donnees/PhD/R_codes/PhD/Inventory_scripts/Exploration/pdf",
#        plots, width = 15, height = 10) 
...
# ggsave("Spatial_distribution_of_each_species_on_relativeelevation.pdf",
#        path = "D:/Mes Donnees/PhD/R_codes/PhD/Inventory_scripts/Exploration/pdf",
#        plots, width = 15, height = 10)
```
...
ggsave(paste("Species_",var,"_niche_density_without_ontogeny.pdf",sep=""),
       path = "D:/Mes Donnees/PhD/R_codes/PhD/Inventory_scripts/Exploration/pdf",
       plots, width = 15, height = 10)
```
...
ggsave(paste("Species_",var,"_density_points_notlog_withoutonto.pdf",sep=''),
       path = "D:/Mes Donnees/PhD/R_codes/PhD/Inventory_scripts/Exploration/pdf",
       plots, width = 15, height = 10)
```
...
```{r, eval=F}
plist <- vector('list', round(length(unique(data$ScientificName))/10))

for(p in 1:round(length(unique(data$ScientificName))/10)){
  # message(s)
  plist[[p]] <- local({
    # p <- 8
    if (p==1) {start = 1
...
# ggsave(paste("Species_",var,"_niche_points_fixy.pdf",sep=''), 
#        path = "D:/Mes Donnees/PhD/R_codes/PhD/Inventory_scripts/Exploration/pdf",
#        plots, width = 15, height = 10)
...
for(p in 1:length(unique(data$ScientificName))){
  ...
  plist[[p]] <- local({
    ...
    ggplot(Data, aes(x= DBHcor, y= Transmittance)) + 
      ...
      labs(title = unique(Data$ScientificName), x= "DBH (cm)", y= "Transmittance")
