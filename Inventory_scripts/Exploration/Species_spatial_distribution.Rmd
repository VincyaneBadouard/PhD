...
P16 <- st_as_sf(raster::shapefile("D:/Mes Donnees/PhD/R_codes/PhD/test/ALT.shp")) %>% 
  st_set_crs(st_crs(MNT))
...
data <- Inventory %>% 
  ...
  filter(!is.na(Xutm)) %>% 
  filter(!is.na(Yutm)) %>% 
  filter(ScientificName=="Oxandra_asbeckii") %>% 
  st_as_sf(coords = c("Xutm", "Yutm")) %>% 
  st_set_crs(st_crs(MNT))
...
ggplot() + 
  ...
  scale_fill_gradientn(name = "Hillshade",
                       colors = hcl.colors(5, "Grays", rev=T),
                       na.value="white") +
  geom_sf(data = sf::st_cast(P16, "LINESTRING")) +
  geom_sf(data = data, size = 2) + # mapping = aes(colour = DBHcor)
  labs(title = "Oxandra asbeckii distribution") +
  theme_classic() +
  theme(title= element_text(size=16),axis.text= element_text(size=16),
        legend.title= element_text(size=16), legend.text= element_text(size=16)) +
  coord_sf()

ggsave("Spatial_distribution_of_Oxandra_asbeckii_on_hillshade.png",
       path = Path,
       width = 15, height = 10)
```
...
ggsave("Spatial_distribution_of_Oxandra_asbeckii_on_MNT.png",
       path = Path,
       width = 10, height = 8)
```
...
```{r}
dataS <- data %>% 
  filter(ScientificName=="Vouacapoua_americana")

ggplot(dataS) +
  geom_sf(data = sf::st_cast(P16, "LINESTRING")) +
  geom_sf(mapping = aes(colour = DBHcor), alpha = 2, size = 1) +
  viridis::scale_colour_viridis(name = "DBH (cm)", direction=-1, labels=round) + # , trans = "log"
  theme_void() +
  guides(fill="none") +
  ...
  coord_sf() +
  labs(title = "Vouacapoua americana")

ggsave("P16_Spatial_distribution_of_Vouacapoua_americana.png",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
       width = 5, height = 5)
```
...
inconsistantsp <- c('Cordia_nodosa', 'Pouteria_gonggrijpii', 'Dicorynia_guianensis')

plist <- vector('list', length(inconsistantsp))

for(p in 1:length(inconsistantsp)){
  plist[[p]] <- local({
    dataS <- data %>% 
      filter(ScientificName==inconsistantsp[p]) 
    ...
    ggplot(dataS) + 
      ...
      scale_fill_gradientn(name = "TWI",
                           ...
                           transform = "log") +
      geom_sf(data = sf::st_cast(P16, "LINESTRING")) +
      geom_sf(mapping = aes(colour = DBHcor), alpha = 0.7, size = 1) +
      viridis::scale_colour_viridis(name = "DBH (cm)", direction=-1, labels=round) + # , trans = "log"
      theme_void() +
      guides(fill="none") +
      # geom_text(label = max(dataS$DBHcor), position ="top")
...
legend <- cowplot::get_legend(map, legend = "bottom")
ggdraw(legend)

# (plist[[1]] + plist[[2]])/
#   (plist[[3]] + plist[[4]]) + plot_spacer() + legend + plot_layout(heights = c(1, 1, 1, -1))

plot_grid(plist[[1]], plist[[2]], plist[[3]], legend, 
          ncol = 2, nrow=3, rel_heights = c(1, 1, .2))

# plot_grid(plist[[1]], plist[[2]], plist[[3]], legend, 
#           ncol = 3, nrow=2, rel_heights = c(2, .2))


ggsave("P16_ALT_Spatial_distribution_of_4_inconsistent_species_on_TWI_log.png",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
...
ggsave("P16_ALT_Spatial_distribution_of_each_species_on_TWI_log.pdf",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
       plots, width = 15, height = 10)
```
