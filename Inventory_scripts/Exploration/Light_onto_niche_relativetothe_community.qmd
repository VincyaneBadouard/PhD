...
dataS %>% 
  ...
  labs(title = "Community abundances")

ggsave("Light_onto_niche_relativetothe_community_binabundances_logcolor.png",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
       width = 8, height = 5)
...
for(p in 1:length(sp)){
  plist[[p]] <- local({
    ...
    dataS %>% 
      ...
      geom_raster() +
      geom_vline(xintercept = log(10)) + # 10 cm DBH
      scale_fill_viridis_c("Individuals number") +
      scale_x_continuous("Diameter at Breast Height (DBH, cm)",
                         n.breaks = 10, labels = ~round(exp(.),2)) + # delog
      scale_y_continuous("Light Transmittance (%)",
                         n.breaks = 10, labels = ~round(exp(.)*100,2)) + # delog
...
ggsave("Light_onto_niche_relativetothe_community_binabundances.pdf",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
       plots, width = 15, height = 10)
```

# Environment bins per species
```{r}
plist <- vector('list', length(sp))
...
ggsave("Light_onto_niche_relativetothe_community_logcolor.pdf",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
       plots, width = 15, height = 10)
```
  ...
  plist[[p]] <- local({
    ...
    dataS %>% 
      ...
      mutate_at(vars(c("log_dbh_bin_l", "log_dbh_bin_h")),
                ~as.character(.)) %>% 
      # ~as.character(round(exp(.),2))) %>% # marche mais désorganise les labels
