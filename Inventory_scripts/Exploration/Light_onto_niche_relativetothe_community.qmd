...
for(p in 1:length(sp)){
  plist[[p]] <- local({
    ...
    # loged bins
    test %>% 
      mutate(log_trans_bin = cut_interval(logTransmittance, n = 10)) %>% 
      group_by(Presence, Size, log_trans_bin) %>% 
      summarise(logTransmittance_mean = mean(logTransmittance),
                Sd = sd(logTransmittance)) %>% 
      ungroup() %>% 
      
      ggplot(aes(Size, logTransmittance_mean, col = Presence, fill = Presence)) +
      theme_minimal() +
      ...
      # geom_jitter(shape=16, position=position_jitterdodge(seed = 1, dodge.width = 0.9)) +
      ...
      geom_text(data = label, aes(label = N, y = 0), size = 3.5,
...
ggsave("Light_onto_niche_relativetothe_community_boxplots_new_size_class.pdf",
       path = "//amap-data.cirad.fr/work/users/VincyaneBadouard/Inventories/Explo_plots",
       plots, width = 15, height = 10)
