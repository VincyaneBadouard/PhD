...
data <- Inventory %>%
  filter(Species=="Vouacapoua americana"|Species=="Bocoa prouacensis") %>% 
  mutate(Species = factor(Species, levels = c("Vouacapoua americana","Bocoa prouacensis")))
...
ggsave("DBH_distribution_Vouacapoua_Bocoa.png", path = PATH, width = 13, height = 8, units = "cm", dpi=800, bg="white")
```
