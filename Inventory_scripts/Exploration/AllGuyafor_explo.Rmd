...
data <- data %>% 
  ...
  # DBH max
  mutate(MaxDBH = max(DBH, na.rm = T)) %>% # peut-etre prendre plutot le 95th percentile to correct for outliers, because it is easy to overestimate the height of tall trees.
  # Abundances
  ...
  filter(N>=30) %>% 
  unique() # 267 sp à Paracou; 454 sur Guyafor
