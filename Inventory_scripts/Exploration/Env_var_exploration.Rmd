...
Hillshade <- terra::rast("D:/Mes Donnees/PhD/R_codes/PhD/test/Paracou_P16_hs.tif")
...
# Light
Inventory <- read.csv(
  "~/PhD/Inventories/Data/Adults_Understory/Paracou/AllP16_Light_2019.csv") %>% # 47 850
...
ggplot() + 
  ...
  theme_classic() +
  ggtitle("Paracou P16 Hillshade - ALS 2022") + 
  coord_sf()
...
ggplot() + 
  ...
  scale_fill_gradientn(name = "Height Above \nNearest Drainage (m)",
                       colors = hcl.colors(45, "Blues"),
                       na.value="white") +
  # geom_sf(data = st_cast(st_cast(TopoLevels, "POLYGON"), "LINESTRING")) +
  geom_sf(data = st_cast(st_cast(ALT, "POLYGON"), "LINESTRING")) +
  theme_classic() +
  theme(axis.text= element_text(size=7),
        legend.title= element_text(size=14), legend.text= element_text(size=12))

ggsave("Paracou_P16_ALT_HAND.png",
       path = "D:/Mes Donnees/PhD/Figures/EnvVar_explo/",
...
MNT[MNT[] < 6] = NA 
# plot(MNT)
MNT_sf <- st_as_sf(as.polygons(MNT))
plot(MNT_sf)
MNT_sf <- st_union(MNT_sf)
st_area(MNT_sf) # 230557 m2/10000 = 23.0557 ha
...
TWI <- aggregate(TWI, fact=5) # to reput at 5 m res
...
# TWI
TWI$Paracou_TWI_5m # 3.948909 - 10.882358
terra::global(TWI, fun="mean", na.rm=TRUE)[,1] # 6.501359
sqrt(terra::global(TWI, fun="sd", na.rm=TRUE)[,1]) # 1.157368
...
ggplot() +
  ...
  geom_histogram(data= TWIalt, aes(x=Paracou_TWI_5m),
                 binwidth=0.25, fill="grey", color="#e9ecef", alpha=0.9) +
  scale_x_continuous(breaks = seq(4, 14, 1)) +
  labs(y='Abundance',x='TWI')

ggsave("Paracou_P16_ALT_TWI.png",
       path = "D:/Mes Donnees/PhD/Figures/EnvVar_explo/",
...
ggplot() +
  ...
  scale_x_continuous(breaks = seq(-0.4, 24, 2)) +
  labs(y='Abundance',x='HAND')
...
ggsave("Paracou_P16_ALTLight_distrib.png",
       path = "D:/Mes Donnees/PhD/Figures/EnvVar_explo/",
       width = 15, height = 10, units = "cm", dpi=800, bg="white")
```
...
Quantiles <- data.frame(Q =c("50%", "5%", "mean", "95%"),
                        x = c(quantile(Light$Transmittance, .5),
                              quantile(Light$Transmittance, .05),
                              mean(Light$Transmittance),
                              quantile(Light$Transmittance, .95)))
...
NotT <- ggplot(Light, aes(x=Transmittance)) +
  ...
  geom_vline(xintercept = mean(Light$Transmittance)) +
  geom_vline(xintercept = quantile(Light$Transmittance, .05), linetype = "dashed") +
  geom_vline(xintercept = quantile(Light$Transmittance, .95), linetype = "dashed") +
...
log <- ggplot(Light, aes(x=Transmittance)) +
  ...
  scale_x_continuous(trans="log",
                     breaks = c(0, min(Light$Transmittance),
                                0.001, 0.002, 0.01, 0.02, 0.07, 0.1, 0.4, 1),
                     labels = as.character(c(0,                                             format(round(min(Light$Transmittance),5), scientific=FALSE),
                                             0.001, 0.002, 0.01, 0.02, 0.07, 0.1, 0.4, 1)) ) +
  geom_vline(xintercept =  0.001, col= "darkgreen") +
  geom_vline(xintercept = mean(Light$Transmittance)) +
  geom_vline(xintercept = quantile(Light$Transmittance, .05), linetype = "dashed") +
  geom_vline(xintercept = quantile(Light$Transmittance, .5), linetype = "dashed") +
  geom_vline(xintercept = quantile(Light$Transmittance, .95), linetype = "dashed") +
  geom_text(data = Quantiles, mapping = aes(x= x, y = Inf, label= Q), vjust=1.5, hjust=-.2, size = 3) +
...
sqrt <- ggplot(Light, aes(x=Transmittance)) +
  ...
  geom_vline(xintercept = quantile(Light$Transmittance, .05), linetype = "dashed") +
  geom_vline(xintercept = quantile(Light$Transmittance, .95), linetype = "dashed") +
  geom_text(data = Quantiles[2:4,], mapping = aes(x= x, y = Inf, label= Q), vjust=1.5, hjust=-0.2, size = 3) +
