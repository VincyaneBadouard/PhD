...
```{r}
Piquets_Paracou <- read_delim("~/PhD/Inventories/Data/Piquets_ALT/Measures_piquets_Paracou13_18_23_24.csv", 
...
ALT <- st_as_sf(ALT, coords = c('Xutm','Yutm')) # as sf
ALT <- st_set_crs(ALT, terra::crs(P16shp)) # set crs

ALTstars <- st_rasterize(ALT %>% dplyr::select(geometry)) # as stars
geometreStars <- st_rasterize(SubplotCoordsf %>% dplyr::select(geometry)) # as stars

ggplot() +
  geom_sf(data = st_as_sf(ALTstars,as_points = T), col="green", size = 1) +
  geom_sf(data = st_as_sf(geometreStars,as_points = T), col="darkgreen", size = 0.8) +
  ggtitle("Before warping")

# rm(warp)
warp <- st_warp(geometreStars, # to warp ALTstars
                geometreStars, # target 
                method = "near", use_gdal=F) # not what I want
warp

# Using GDAL -------------------------------------------------------------------
sf::sf_extSoftVersion() # GDAL 3.8.2
Sys.setenv("GDAL_DATA" = "/path/to/gdal/data")

warp <- st_warp(ALTstars, # to warp
                geometreStars, # target 
                method = "average", use_gdal=T) # doesn't work 
warp_gdal
warp; warp_gdal
# ------------------------------------------------------------------------------

ggplot() +
  geom_sf(data = SubplotCoordsf, col="red", size = 0.8) +
  geom_sf(data = st_as_sf(geometreStars,as_points = T), col="darkgreen", size = 0.8) # 1 point a disparu, pq?
  # geometreStars != SubplotCoordsf, why?
  
ggplot() +
  geom_sf(data = st_as_sf(geometreStars,as_points = T), col="darkgreen", size = 0.8) +
  geom_sf(data = st_as_sf(ALTstars,as_points = T), col="green", size = 0.8) +
  ggtitle("Before warping")
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(geometreStars,as_points = T), col="darkgreen", size = 0.8) +
  # geometreStars != SubplotCoordsf, why?
  # geom_sf(data = st_as_sf(ALTstars,as_points = T), col="blue", size = 1) +
  geom_sf(data = st_as_sf(warp,as_points = T), col="green", size = 0.8) + # des points ont disparus, et ça n'a servi à rien
  ggtitle("After arping")
...
fromCornerCoord <- expand.grid(X = c(0, 100), Y = c(0, 50)) # coord relatives des coins de parcelle
rot_mat <- matrix(c(cos(-pi/6),sin(-pi/6),-sin(-pi/6),cos(-pi/6)),nrow=2)
toCornerCoord <- as.matrix(fromCornerCoord) %*% rot_mat 
toCornerCoord <- sweep(toCornerCoord, 2, c(50,100), FUN = "+") # coord de référence des coins de parcelle
coord <- expand.grid(X = seq(0,100,10), Y = seq(0,50,5)) # coord relatives à transformer
