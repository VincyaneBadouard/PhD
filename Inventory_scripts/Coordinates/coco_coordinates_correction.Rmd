...
# vect("D:/Mes Donnees/PhD/SIG_data/Understory-ALT/Geometre_2024/Sy23179-Paracou_P16_9ha_coord_Subplot.shp") %>% 
#   st_as_sf() -> surveyor.sf
...
```{r}
# laser measurements data I think
quadrats <- read_xlsx("D:/Mes Donnees/PhD/Inventories/Data/Piquets_ALT/plots.xlsx", sheet = "quadrats") %>% 
...
- ajoute les piquets des bordures droites et supérieures, absents du tableau Excel,
...
trees <- read.csv("D:/Mes Donnees/PhD/Inventories/Data/Understory/Paracou/ALT_Paracou_9ha_CLEAN.csv") %>% # 36 311
  # Homogenize the column names
  rename(
    subplot = "Subplot",
    quadrat_x = "Quadra.nb.X",
...
trees <- trees %>% 
  ...
  mutate(x = x_quadrat + x, y = y_quadrat + y) 
```

Des cartes locales peuvent être produites, par exemple pour les quadrats du coin du bas à gauche de la sous-parcelle 23:
...
ggplot(
  trees,
  aes(x = x_utm, y = y_utm, color = as.factor(subplot), size = DBHcor)
) +
  geom_point() + 
  coord_fixed() +
...
```
```{r}
range(trees$correction_d) # 3.5e-12 4.36 m de déplacement
median(trees$correction_d) # 0.75 m de déplacement
...
write.csv(trees, "D:/Mes Donnees/PhD/Inventories/Data/Understory/Paracou/ALT_Paracou_9ha_CLEAN_corcord.csv")
