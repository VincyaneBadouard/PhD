...
# Surveyor's points
library("readxl")
read_xlsx("D:/Mes Donnees/PhD/SIG_data/Understory-ALT/Geometre_2024/plots.xlsx", sheet = "surveyor") %>% # created by Eric
...
```{r}
read_xlsx("D:/Mes Donnees/PhD/SIG_data/Understory-ALT/Geometre_2024/plots.xlsx", sheet = "subplots") %>% # created by Eric
...
read_xlsx("D:/Mes Donnees/PhD/SIG_data/Understory-ALT/Geometre_2024/plots.xlsx", sheet = "quadrats") %>% 
...
to_correct_right <- subplots %>% 
  filter(!is.na(next_right)) %>% 
  pull(subplot) 
to_correct_up <- subplots %>% 
  filter(!is.na(next_up)) %>% 
  pull(subplot)

# Right edge correction
reconcile_right <- quadrats %>% 
  filter(
...
for (subplot in unique(quadrats$subplot)) {
  ...
  # Save the coordinates
  xy_local <- quadrats[is_in_subplot, c("x", "y")]
}
...
trees <- read.csv("D:/Mes Donnees/PhD/Inventories/Data/Understory/Paracou/ALT_Paracou_9ha_CLEAN.csv") %>% # 36 338
  ...
  drop_na(subplot, point_x, point_y, x_field, y_field) # 36 338
```
...
for (subplot in unique(trees$subplot)) {
  ...
  # Save the coordinates
  xy_utm <- trees[is_in_subplot, c("x_utm", "y_utm")]
}
...
ggplot(
  trees,
  aes(x = x_utm, y = y_utm, color = as.factor(subplot), size = DBH)
) +
  geom_point() + 
  coord_fixed() +
