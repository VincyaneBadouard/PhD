...
```{r}
# Botanical name diff
nrow(all %>% filter(ScientificNameA != ScientificNameU)) # 47
...
nrow(all %>% filter(DBHdiff > 15 | DBHdiff <(- 6))) # 56 arbres

nrow(all %>% filter(grepl("Non-unique combinaison of IdTree and Guyafor.nb", Comment)) %>% filter(DBHdiff > 15 | DBHdiff <(- 6))) # 0 arbres

# Together
nrow( all %>% 
        filter(ScientificNameA != ScientificNameU) %>% 
        filter(Dist>2) %>% # 15
        filter(DBHdiff > 15 | DBHdiff <(- 6)) ) # 
...
OK <- test %>% 
  filter(SelectedCoord==T & Dist<5 & GenusA==GenusU & (DBHdiff < 15 & DBHdiff >(- 6)))  # 3743/4485 -> 83%
...
growthpb <- test %>%
  filter(SubPlotU==SubPlotA & (DBHdiff > 15 | DBHdiff <(- 6))) %>% select(IdTree) %>% unique() # 56 
...
growthpb <- test %>%
  filter(SelectedCoord==T & (DBHdiff > 15 | DBHdiff <(- 6))) %>% select(IdTree) %>% unique() # 56 
