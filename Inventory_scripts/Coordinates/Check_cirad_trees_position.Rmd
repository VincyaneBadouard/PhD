...
length(unique((all %>% 
                 filter(is.na(GenusA)))$IdTree)) # 1096 arbres sans identification bota
...
OK <- test %>% 
  filter(SelectedCoord==T & Dist<5 & GenusA==GenusU & (DBHdiff < 15 & DBHdiff >(- 6)))  # 3714/4537 -> 82%
range(OK$DBHdiff, na.rm = T) #  -38.41268 178.01549
OKchange <- OK %>%
  filter(SelectedCoord & SubPlotU!=SubPlotA) # 10 parmis les ok ont changé de plot

# Combien étaient mal identifiés
badbota <- test %>%
  filter(GenusA!=GenusU & SubPlotU==SubPlotA) %>% select(IdTree) %>% unique() # 39 
...
growthpb <- test %>%
  filter(SubPlotU==SubPlotA & (DBHdiff > 15 | DBHdiff <(- 6))) %>% select(IdTree) %>% unique() # 69 
...
loin <- test %>%
  filter(SelectedCoord==T & Dist>10) %>% select(IdTree) %>% unique() # 154 

# Combien ont encore des taux de croissance aberrants
growthpb <- test %>%
  filter(SelectedCoord==T & (DBHdiff > 15 | DBHdiff <(- 6))) %>% select(IdTree) %>% unique() # 88 
...
testsup <- test %>% 
  group_by(IdTree) %>% 
  filter(SelectedCoord==T) # 0 -> 1 seul match final youhou! 
