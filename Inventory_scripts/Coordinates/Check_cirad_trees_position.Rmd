...
test <- all %>% 
  group_by(Guyafor.nb) %>% 
  # Same species name
  mutate(SelectedSp = ifelse(GenusA==GenusU, T, F)) %>% 
  mutate(NbSelectedSp = sum(SelectedSp, na.rm = T)) %>%  # combien de T par Guyafor.nb
  # Closer coordinates
  # rowwise() %>% # en fait ça marche pas
  group_by(Guyafor.nb, SelectedSp) %>% 
  ...
  # Closer DBH
  group_by(Guyafor.nb, SelectedCoord) %>% 
  rowwise() %>% 
  mutate(SelectedDBH = ifelse(SelectedCoord & (DBHdiff== min(abs(DBHdiff))), T, F)) %>% 
  mutate(NbSelectedDBH = sum(SelectedDBH)) %>% 
  rowwise() %>% 
  mutate(SelectedDBH = ifelse(SelectedCoord & NbSelectedDBH ==0 & (DBHdiff < 15 & DBHdiff >(- 6)), T, SelectedDBH)) %>% 
  mutate(NbSelectedDBH = sum(SelectedDBH)) %>% 
  ungroup()

# Checks
test0 <- test %>% 
  filter(!is.na(GenusU) & !is.na(GenusA)) %>% 
  filter(NbSelectedDBH==0) # 4129/4537 arbres cirad n’ont pas un bon match
length(unique(test0$Guyafor.nb)) 

test0sp <- test %>% 
  filter(!is.na(GenusU) & !is.na(GenusA)) %>% 
  filter(NbSelectedSp==0) #%>% # 188 arbres sans aucun match de nom de genre entre ALT et Guyafor...
length(unique(test0sp$Guyafor.nb)) 

testsup <- test %>% 
  filter(NbSelectedDBH>1) # 0 -> 1 seul match final youhou! 

testchange <- test %>%
  filter(SelectedDBH & SubPlotU!=SubPlotA) 
length(unique(testchange$Guyafor.nb)) # que 5 qui étaient en fait d'un autre subplot...-> 48

checkchange <- test %>%
  filter(Guyafor.nb %in% testchange$Guyafor.nb)

testgrand <- test %>%
  filter(SelectedDBH & DBHdiff >15) # 18 arbres qui ont grandit vachement vite en 3 ans
length(unique(testgrand$Guyafor.nb))
range(testgrand$DBHdiff) # 15.02253 178.01549
range(testgrand$Dist) # 0.05303301 2.66866218 : mais niveau spatial ça va

testloin <- test %>%
  filter(SelectedDBH & Dist >3) # 498 qui commencent à etre loin..
length(unique(testloin$Guyafor.nb))
range(testloin$Dist) # 3.001276 89.261326
range(testloin$DBHdiff) # -4.774648 11.756717 mais  ne sont pas sur les grosses distances

test %>% filter(SelectedDBH) %>%
  ggplot(aes(x=Dist))+
