...
S_25 <- read_csv("~/PhD/Inventories/Data/Piquets_ALT/C25_20240327.csv") %>%
  ...
  group_by(plot, quadra.nb., quadra.n_1, q.length.X, q.length_1, q.length.Y, q.length_2) %>% 
  mutate(N = n()) %>% 
  ungroup() %>% 
  unique() %>% na.omit()
...
nrow(S_20_2) == nrow(S_25) # S_25 11 quadrats dupliqués mais distances dif ...
...
S_25 <- S_25 %>% 
  ...
  unite(quadra.nb.X, quadra.nb.Y, col = "quadra", sep = "_", remove = F)
...
```{r}
diffdf::diffdf(S_20_1, S_20_2) # -> keep S_20_1 because no NA
```

```{r}
# dupli <- S_25[duplicated(S_25 %>% select(quadra)), ] %>% select(quadra)
# dupli <- S_25 %>% filter(quadra %in% dupli$quadra) %>% arrange(quadra)

S_25 <- S_25 %>% 
  group_by(quadra) %>% 
  filter(N == max(N)) %>% # take the measurement the most abundant
  ungroup()

# any(duplicated(S_25$quadra))
```
...
quadra_code <- function(data){
  if(!all(c("quadra.nb.X", "quadra.nb.Y") %in% names(data)) & "Num" %in% names(data)){
    data <- data %>% 
      arrange(Num) %>% 
      mutate(
        quadra.nb.X = sort(rep(0:9, length.out = 100)),
        quadra.nb.Y = rep(c(0:9, 9:0), length.out = 100)
      )
  }
  return(data)
}
...
# Code quadrats & bind
Stakes <- bind_rows(lapply(list(S_13_18_23_24, S_14, S_15, S_19, S_20_1, S_25), function(data) quadra_code(data)))
nrow(Stakes) == nrow(S_13_18_23_24) + nrow(S_14) + nrow(S_15) + nrow(S_19) + nrow(S_20_1) + nrow(S_25)
unique(Stakes$Subplot) # 9
```
# Missing stake
Max length 100 or 109 : pas de piquet -> 10 m default
```{r}
Com <- Stakes %>% filter(!is.na(Comments))
# no piquet -> 10

Na <- Stakes[!complete.cases(Stakes %>% select(quadra.nb.X,quadra.nb.Y, Q_lengthX, Q_lengthX1, Q_lengthY, Q_lengthY1)), ]

Stakes <- Stakes %>% 
  mutate(Q_lengthX = ifelse(Q_lengthX >=100 | is.na(Q_lengthX), 10, Q_lengthX),
         Q_lengthX1 = ifelse(Q_lengthX1 >=100 | is.na(Q_lengthX1), 10, Q_lengthX1), 
         Q_lengthY = ifelse(Q_lengthY >=100 | is.na(Q_lengthY), 10, Q_lengthY),
         Q_lengthY1 = ifelse(Q_lengthY1 >=100 | is.na(Q_lengthY1), 10, Q_lengthY1))

summary(Stakes)
```
```{r}
Stakes <- Stakes %>% select(-c(quadra, N))
write.csv(Stakes, "~/PhD/Inventories/Data/Piquets_ALT/Stakes_measurments_Paracou_ALT.csv")
```

# Eric's format (en attente Eric)
...
Stakes %>% 
  rename(subplot = Subplot,
         ...
         point_y	= quadra.nb.Y,
         d_left	= , # Q_lengthX du quadrat à sa gauche
         d_down = ) # Q_lengthY du quadrat du dessous
