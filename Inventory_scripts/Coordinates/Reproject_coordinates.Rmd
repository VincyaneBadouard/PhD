...
Understory <- read_csv("~/PhD/Inventories/Data/Understory/Paracou/ALT_Paracou_9ha_CLEAN.csv") %>% 
  ...
  unite(col = "Quadrat", c("Quadra.nb.X", "Quadra.nb.Y"),
        sep = "_", na.rm = TRUE, remove = F) %>%
  filter(!is.na(Subplot) & !is.na(Quadrat)) %>% # coordonnées relatives au quadrat
  filter(Quadrat != "") %>%
  arrange(Quadrat) %>% arrange(Subplot)

unique(Understory$Subplot)
unique(Understory$Quadrat)
```
...
geometre_coordsf <- st_as_sf(geometre_coord, coords = c('Xutm','Yutm'))
...
## Attribute SubPlots information to corner coordinates
```{r}
...
## Attribute SubPlots and quadrats information to each stake coordinates
```{r}
# SubPlots
...
subplots_cornsf <- subplots_corn %>% 
  select(-SubPlot) %>% unique() %>%
...
ALT <- ALT %>% 
  ...
  # Quadrats (ça m'a pris 2 jours...) la fct divide_plot de biomass le fait - a voir
  mutate(X1 = substr(sprintf('%02d', x %% 100), 1,1)) %>% 
  mutate(Y1 = substr(sprintf('%02d', y %% 100), 1,1)) %>% 
  rowwise() %>% 
  ...
  unite(col = "Quadrat1", c("X1", "Y1"),
        sep = "_", na.rm = TRUE, remove = F) %>%
  unite(col = "Quadrat2", c("X2", "Y2"),
        sep = "_", na.rm = TRUE, remove = F) %>%
  unite(col = "Quadrat3", c("X3", "Y3"),
        sep = "_", na.rm = TRUE, remove = F) %>%
  unite(col = "Quadrat4", c("X4", "Y4"),
        sep = "_", na.rm = TRUE, remove = F) %>% 
  pivot_longer(cols = starts_with("Quadrat"),
               names_to = "Quadrat_nb",
               values_to = "Quadrat") %>% 
  
  mutate(SubPlot = ifelse(Y1== "0" &
                            data.table::tstrsplit(Quadrat, split = "_")[[2]] != "0" & y!=0 & y!=300,
                          SubPlot+5, SubPlot)) %>%
  mutate(SubPlot = ifelse(X1== "0" & !SubPlot %in% c(13,18,23) &
                            data.table::tstrsplit(Quadrat, split = "_")[[1]] == "9" & x!=200 & x!=500,
                          SubPlot-1, SubPlot)) %>%
  
  filter(!((x== 200 | y == 0) & data.table::tstrsplit(Quadrat, split = "_")[[2]] == "9" &
             data.table::tstrsplit(Quadrat, split = "_")[[1]] == "9")) %>%
  filter(!((x==500 | y == 0) & data.table::tstrsplit(Quadrat, split = "_")[[2]] == "9" &
             data.table::tstrsplit(Quadrat, split = "_")[[1]] != "9")) %>%
  filter(!((y==300 | x== 200) & data.table::tstrsplit(Quadrat, split = "_")[[1]] == "9" &
             data.table::tstrsplit(Quadrat, split = "_")[[2]] != "9")) %>%
  filter(!(x==500 & data.table::tstrsplit(Quadrat, split = "_")[[1]] == "0")) %>% 
  filter(!(y==300 & data.table::tstrsplit(Quadrat, split = "_")[[2]] == "0")) %>% 
  select(X, Y, x, y, SubPlot, Quadrat) %>% unique() %>% 
  arrange(x) 

# Test
ALT %>%
  group_by(SubPlot, Quadrat) %>% 
  summarize(N = n()) %>% 
  ungroup() %>% 
  # select(-SubPlot) %>% 
  filter(N!=4) %>% unique()
...
# Compute distance between geometre and previous corner coordinates 
```{r}
ggplot() +
  geom_sf(data = subplots_cornsf, col="darkred", size = 1) + # coordonées théoriques des piquets
  geom_sf(data = geometre_coordsf, col="darkgreen", size = 0.8) + # geometre
  ggrepel::geom_text_repel(data = subplots_corn %>% 
                             select(-SubPlot) %>% unique(),
                           aes(x = X, y = Y, label = ID), color = "darkred", size = 2) +
  ggrepel::geom_text_repel(data = geometre_coord %>% 
                             select(-SubPlot) %>% unique(),
                           aes(x = X, y = Y, label = ID), color = "darkgreen", size = 2)


dist_err <- subplots_cornsf %>% 
  mutate(ID = factor(ID, levels = geometre_coordsf$ID)) %>% 
  arrange(ID) %>% 
  st_distance(.,geometre_coordsf, by_element = T)
...
```{r}
projQuadrat <- data.frame() ; projQuadratS <- data.frame() ; projQuadratQ <- data.frame()

for(S in unique(Understory$Subplot)){
  # S = 18
  for(Q in unique(Understory$Quadrat)){ # par quadrat
    projQuadratQ <- bilinear_interpolation(
      coord = ALT[ALT$SubPlot == S & ALT$Quadrat == Q, c("x","y")], # quadrats theoric relative coord
    ...
    projQuadratS <- rbind(projQuadratS, projQuadratQ)
  }
  projQuadrat <- rbind(projQuadrat, projQuadratS)
...
```{r}
projQuadratsf <- st_as_sf(projQuadrat, coords = c('X','Y'))
projQuadratsf <- st_set_crs(projQuadratsf, crs(P16shp))


ggplot() +
  geom_sf(data = ALTsf, col="grey", size = 0.8) + # points to reproject
  geom_sf(data = geometre_coordsf, col="red", size = 1.5) + # reference
  geom_sf(data = projQuadratsf, aes(col=as.factor(SubPlot)), size = 0.8) + # reprojection
  labs(col= "SubPlot") +
  theme_classic() +
...
# first <- sort(unique(Understory$Quadrat))[1:30] # 100
cols <- rainbow(100, s=.6)[sample(1:100,100)] # , v=.9

Understorysf %>% 
  filter(Subplot == 13) %>% 
  # filter(Quadrat %in% first) %>%
  filter(Quadrat == "6_2" | Quadrat == "6_3") %>%
  ggplot() +
  geom_sf(aes(col=as.factor(Quadrat)), size = 3, shape=20) + 
  labs(col= "Quadrat") +
...
```{r}
projQuadrat <- data.frame() ; projQuadratS <- data.frame()
projTrees <- data.frame()

# Reprojeter chaque quadrat dans son subplot
for(S in unique(Understory$Subplot)){ # par SubPlot
  # S = 13 ; Q= "6_2"
  for(Q in unique(Understory$Quadrat)){ # par quadrat
    projQuadratQ <- bilinear_interpolation(
      coord = ALT[ALT$SubPlot == S & ALT$Quadrat == Q, c("x","y")], # quadrats theoric relative coord
    ...
    projTreesS <- bilinear_interpolation(
      coord = Understory[Understory$Subplot == S & Understory$Quadrat == Q, c("Dist.X","Dist.Y")], # Trees quadrats relative coord 
      from_corner_coord = data.frame(x = c(0, 0,10, 10),
                                     y= c(0,10, 0, 10)),  # quadrats theoric relative coord
      # from_corner_coord = ALT[ALT$SubPlot == S & ALT$Quadrat == Q, c("x","y")],  # quadrats theoric relative coord
      to_corner_coord = projQuadratQ[, c("X","Y")], # quadrats UTM coord
      ordered_corner = F) %>% 
      mutate(SubPlot = S) %>% 
      mutate(Quadrat = Q)
...
projTreessf %>% 
  ggplot() +
  # geom_sf(data = Understorysf, col="grey", size = 0.5) + # points to reproject
  geom_sf(aes(col= as.factor(Quadrat)), size = 0.3) + # reprojection
  geom_sf(data = projQuadratsf, col="black", size = 0.8) + # reprojection
    scale_color_manual(values=cols) +
  labs(col= "Quadrat") +
  theme_classic() +
  guides(color = F) +
...
ggsave("Trees_per_Quadrat_afterwarping.png",
       path = "D:/Mes Donnees/PhD/Figures/inventory",
...
projTreessf %>% 
  ...
  geom_sf(data = Understorysf %>% 
            filter(Subplot ==13) %>%
            filter(Quadrat == "6_2" | Quadrat == "6_3"), col="grey", size = 0.5) + # points to reproject
  geom_sf(aes(col=as.factor(Quadrat)), size = 1) + # reprojection
  geom_sf(data = projQuadratsf %>%
            filter(SubPlot ==13) %>%
