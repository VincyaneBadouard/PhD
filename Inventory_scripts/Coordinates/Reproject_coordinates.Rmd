...
ALT <- ALT %>% 
  ...
  mutate(X2 = ifelse(X2== "0", "9", X2)) %>%
  mutate(Y2 = ifelse(Y2== "0", "9", Y2)) %>% 
  mutate(X3 = ifelse(X3== "0" & X1== "0", "9", X3)) %>% 
  mutate(Y4 = ifelse(Y4== "0", "9", Y4)) %>% 
...
ALT %>% 
  filter(Quadra == "2_6") %>% 
  ggplot(aes(x= x, y=y)) +
  geom_point(aes(col=Quadra), size = 2) + # points to reproject
...
```{r}
projQuadra <- data.frame()

for(S in c(13,14,15,18,19,20,23,24,25)){
  # S = 18
  projQuadraS <- bilinear_interpolation(
    coord = ALT[ALT$SubPlot == S, c("x","y")], # quadrats theoric relative coord
...
for(S in unique(Understory$Subplot)){ # par subplot
  ...
  for(Q in unique(Understory$Quadra)){ #  à faire par quadrat
    projTreesS <- bilinear_interpolation(
      coord = Understory[Understory$Subplot == S, c("Dist.X","Dist.X")], # Trees quadrats relative coord 
      from_corner_coord = ALT[ALT$SubPlot == S, c("x","y")],  # quadrats theoric relative coord
      to_corner_coord = projQuadraS, # quadrats UTM coord
      ordered_corner = T) %>% 
      mutate(SubPlot = S)  %>% 
      mutate(Quadrat = Q)
...
Understorysf <- st_as_sf(Understory[,c('Xutm','Yutm')],
                         coords = c('Xutm','Yutm'))
...
dist_err <- st_distance(Understorysf, projCoordsf, by_element = T)
...
ggplot() +
  ...
  geom_sf(data = projCoordsf, aes(col=as.factor(SubPlot)), size = 0.5) + # reprojection
  labs(col= "SubPlot") +
  theme_classic() +
  ggtitle("After warping")
```
