...
# Manage multistems (sum the diameters, and keep only 1 stem)
```{r}
unique(data$Comment) 
unique(data$Comments)

test <- data %>% 
  filter(grepl("Duplicated 'IdStem' in the census", Comment)) %>% 
    filter(grepl("Missing value in XTreeUTM", Comment)) %>% 
  group_by(TreeID) %>% 
  arrange(desc(DBH)) %>% 
