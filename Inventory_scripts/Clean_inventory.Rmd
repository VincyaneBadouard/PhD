...
UnderstoryP <- 
  # read_csv("~/PhD/Inventories/Data/Understory/Paracou/ALT_Paracou9ha_20240906F.csv") # utilisé au départ
...
sum(is.na(UnderstoryP$Strategy)) # 6874
...
```{r}
data %>%
  filter(DBH==0) # 1 arbre

data <- data %>% # UnderstoryP
  filter(DBH>=1 | is.na(DBH)) # P : 41999/42479, N : 29010
# names(data)[sapply(data, anyNA)]

mycols <- data %>% select("IdTree","Stem.nb", 
                          "DBH", "Dist.X", "Dist.Y", "ScientificName") # "X", "Y"
...
# Missing DBH
data %>% filter(is.na(DBH) & is.na(Guyafor.nb)) %>% select("Guyafor.nb", "IdTree","Stem.nb", 
                                                           "DBH","Dist.X", "Dist.Y", "ScientificName") #  31 ALT
data %>% filter(is.na(DBH) & !is.na(Guyafor.nb)) %>% select("Guyafor.nb", "IdTree","Stem.nb", 
                                                            "DBH","Dist.X", "Dist.Y", "ScientificName") #  41 cirad

# Missing ID
data %>% filter((is.na(IdTree) | is.na(Stem.nb)) & is.na(Guyafor.nb)) %>% select("Guyafor.nb", "IdTree","Stem.nb", 
                                                                                 "DBH","Dist.X", "Dist.Y", "ScientificName") # 1

data %>% filter((is.na(IdTree) | is.na(Stem.nb)) & !is.na(Guyafor.nb)) %>% select("Guyafor.nb", "IdTree","Stem.nb", 
                                                                                  "DBH","Dist.X", "Dist.Y", "ScientificName") # 0
# Missing coordinates
data %>% filter((is.na(Dist.X) | is.na(Dist.Y)) & is.na(Guyafor.nb)) %>% select("Guyafor.nb", "IdTree","Stem.nb", 
                                                                                "DBH","Dist.X", "Dist.Y", "ScientificName") # 15
data %>% filter((is.na(Dist.X) | is.na(Dist.Y)) & !is.na(Guyafor.nb)) %>% select("Guyafor.nb", "IdTree","Stem.nb", 
                                                                                 "DBH","Dist.X", "Dist.Y", "ScientificName") # 17
```
...
```{r}
# Check duplicated IdStem
data[!is.na(data$IdStem) & duplicated(data$IdStem),] # oui il y a des IdStem dupliqués (158)

source("~/PhD/R_codes/PhD/Inventory_scripts/Functions/GeneralErrorsDetection.R")

# rename columns for the function
Data <- data %>%
  select(-Comment) %>% # Léa a gardé la colonne 
  rename(XTreeUTM=Xutm, YTreeUTM=Yutm, Diameter=DBH)
...
duplistems <- Data %>% filter(!is.na(IdStem)) %>% filter(grepl("Duplicated 'IdStem'", Comment)) %>%
  select(Comment, IdTree, IdStem, Guyafor.nb, Diameter, Genus, ScientificName, XTreeUTM, YTreeUTM, Code_problem) # 348

dupli <- unique((duplistems %>% filter(grepl("Duplicated row", Comment)))$IdStem) # 127

length(unique(duplistems$IdStem)) # 195
length(unique(duplistems$IdTree)) # 132
```
...
data <- data %>% 
  ...
  mutate(suplot = tstrsplit(IdTree, split = "_")[[1]]) %>% #1er elmt  
...
all(na.omit(data$suplot == data$Subplot)) # il ya des erreurs dans les n° de subplots les IdTree
...
Adults$CIRAD %in% data$CIRAD | Adults$CIRAD %in% data$Guyafor.nb # Guyafor.nb has some different suplot number than SubPlot
...
# Stem number
```{r}
all(data[!is.na(data$IdTree),]$Stem.nb==1)
stems <- data[!is.na(data$IdTree) & data$Stem.nb!=1,]$IdTree # stem id >1
stem1 <- data[data$IdTree %in% stems & data$Stem.nb==1,]$IdTree # stem id == 1

#!! ATTENTION trees without stem 1 !!
test <- as.data.frame(unique(stems[!stems %in% stem1])) # 47 trees with multiple stems but without a stem 1
write.csv(test, "~/PhD/Inventories/Data/Understory/Paracou/Problems/StemNb_1missing.csv")
...
(nrow(stems) + (nrow(duplistems)-length(unique(duplistems$IdStem))))/nrow(data)*100 # 0.09 % que stems,  1.5 % avec les stems mal codées

duplistems <- duplistems %>% rename(DBH=Diameter, Xutm = XTreeUTM, Yutm = YTreeUTM) # multistems probables

totalmultistems <- stems %>% bind_rows(duplistems) %>% filter(!is.na(ScientificName))
length(unique(totalmultistems$IdTree))/length(unique(data$IdTree))*100 # 1299 trees; 1.4%

nrow(totalmultistems %>% filter(is.na(Guyafor.nb)) %>% select(IdTree) %>% unique())/length(unique(data$IdTree))*100 # 433; 1.2% ALT

nrow(totalmultistems %>% filter(!is.na(Guyafor.nb)) %>% select(IdTree) %>% unique())/length(unique(data$IdTree))*100 # 46; 0.1 % Cirad

Multistems_sp <- duplistems %>%  # sans stems pcq il sont apparemment dupliqué (Code_problem)
  select(IdTree, ScientificName) %>%  unique() %>% 
  group_by(ScientificName) %>%
...
multistems_plot <- duplistems %>% 
  select(IdTree, ScientificName, Xutm, Yutm) %>%  unique() %>% 
...
on se fie qu'au DBH et aux idTree, la numérotation des stems contient des erreurs
...
data <- data %>% 
  ...
  mutate(Multistem = ifelse(length(table(DBH)) > 1 & 
...
# Les NA
test4 <- data %>% 
  filter(is.na(Multistem)) # the not sure cases (162) (soit pas même ScientificName ou coordonnées, soit memes DBHs pour tout l'idTree, soit certains DBH sont dupliqués mais pas tous)

write.csv(test4, "~/PhD/Inventories/Data/Understory/Paracou/Problems/Multistems_notsure.csv")
length(unique(test4$IdTree)) # 54 trees
```

# Manage multistems (sum the basal area, and keep only 1 stem) (Stem.nb pas codé)
**que fait on des Multistems= NA ?!**
```{r}
...
Multistems <- data %>% 
  ...
  distinct(IdTree, DBH, .keep_all = T) %>% # Résolution cas 2: remove duplicates -> clean real multistems
...
data <- data %>% 
  filter(!Multistem & !is.na(Multistem)) %>% # supprimer les Multistem=T
  mutate(DBHcor = DBH) %>%
...
Take only sp >= 30 ind
```{r}
SpeciesA <- (read.csv("~/PhD/Inventories/Data/Adults/Paracou/AdultsP16_species_filtred.csv")[,2])

data <- data %>% 
  rowwise() %>% 
  filter(ScientificName %in% SpeciesA) %>%
  ungroup() %>% 
...
```{r}
nrow(data) # 15409
length(unique(data$ScientificName)) # 55
