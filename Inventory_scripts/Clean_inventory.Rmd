...
sum(is.na(UnderstoryN$Strategy)) # 20000
```
...
mycols <- data %>% select("TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName") # "ScientificName" "X", "Y"
...
data %>% filter(is.na(DBH) & is.na(Guyafor.nb)) %>% select("Guyafor.nb", "TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName") #  31 ALT
data %>% filter(is.na(DBH) & !is.na(Guyafor.nb)) %>% select("Guyafor.nb", "TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName") #  92 cirad

# Missing ID
data %>% filter((is.na(TreeID) | is.na(StemID)) & is.na(Guyafor.nb)) %>% select("Guyafor.nb", "TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName") # 0

data %>% filter((is.na(TreeID) | is.na(StemID)) & !is.na(Guyafor.nb)) %>% select("Guyafor.nb", "TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName") # 13
# Missing coordinates
data %>% filter(((is.na(Xutm) | is.na(Yutm)) & (is.na(X) | is.na(Y))) & is.na(Guyafor.nb)) %>% select("Guyafor.nb", "TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName")
data %>% filter(((is.na(Xutm) | is.na(Yutm)) & (is.na(X) & is.na(Y))) & !is.na(Guyafor.nb)) %>% select("Guyafor.nb", "TreeID","StemID", 
                          "DBH","Xutm","Yutm", "X", "Y", "All_ScientificName")
...
Data %>% filter(Comment=="Different botanical informations (Family, ScientificName) for a same IdTree") %>%
  select(Comment, IdTree, IdStem, Guyafor.nb, DBH, Family, Genus, Species, ScientificName)
...
```{r}
data %>%
  filter(!is.na(All_ScientificName)) %>% 
  filter(All_ScientificName!="Tres haut") %>% 
  filter(!grepl("indet", All_ScientificName)) %>% 
  filter(!grepl("Indet", All_ScientificName)) %>% 
  filter(!grepl("aceae", All_ScientificName)) %>% 
  group_by(All_ScientificName) %>%
  summarise(N = n()) %>%
  ungroup() %>% 
  filter(N<200) %>% 
...
```
```{r}
length(unique(test$All_ScientificName))
...
data %>%
  ...
  summarise(N = n()) %>%
  ungroup() %>% 
  mutate(Cut = cut(N, c(0, 30, 50, 100, 2500))) %>% 
  ggplot(aes(x= Cut)) + # fill = as.factor(Strategy)
  geom_histogram(stat= "count",fill="#69b3a2", color="#e9ecef", alpha=0.9) +
  geom_vline(xintercept = 30) +
  # scale_x_continuous(limits = c(0, 2500), breaks =  c(0, 30, 50, 100, 2500)) +
  scale_y_continuous(breaks = seq(0,550, by=100)) +
  labs(x= "Individuals number",y= "Species number")

# ggsave(paste("ALT", unique(data$Site), unique(data$Plot),"Species_abundances_class.png", sep="_"),
#        path = "D:/Mes Donnees/PhD/Figures/inventory", width = 20, height = 20, units = "cm", dpi=800, bg="white")
```
...
```{r}
DuplicatedID <- unique(UnderstoryP[duplicated(UnderstoryP),]$StemID)

UnderstoryP[UnderstoryP$StemID%in%DuplicatedID,]

UnderstoryP <- UnderstoryP[!duplicated(UnderstoryP),] # 42524 -> 42366 (158 rows removed)
UnderstoryN <- UnderstoryN[!duplicated(UnderstoryN),] # 29370 -> 29363 (7 rows removed)
```
