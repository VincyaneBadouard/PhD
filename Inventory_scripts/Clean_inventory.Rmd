...
data <- data %>% 
  ...
  mutate(Multistem = ifelse(grepl("False pas double tige", Comments), F, Multistem)) %>%  # according to the Comments
  ungroup()
...
data <- data %>% 
  filter(!Multistem & !is.na(Multistem)) %>% # supprimer les Multistems=T
  mutate(DBHcor = DBH) %>%
...
```{r}
nrow(data) # 32793
length(unique(data$ScientificName)) # 184
