...
data %>% 
  ...
  filter(All_ScientificName == "Maytenus_oblongata")
group_by(Size) %>% 
...
for(p in 1:length(unique(data$All_ScientificName))){
  ...
  plist[[p]] <- local({
    ...
    ggplot(data, aes(x= Elevation)) +
      geom_histogram(binwidth=1 , aes(y=..density.., color= Size, fill= Size),
                     alpha=0.2) + #  binwidth=1, aes(y=..density..), 
      ...
      guides(fill = FALSE) +
      labs(title = unique(data$All_ScientificName),
           x= "Elevation (m)", y="Density") # Density
...
ggsave("Species_MNT_niche_density_hist.pdf", plots, width = 15, height = 10)
```
...
for(p in 1:length(unique(data$All_ScientificName))){
  ...
  plist[[p]] <- local({
    ...
    ggplot(data, aes(x= Elevation, color= Size, fill= Size)) +
      ...
      geom_text(data = label, aes(label = N),
                x = -Inf, y = Inf, hjust = -0.2, vjust = 1.5, show.legend = F) +
      # annotate(geom = 'text', label = paste("N: ",round(max(data$DBH),0),"cm"),
...
ggsave("Species_MNT_niche_density.pdf", plots, width = 15, height = 10)
...
for(p in 1:length(unique(data$All_ScientificName))){
  ...
  plist[[p]] <- local({
    ...
    ggplot(data, aes(x= Elevation, color= Size, fill= Size)) +
      geom_freqpoly(binwidth=1, aes(y=..density..)) + # frequency polygons display the counts with lines.
      # geom_point(aes(y=..density..)) +
      # geom_histogram(binwidth=1, aes(y=..density..), alpha=0.1,
      #                position="identity")+
      # geom_density(alpha=0.4) + # aes(y = after_stat(count)),
      geom_vline(aes(xintercept=Elev_med, color=Size), linetype="dashed", size=0.8) +
...
ggsave("Species_MNT_niche_freqpol.pdf", plots, width = 15, height = 10)
...
# ggsave("Species_MNT_niche_boxplot_test.pdf", plots, width = 15, height = 10)

```

```{r}
data %>% 
  filter(All_ScientificName=="Hymenopus_heteromorphus") %>% 
  ggplot(aes(y= Elevation, x= DBH, color= DBH)) +
  geom_point()
