...
Inventory <- read.csv(
  ...
  dplyr::select(idTree, SubPlot, ScientificName, DBHcor, Xutm, Yutm, TreeHeight) %>% 
  st_as_sf(coords = c("Xutm", "Yutm")) %>% 
  st_set_crs(st_crs(MNT))
```
...
alt <- extract(MNT, Inventory) %>% 
  rename(Elevation = dtm2023_HighAlt_25ha_buffer) %>% 
  dplyr::select(-ID)
# extract twi
twi <- extract(TWI, Inventory) %>% 
  rename(TWI = `Paracou_TWI_5m`) %>% dplyr::select(-ID)
# extract re
re <- extract(elev, Inventory) %>%  rename(HAND = re) %>% dplyr::select(-ID) 
...
```{r}
write.csv(Inventory, "D:/Mes Donnees/PhD/Inventories/Data/Understory/Paracou/ALT_Paracou_9ha_andEnv.csv")
```
