...
```{r}
warning("Recalculer TWI & HAND avec buffer si encore des NA") # !!!!!

# Topography
MNT <- terra::rast("~/PhD/Env_data_forModel/dtm2023_HighAlt_25ha_buffer.asc")

# TWI
TWI <- terra::rast("D:/Mes Donnees/PhD/R_codes/PhD/test/Plot16_25ha_buffer_TWI_5m.tif")

# Height Above the Nearest Drainage (HAND)
relelev <- terra::rast("D:/Mes Donnees/PhD/R_codes/PhD/test/Plot16_25ha_buffer_RelativeElev.tif")
...
alt <- raster::extract(MNT, Inventory_sf) %>% 
  rename(Elevation = dtm2023_HighAlt_25ha_buffer) %>% 
  dplyr::select(-ID)
# extract twi
twi <- raster::extract(TWI, Inventory_sf) %>% 
  rename(TWI = `Paracou_TWI_5m`) %>% dplyr::select(-ID)
# extract re
re <- raster::extract(relelev, Inventory_sf) %>%  rename(HAND = re) %>% dplyr::select(-ID) 

Inventory <- Inventory %>% cbind(alt) %>% cbind(twi) %>% cbind(re) # 47 850
...
# Check
```{r}
any(is.na(Inventory$Elevation))
any(is.na(Inventory$TWI)) 
any(is.na(Inventory$HAND))
```

# Write
```{r}
write.csv(Inventory, "D:/Mes Donnees/PhD/Inventories/Data/Adults_Understory/Paracou/P16_2019_Paracou_InvandEnv.csv")
```
...
# Too small values of transmittance (Meth3)

```{r, eval=F}
Inventory <- read.csv("D:/Mes Donnees/PhD/Inventories/Data/Adults_Understory/Paracou/P16_Paracou_InvandEnv.csv") %>% 
