...
# unique(UnderstoryP$Year) # 2020-2023
# unique(UnderstoryP$CensusDate) # 2020-2023
...
sum(is.na(UnderstoryP$Strategy)) # 7825
sum(is.na(UnderstoryN$Strategy)) # 20345

UnderstoryP %>% filter(is.na(Strategy) & !grepl("indet", ScientificName)) %>% select(ScientificName) %>% unique() # 340 sp
```
...
Data <- Data %>% 
  ...
  mutate(ScientificName = ifelse(ScientificName=="Caraipa_idem181381",
                                 Data[!is.na(Data$IdTree) & Data$IdTree=="18_1381",]$ScientificName,
                                 ScientificName)) %>%
...
Data %>% filter(grepl('Special characters', Comment))  %>% # because of points
  select(Comment, Genus, Genus_TreeDataCor, Species, Species_TreeDataCor, ScientificName, ScientificName_TreeDataCor, Family, Family_TreeDataCor) 

Data %>% filter(grepl('Names ending in', Comment))  %>% # 'aceae' in Genus
  # filter(!grepl('aceae', Genus)) %>%
  select(Comment, Genus, Genus_TreeDataCor,  ScientificName, ScientificName_TreeDataCor, Species, Family, Family_TreeDataCor) 
```
...
MissingCirad <- Adults %>% 
  filter(!(CIRAD %in% data$CIRAD|CIRAD %in% data$Guyafor.nb)) # 245 trees → c’est ok

length(unique(MissingCirad$CIRAD)) 
...
# Répartition spatiale des multistems
```{r}
...
```{r recruts}
recruts <- data %>% 
  filter(is.na(Guyafor.nb) & DBH >=10) %>% 
  select(ScientificName, DBH, Comments, Code_problem)
unique(recruts$Code_problem)
no_problem <- recruts %>% filter(is.na(Code_problem))
Not_ided <- recruts %>% filter(Code_problem=="Not_ided")
cirad <- recruts %>% filter(grepl("num_cirad", Code_problem))

hist(no_problem$DBH)
hist(Not_ided$DBH)
hist(cirad[cirad$DBH<20,]$DBH)
```
...
data_filtred <- data %>% 
  ...
  filter(DBH>=1 & is.na(Guyafor.nb) & DBH<20 &
           !grepl("num_cirad", Code_problem) &
           Code_problem!="TRUE") %>% # filter ce qui a un guyafor.nb pas bcp plus grand que 10 cm (recrus) (pas sous le critère <10 cm dbh pcq recrus) -> reste 88% par rapport aux filtres précédents
  # Only alive trees
...
data_filtred[!is.na(data_filtred$IdTree) & duplicated(data_filtred$IdTree),] # duplicated IdTree 0

```

# Write cleaned data
```{r}
nrow(data_filtred) # 36311
...
Take only sp >= 30 ind
```{r}
SpeciesA <- (read.csv("~/PhD/Inventories/Data/Adults/Paracou/AdultsP16_species_filtred.csv")[,3]) # 64

spSelection <- data_filtred %>% 
  rowwise() %>% 
  filter(ScientificName_TreeDataCor %in% SpeciesA | Strategy=="Understory species") %>%
  ungroup() %>% 
  # Only botanical identified
...
unique(spSelection$Comment) 
unique(SpeciesA[!SpeciesA %in% spSelection$ScientificName_TreeDataCor])
...
Sp <- spSelection %>% 
  select(ScientificName_TreeDataCor, ScientificName, Family, N) %>%
 distinct(ScientificName_TreeDataCor, .keep_all = T) %>% arrange(desc(N)) # 75

length(unique(Sp$Family)) # 28 families

Sp %>% filter(ScientificName != ScientificName_TreeDataCor)
