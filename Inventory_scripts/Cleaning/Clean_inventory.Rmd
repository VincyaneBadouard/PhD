...
Data <- Data %>% 
  mutate(ScientificName = recode(ScientificName,
                                 ...
                                 "Maytenus_oblongata" = "Monteverdia_oblongata",
                                 "Garcinia.benthaminia" = "Garcinia.benthamiana",
                                 "Eugenia.sp.FG9" = "Eugenia.sp.FG9-Holst")) %>% 
...
```{r}
nrow(data) # 42335
length(unique(data$ScientificName)) # 684
```
...
# Write cleaned data
```{r}
nrow(data_filtred) # 36310
length(unique(data_filtred$ScientificName)) # 581
...
Sp <- as.data.frame(sort(unique(spSelection$ScientificName))) # 75
