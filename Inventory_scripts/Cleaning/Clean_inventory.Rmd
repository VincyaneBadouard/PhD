...
length(Data$Comment[grepl("TreeUTM", Data$Comment)])/nrow(Data)*100 # 0.5%
...
```{r}
length(Data$Comment[grepl("Duplicated 'IdStem'", Data$Comment)])/nrow(Data)*100 # 0.6%
...
length(Data$Comment[grepl("Different botanical informations", Data$Comment)])/nrow(Data)*100 # 0.04%
...
test <- as.data.frame(unique(stems[!stems %in% stem1])) # 6 trees with multiple stems but without a stem 1
