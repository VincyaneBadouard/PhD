...
# Sure multistem
nrow(data %>% 
       filter(Multistem & !is.na(Multistem)))/nrow(data)*100 # 3.2% 
stemssup1 <- data %>% 
...
# Cod.CTFS M
nrow(data %>% 
       filter(grepl("M", Cod.CTFS)))/nrow(data)*100 # 3.5%
...
data <- data %>% 
  ...
  filter(DBH>=1 & is.na(Guyafor.nb) & DBH<20) %>% # filter ce qui a un guyafor.nb pas bcp plus grand que 10 cm( recrus) (pas sous le critère <10 cm dbh pcq recrus)
  # Only alive trees
...
```{r}
nrow(data) # 35898
length(unique(data$ScientificName)) # 579
...
spSelection <- data %>% 
  ...
  # Only botanical identified
  filter(!is.na(ScientificName)) %>%
  # filter(ScientificName!="Tres haut") %>% # on fait ça ? apriori oui
  # filter(!grepl("indet", ScientificName)) %>%
  # filter(!grepl("Indet", ScientificName)) %>%
  # filter(!grepl("aceae", ScientificName)) %>%
...
Sp <- as.data.frame(sort(unique(spSelection$ScientificName))) # 86
names(Sp) <- "ScientificName"
