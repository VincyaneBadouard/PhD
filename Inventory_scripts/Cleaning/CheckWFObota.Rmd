...
```{r synonym}
nrow(ALT %>% filter(Old.status=="Synonym"))/nrow(ALT)*100 # 8% (60 sp)
nrow(Guyafor %>% filter(Old.status=="Synonym"))/nrow(Guyafor)*100 # 8.3% (36 sp))

nrow(ALT %>% filter(spec.name.ORIG!=scientificName & is.na(Old.status)))/nrow(ALT)*100 # 21% des cf, sp num, correction ortho ! attention donc a ce qu'on prend dans scientificName
nrow(Guyafor %>% filter(spec.name.ORIG!=scientificName & is.na(Old.status)))/nrow(Guyafor)*100 # 10% des points, sp

# -> prendre scientificName que si Old.status=="Synonym"
# -> faire attention il reste des points voir comment gérer
