...
```{r}
MNT <- terra::rast("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/HighAltitudeFlight/dtm2023_4ha_HighAlt_buffer.asc") # MNT

plot(MNT) ; plot(LAI2200UTMsf, add = T)

alt <- extract(MNT, LAI2200UTMsf) %>% # extract z
  rename(Zutm = dtm2023_4ha_HighAlt_buffer) %>% 
  select(-ID) %>% 
  mutate(Zutm = Zutm +1) # sensors at 1 m height

LAI2200UTMxyz <- LAI2200UTM %>% cbind(alt)
...
```{r}
LAI2200UTMxyz <- as.data.table(read.csv("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/LAI2200_theoretical_UTMxyz.csv"))
VOP <- as.matrix(read.table("//amap-data.cirad.fr/work/users/VincyaneBadouard/Lidar/ALS2023/VOP_P16_9ha.txt"))

matrix <- as.matrix(LAI2200UTMxyz[, `:=`(c = 0 , d = 1)][,.(Xutm, Yutm, c, d)]) # as matrix of the same dimensions, without Z

LAI2200local <- matrix %*% t(VOP) # same dimensions

LAI2200local <- cbind(LAI2200local,LAI2200UTMxyz[,Zutm])[, -c(3,4)] # add Zutm
