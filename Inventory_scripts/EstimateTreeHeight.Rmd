...
# slist <- vector('list', length(unique(Understory$All_ScientificName)) )

for(s in unique(Understory$All_ScientificName)){
  ...
  if(nrow(Sdata)>=15){
    ...
    Understory <- Understory %>%
      mutate(TreeHeight = ifelse(All_ScientificName == s,
...
```{r}
unique((Understory %>% filter(is.na(TreeHeight)) %>% select(All_ScientificName, Strategy))) # 6,005 ind, 68 sp, 1-51 cm DBH
