...
P16 <- EcoFoG::Guyafor2df(UID = "Vincyane.Badouard", PWD = "Vincyane973+", Driver = "SQL Server",WHERE="Forest='Paracou' AND Plot='16'") %>% 
  ...
  select(idTree, Plot, ScientificName) %>% unique()
```
...
test <- CC %>% 
  ...
  select(idTree, DBH, Height, CrownRadius, ScientificName) %>% unique()

nrow(CC);nrow(test)

length(unique(test$idTree)) # 13863*5 = 69315

length(sort(unique(test$ScientificName))) # ya des sp, indet
```
