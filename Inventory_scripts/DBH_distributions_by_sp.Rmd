...
Understory <- Understory %>% 
  ...
  select(Dataset, idTree, CIRAD.BigTreeID, Family, Genus, Species, DBH, POM, CodeAlive) %>% 
  # Only alive trees
  filter(CodeAlive == T) %>%
...
Adults <- Adults %>% 
  ...
  select(Dataset, idTree, Family, Genus, Species, DBH, MeasCode, CodeAlive)
...
data <- Understory %>% 
  ...
  filter(!grepl("Indet", ScientificName)) %>% 
  filter(Genus=="Xylopia")

ggplot(data, aes(x=DBH)) +
  geom_histogram(binwidth=1) +
  facet_wrap(vars(ScientificName), scales = "free")
...
for(p in 1:(ceiling(length(unique(sp))/9))){
  print(
    ggplot(Understory, aes(x=DBH)) +
      geom_histogram(binwidth=1) +
      facet_wrap(vars(ScientificName), scales = "free")
  ...
  if(p < ceiling(length(unique(sp))/9))
    dev.new()
