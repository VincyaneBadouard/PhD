...
```{r}
sort(unique(Understory$All_ScientificName))
...
ggplot(data, aes(x=DBH)) +
  geom_histogram(binwidth=1, fill="#69b3a2", color="#e9ecef", alpha=0.9) +
  facet_wrap(vars(All_ScientificName, N, Strategy), scales = "free") +
...
for(p in 1:(ceiling(length(unique(sp))/16))){
  print(
    ggplot(Understory, aes(x=DBH)) +
      ...
      ggforce::facet_wrap_paginate(vars(All_ScientificName, N, Strategy),
                                   scales = "free",
